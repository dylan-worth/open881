...
public class SerializableFilesManager {
	...
	private static final String GE_OFFERS = "./lists/ge/grandExchangeOffers.ser";
	private static final String GE_OFFERS_HISTORY = "./lists/ge/grandExchangeOffersTrack.ser";
	private static final String GE_PRICES = "./lists/ge/grandExchangePrices.ser";

	/**
	 * Path where offences are stored.
	 */
	private static final String OFFENCES = ".lists/offences.ser";
	/**
	...
	public static synchronized HashMap<Long, Offer> loadGEOffers() {
		if (new File(GE_OFFERS).exists()) {
			...
			try {
				new File(GE_OFFERS).createNewFile();
				Logger.log("Launcher", "New GE Offers file made");
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static synchronized ArrayList<OfferHistory> loadGEHistory() {
		if (new File(GE_OFFERS_HISTORY).exists()) {
			...
			try {
				new File(GE_OFFERS_HISTORY).createNewFile();
				Logger.log("Launcher", "New GE Offers history file made");
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static synchronized HashMap<Integer, Integer> loadGEPrices() {
		if (new File(GE_PRICES).exists()) {
			...
			try {
				new File(GE_PRICES).createNewFile();
				Logger.log("Launcher", "New GE prices file made");
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static final Object loadSersializedFile(File f) throws IOException, ClassNotFoundException {
		if (!f.exists()) {
			return null;
		}
