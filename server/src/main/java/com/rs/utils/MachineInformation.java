...
public class MachineInformation {
	...
	public MachineInformation(int os, boolean x64Arch, int osVersion, int osVendor, int javaVersion, int javaVersionBuild, int javaVersionBuild2, boolean hasApplet, int heap, int availableProcessor, int ram, int cpuClockFrequency, int cpuInfo3, int cpuInfo4, int cpuInfo5) {
		...
		if (this.javaVersion == 0 && this.javaVersionBuild == 0) {
			// Java 11+
			this.javaVersion = this.javaVersionBuild2;
			this.javaVersionBuild2 = 0;
		}
	...
	public void sendSuggestions(Player player) {
		...
		/*
			* else if(javaVersionBuild != 0 || javaVersionBuild2 < 31) { title = "Outdated Java Version";
			* suggestion = "Your java seems outdated: "+getVersion ()+".<br>You should update your to 6.0.31.";
			*//*
			* }else if (hasApplet && ((availableProcessors <= (x64Arch ? 2 : 1)) || ram <= (x64Arch ? 1024 : 512)
			* || cpuClockFrequency <= 1500)) { title = "Weak Specs"; suggestion =
