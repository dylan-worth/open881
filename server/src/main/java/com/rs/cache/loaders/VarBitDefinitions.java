...
public final class VarBitDefinitions {
	...
	public static final void main(String[] args) throws IOException {
		Cache.init();
		System.out.println("There are currently: " + Cache.STORE.getIndexes()[2].getLastFileId(69) + " bitConfigs.");
		...
		for (int i = 0; i < Cache.STORE.getIndexes()[2].getLastFileId(69) + 1; i++) {
			...
			/*
			 * if (i == 26172) { System.out.println("REVERSED: "+cd.baseVar+", "+cd.id); }
			 */
			if (cd.baseVar == 682) {
				int value = 58720288;
				int v = value >> cd.startBit & VarsManager.masklookup[cd.endBit - cd.startBit];
				System.out.println("BitConfig " + cd.getVarDomainName() + ": " + i + ", from bitshift:" + cd.startBit + ", till bitshift: " + cd.endBit + ", " + cd.baseVar + ", " + v);
		...
		// System.out.println("from bitshift:" + cd.startBit + ", till bitshift:
		// " + cd.endBit + ", " + cd.baseVar+", "+cd.getVarDomainName());
	}
