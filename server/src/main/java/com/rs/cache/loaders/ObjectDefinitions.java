...
public class ObjectDefinitions {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * for (int i = 0; i < defs.options.length; i++) if (defs.options[i] != null) System.out.println("[" + i +
		 * "]:" + defs.options[i]);
		 */

		System.out.println(defs.objectAnimation);
		System.out.println(defs.configId);
		System.out.println("Var config: " + defs.configFileId + ", " + defs.configId);
	...
	public static ObjectDefinitions getObjectDefinitions(int id) {
		...
		if (def == null) {
			...
			def.method3287();
			/*
			 * if(def.name.equalsIgnoreCase("bank booth") || def.name.equalsIgnoreCase("counter")) { def.notCliped
			 * = false; def.projectileCliped = true; if (def.clipType == 0) def.clipType = 1; } else if
			 * (DungeonUtils.isDoor(id) || DungeonUtils.isBossDoor(id)) { def.notCliped = false;
			 * def.projectileCliped = true; if (def.clipType == 0) def.clipType = 1; } if (def.notCliped) {
			 * def.projectileCliped = false; def.clipType = 0; }
			 */
