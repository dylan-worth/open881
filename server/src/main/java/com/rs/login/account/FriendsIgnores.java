...
public class FriendsIgnores implements Serializable {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_PMSTATUS, pmStatus)
							.getBuffer());
	...
	public void setFcName(String newName, boolean updatePlayer) {
		fcName = newName;
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
					.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCNAME, fcName).getBuffer());
	...
	public void setFcJoinReq(int newReq, boolean updatePlayer) {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCJOINREQ, fcJoinReq)
							.getBuffer());
		Login.onFriendsChatSettingsUpdate(account);
	}
	...
	public void setFcTalkReq(int newReq, boolean updatePlayer) {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCTALKREQ, fcTalkReq)
							.getBuffer());
		Login.onFriendsChatSettingsUpdate(account);
	}
	...
	public void setFcKickReq(int newReq, boolean updatePlayer) {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCKICKREQ, fcKickReq)
							.getBuffer());
		Login.onFriendsChatSettingsUpdate(account);
	}
	...
	public void setFcLootShareReq(int newReq, boolean updatePlayer) {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
					.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCLSREQ, fcLootshareReq)
					.getBuffer());
		Login.onFriendsChatSettingsUpdate(account);
	}
	...
	public void setFcCoinshare(boolean coinshare, boolean updatePlayer) {
		...
		if (updatePlayer)
			LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
					.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCCSSTATE, fcCoinshare ? 1 : 0)
					.getBuffer());
		Login.onFriendsChatSettingsUpdate(account);
	}
	...
	public void sendAll() {
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_PMSTATUS, pmStatus).getBuffer());
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCNAME, fcName).getBuffer());
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCJOINREQ, fcJoinReq).getBuffer());
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCTALKREQ, fcTalkReq).getBuffer());
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCKICKREQ, fcKickReq).getBuffer());
		LoginServerChannelManager.sendReliablePacket(account.getWorld(),
				LoginChannelsPacketEncoder
						.encodePlayerVarUpdate(account.getUsername(), LoginProtocol.VAR_TYPE_FCLSREQ, fcLootshareReq)
						.getBuffer());
		...
		while (it$.hasNext()) {
			...
			if (account != null && !isAccountOnline(account, account.getFriendsIgnores().getPmStatus()))
				account = null;
			LoginChannelsPacketEncoder.appendUpdateFriend(packet, false, displayName, Login.getDisplayName(user),
					account != null ? account.getWorld().getId() : 0, getRank(user),
					account != null ? ((account.isLobby() ? "Lobby " : "World ") + account.getWorld().getId()) : null);
		}
		...
		while (it$.hasNext()) {
			...
			LoginChannelsPacketEncoder.appendUpdateIgnore(packet, false, displayName, Login.getDisplayName(user));
		}
	...
	public void onAccountPmStatusChange(Account account, int previousStatus, int currentStatus) {
		...
		LoginChannelsPacketEncoder.appendUpdateFriend(packet, true, account.getDisplayName(),
				account.getPreviousDisplayName(), isOnline ? account.getWorld().getId() : 0,
				getRank(account.getUsername()),
				isOnline ? ((account.isLobby() ? "Lobby " : "World ") + account.getWorld().getId()) : null);
		LoginChannelsPacketEncoder.finishUpdateFriendsPacket(packet);
	...
	public void onAccountDisplayNameChange(Account account) {
		if (isFriend(account.getUsername())) {
			...
			OutputStream packet = LoginChannelsPacketEncoder.createUpdateFriendsPacket(this.account.getUsername());
			LoginChannelsPacketEncoder.appendUpdateFriend(packet, false, account.getDisplayName(),
					account.getPreviousDisplayName(), isOnline ? account.getWorld().getId() : 0,
					getRank(account.getUsername()),
					isOnline ? ((account.isLobby() ? "Lobby " : "World ") + account.getWorld().getId()) : null);
			LoginChannelsPacketEncoder.finishUpdateFriendsPacket(packet);
			...
			OutputStream packet = LoginChannelsPacketEncoder.createUpdateIgnoresPacket(this.account.getUsername(),
					false);
			LoginChannelsPacketEncoder.appendUpdateIgnore(packet, true, account.getDisplayName(),
					account.getPreviousDisplayName());
			LoginChannelsPacketEncoder.finishUpdateIgnoresPacket(packet);
			LoginServerChannelManager.sendReliablePacket(this.account.getWorld(), packet.getBuffer());
		}
	}
	...
	public void sendPrivateMessage(String username, String message) {
		...
		if (username == null) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							"Couldn't find player " + displayName + ".").getBuffer());
			return;
		}
		...
		if (!isFriend(username)) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							displayName + " is not on your friends list.").getBuffer());
			return;
		}
		...
		if (account.isMuted()) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
		...
		if (acc == null) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerFriendIgnoreSystemMessage(account.getUsername(), displayName + " is offline.")
							.getBuffer());
			return;
		}
		...
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerSentPrivateMessage(account.getUsername(), displayName, message).getBuffer());
		LoginServerChannelManager
				.sendReliablePacket(acc.getWorld(),
						LoginChannelsPacketEncoder.encodePlayerReceivedPrivateMessage(acc.getUsername(),
								account.getDisplayName(), Login.createNewUid(), account.getMessageIcon(), message)
								.getBuffer());
	}
	...
	public void sendPrivateMessage(String username, int qcFileId, byte[] qcData) {
		...
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), LoginChannelsPacketEncoder
				.encodePlayerSentPrivateMessage(account.getUsername(), displayName, qcFileId, qcData).getBuffer());
		LoginServerChannelManager.sendReliablePacket(acc.getWorld(),
				LoginChannelsPacketEncoder.encodePlayerReceivedPrivateMessage(acc.getUsername(),
						account.getDisplayName(), Login.createNewUid(), account.getMessageIcon(), qcFileId, qcData)
						.getBuffer());
	}
	...
	public void addFriend(String username) {
		if (friends.size() >= 200) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerFriendIgnoreSystemMessage(account.getUsername(), "Your friends list is full.")
							.getBuffer());
			return;
		}
		...
		if (username == null) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							"Couldn't find player " + username + ".").getBuffer());
			return;
		}
		...
		if (username.equals(account.getUsername())) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerFriendIgnoreSystemMessage(account.getUsername(), "You can't add yourself.")
							.getBuffer());
			return;
		}
		...
		if (isIgnore(username)) {
			LoginServerChannelManager
					.sendUnreliablePacket(account.getWorld(),
							LoginChannelsPacketEncoder
									.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
											"Please remove " + username + " from your ignores list first.")
									.getBuffer());
			return;
		}
		...
		if (isFriend(username)) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							username + " is already on your friends list.").getBuffer());
			return;
		}
		...
		OutputStream packet = LoginChannelsPacketEncoder.createUpdateFriendsPacket(account.getUsername());
		LoginChannelsPacketEncoder.appendUpdateFriend(packet, false, displayName, Login.getDisplayName(username),
				acc != null ? acc.getWorld().getId() : 0, getRank(username),
				acc != null ? ((acc.isLobby() ? "Lobby " : "World ") + acc.getWorld().getId()) : null);
		LoginChannelsPacketEncoder.finishUpdateFriendsPacket(packet);
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), packet.getBuffer());
	...
	public void addIgnore(String target, boolean temporary) {
		if ((ignores.size() + tempIgnores.size()) >= 100) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerFriendIgnoreSystemMessage(account.getUsername(), "Your ignores list is full.")
							.getBuffer());
			return;
		}
		...
		if (isFriend(target)) {
			LoginServerChannelManager
					.sendUnreliablePacket(account.getWorld(),
							LoginChannelsPacketEncoder
									.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
											"Please remove " + displayName + " from your friends list first.")
									.getBuffer());
			return;
		}
		...
		if (isIgnore(target)) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							displayName + " is already on your ignores list.").getBuffer());
			return;
		}
		...
		LoginChannelsPacketEncoder.appendUpdateIgnore(packet, false, displayName, Login.getDisplayName(target));
		LoginChannelsPacketEncoder.finishUpdateIgnoresPacket(packet);
		LoginServerChannelManager.sendReliablePacket(account.getWorld(), packet.getBuffer());
	}
	...
	public void removeIgnore(String target) {
		...
		if (!isIgnore(target)) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSystemMessage(account.getUsername(),
							displayName + " is not on your ignores list.").getBuffer());
			return;
		}
	...
	public void changeRank(String username, int newRank) {
		...
		if (getRank(username) != newRank) {
			...
			OutputStream packet = LoginChannelsPacketEncoder.createUpdateFriendsPacket(account.getUsername());
			LoginChannelsPacketEncoder.appendUpdateFriend(packet, true, displayName, Login.getDisplayName(username),
