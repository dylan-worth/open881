...
import com.rs.utils.saving.JsonFileManager;
...
public class Login {
	...
	public static void changeDisplayName(Account account, String newDisplayName) {
		...
		if (!displayNames.reserveDisplayName(account.getUsername(), newDisplayName)) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(), LoginChannelsPacketEncoder
					.encodePlayerGameMessage(account.getUsername(), "This name appears to be taken.").getBuffer());
			return;
		}
	...
	private static void sendAddOffenceReply(String moderator, String target, boolean successfull) {
		...
		if (account != null)
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(),
					LoginChannelsPacketEncoder
							.encodePlayerGameMessage(account.getUsername(), successfull
									? (target + " was successfully punished.") : "Could not punish " + target)
							.getBuffer());
	}
	...
	public static String getUsername(String displayname) {
		return Utils.formatPlayerNameForDisplay(displayname);
	}
