...
public class DisplayNames implements Serializable {
	...
	public synchronized void initReverseMapping() {
		userToDisplay = new HashMap<String, String>();
		Iterator<Map.Entry<String, String>> it$ = displayToUser.entrySet().iterator();
		while (it$.hasNext()) {
			Map.Entry<String, String> pair = it$.next();
			if (userToDisplay.containsKey(pair.getValue()))
				throw new RuntimeException("Display names are corrupted, reset is a must!");
	...
	public synchronized boolean reserveDisplayName(String username, String displayname) {
		if (displayToUser.containsKey(displayname) || JsonFileManager.isValidAccount(displayname))
			return false;
