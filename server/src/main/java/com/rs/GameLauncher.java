...
import com.rs.game.player.dialogues.impl.DialogueHandler;
...
import com.rs.utils.ScrollMessage;
...
public class GameLauncher {
	...
	public static void main(String[] args) throws Exception {
		...
		Logger.log("LoginEngine",
				"Login server took " + (Utils.currentTimeMillis() - currentTime) + " milli seconds to launch.");
		GameWorld world = Login.getWorld(1);
		LoginServerChannelManager.sendReliablePacket(world,
				LoginChannelsPacketEncoder.encodeConsoleMessage("Login Engine connected.").getBuffer());
		Logger.log("Launcher", "Initiating Settings...");
...
//		Main.init();
		...
		Logger.log("Launcher",
				"Game Server took " + (Utils.currentTimeMillis() - currentTime) + " milli seconds to launch.");
		addAutoSavingTask();
		addCleanMemoryTask();
	...
	public static boolean shutdown(int delay, boolean force, boolean reboot) {
		...
		Logger.log("GameLauncher", delay + " seconds remaining.");
	...
	public static void cleanMemory(boolean force) throws IOException {
		if (force) {
			...
			/*
			 * skip:for (Region region : World.getRegions().values()) { for(int
			 * regionId : MapBuilder.FORCE_LOAD_REGIONS) if(regionId ==
			 * region.getRegionId()) continue skip; region.unloadMap(); }
			 */
		}
