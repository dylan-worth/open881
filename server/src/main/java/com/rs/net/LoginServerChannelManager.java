...
import com.rs.net.decoders.LoginServerPacketsDecoder;
...
public class LoginServerChannelManager {
	...
	private static void run() {
		while (true) {
			try {
				...
				Logger.log("LoginServer", "LoginServer exit...");
				...
				//waiting for device to write packets
				//awaitQueue();
				
				
				Logger.log("LoginServer", "LoginServer shutdown...");
	 ...
	 */
	private static void writeReliablePacket(GameWorld world, byte[] data, long syncnum) {
		...
		DatagramPacket dataout = new DatagramPacket(out.getBuffer(), 0, out.getBuffer().length);
		...
		for (int i = 0; i < amount; i++) {
			...
			case LoginProtocol.MSG_SYNC_LAST_RECEIVED:
				synchronized (queueLock) {
					...
					if (syncnum < out_syncnum[world.getId()] || syncnum > (out_syncnum[world.getId()] + outgoingQueueCount[world.getId()])) {
						// we don't have this packet buffered
						if (Settings.DEBUG)
							Logger.log(LoginServerChannelManager.class, "Sync error, reliable packet loss expected");
						writeLastSentPacketNum(world);
						break;
					}

					int drop_count = (int) (syncnum - out_syncnum[world.getId()]);
					for (int a = drop_count, x = 0; a < outgoingQueueCount[world.getId()]; a++)
						outgoingQueue[world.getId()][x++] = outgoingQueue[world.getId()][a];
					...
					for (int a = 0; a < outgoingQueueCount[world.getId()]; a++)
						writeReliablePacket(world, outgoingQueue[world.getId()][a], out_syncnum[world.getId()] + (long) (a + 1));
					break;
				}
			case LoginProtocol.MSG_SYNC_LAST_SENT:
				in_syncnum[world.getId()] = stream.readLong();
				if (Settings.DEBUG)
					Logger.log(LoginServerChannelManager.class, "Received incoming resync request, reliable packet loss expected");
				break;
			...
			case LoginProtocol.MSG_DATA_PACKET_U: // unreliable packet
				...
				if (datasyncnum != 0) {
					if (Settings.DEBUG)
						Logger.log(LoginServerChannelManager.class, "Received unreliable packet with wrong syncnum, does gameserver use different encryption?");
					break;
				}
				try {
					Login.getDecoderLock().lock();
			...
			default:
				if (Settings.DEBUG)
					Logger.log(LoginServerChannelManager.class, "Unknown packet received from gameserver, does gameserver use different encryption?");
				break;
			}
		}
	}
