...
import com.rs.game.player.content.CharacterSettings;
...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream, final int packetId)
		...
		if (Utils.getInterfaceDefinitionsSize() <= interfaceId) {
			Logger.log(player, "Player sent handleButtons with too high interfaceID.");
		}
		final int componentId = interfaceHash - (interfaceId << 16);
		if (Settings.DEBUG)
			System.out.println("Action Button: InterfaceID = " + interfaceId + ", ComponentID = " + componentId
					+ ", SlotID = " + slotId + ", ItemID = " + itemId + ", PacketID = " + packetId);
		...
		if (interfaceId == 1371) { // skill dialogue rs3
			...
			if (componentId == 202)
				player.getDialogueManager().startDialogue("OpenURLPrompt", "Error404");
			...
			if (componentId == 9) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					player.getInterfaceManager().setInterface(true, 1422, 113, 698);
			...
			if (slotId == 158) {
				player.getPackets().sendGameMessage("Use the home teleport button.");
			}
			if (componentId == 1)
			...
			if (componentId == 1)
				...
				player.getCombatDefinitions().setStrengthMenu(slotId == 8 ? 1 : 0);
			...
			if (componentId == 1)
				...
				player.getCombatDefinitions().setDefenceMenu(slotId == 8 ? 1 : 0);
			...
			if (componentId == 7)
				...
				player.getPackets().sendHideIComponent(1443, componentId + 1, false);
			...
			if (componentId == 8) {
				...
				if (player.getFamiliar() == null)
					player.getPackets().sendGameMessage("You don't have a familiar.");
				else
				...
				player.getActionbar().pushShortcut((componentId - 57) / 13, packetId);
			...
			if (componentId == 15) // hide familiar options
				...
				player.switchLegacyInterfaceSkin();
			...
			if (componentId >= 25 && componentId <= 194)
				player.getActionbar().pushShortcut((componentId - 16) / 13, packetId);
			...
			if (componentId == 5) {
				...
				player.getPackets().sendGameMessage("The RuneMetrics aren't available yet.");
			...
			if (componentId == 16)
				player.switchMoveTargetInterface();
		} else if (interfaceId == 1442) {
			if (componentId == 9)
			...
			else if (componentId == 84)
				player.getSkills().switchXPPopup();
			else if (componentId == 90)
				player.switchMakeXProgressWindow();
			else if (componentId == 114)
				player.setOptionType(0);
			...
			if (componentId == 11 || componentId == 14 || componentId == 18 || componentId == 22) {
				if (player.isInLegacyCombatMode() || player.isInLegacyInterfaceMode()) {
					player.getPackets().sendGameMessage("That option is disabled in legacy mode.");
					return;
				}
				player.setOptionType(2);
				...
				player.getPackets().sendIComponentSettings(1477, 926, 0, 11, 2);
				player.getTemporaryAttributtes().put(Key.ACTIONBAR_ID, (componentId - 10) / 4);
			}
			...
			if (componentId == 3)
				...
				if (slotId == 3)
					...
					player.getInterfaceManager().switchMenu(6);
			} else if (componentId == 926) {
				System.out.println(player.getOptionType());
				switch (player.getOptionType()) {
				case 0:
					if (slotId == 0)
						player.setUTCClock(0);
					else if (slotId == 1)
						player.setUTCClock(1);
					else if (slotId == 2)
						player.setUTCClock(2);
					break;
				case 1:
					player.setBackPackColumn(65792 * slotId);
					break;
				case 2:
					Object barSlot = player.getTemporaryAttributtes().remove(Key.ACTIONBAR_ID);
					if (barSlot != null) {
			...
			if (componentId == 4)
				player.getPackets().sendGameMessage(ItemExamines.getExamine(new Item(itemId)));
			...
			if (componentId == 52) // Decline
				player.closeInterfaces();
			else if (componentId == 47) // Accept
				player.getTrade().accept(false);

		} else if (interfaceId == 335) {
			if (componentId == 61)// Accept button
				player.getTrade().accept(true);
			else if (componentId == 67)// Decline button
				player.closeInterfaces();
			else if (componentId == 81) { // Money offer
				player.getPackets().sendInputIntegerScript("Enter amount:");
				player.getTemporaryAttributtes().put(Key.TRADE_COIN_WITHDRAWL, true);
			} else if (componentId == 74) { // put all items
				player.getTrade().addItem(player.getInventory().getItems());
			} else if (componentId == Trade.PLAYER_OFFER_COMPONENT_ID) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
			...
			} else if (componentId == Trade.OTHER_OFFER_COMPONENT_ID) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
			...
			if (componentId == 26) {
				...
				for (int i = 0; i < 28; i++)
					player.getFamiliar().getBob().addItem(i, Integer.MAX_VALUE);
			} else if (componentId == 29)
				player.getFamiliar().takeBob();
			...
			if (interfaceId == 1458) {
				if (componentId == 10 || componentId == 14)
					player.getPrayer().switchSettingQuickPrayer();
				else if (componentId == 33) {
					player.getPrayer().delayUsePrayer(slotId, false);
				} else if (componentId == 34) {
					for (int i = 0; i < player.getPrayer().getLockedPrayers().length; i++)
					if (slotId == player.getPrayer().getLockedPrayers()[i] && player.getPrayer().isAncientCurses()) {
						player.getPackets().sendGameMessage("This prayer hasn't been added yet.");
						return;
					}
					player.getPrayer().delayUsePrayer(slotId, true);
				}
			} else if (interfaceId == 1457)
				if (componentId == 21 || componentId == 31)
					player.getPrayer().switchSettingQuickPrayer();
			...
			if (componentId == 33) {
			...
			} else if ((interfaceId == Inventory.INVENTORY_INTERFACE && componentId == 5)
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
					...
					InventoryOptionsHandler.handleItemOption10(player, slotId, itemId, item);
			...
			if (componentId == 24)
				...
				int tabId = 1 + ((componentId - 148) / 8);
				if (Settings.DEBUG)
					Logger.log("Bank tabs", "Get " + packetId + " on tab " + tabId);
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
				...
				if (player.isOnBoBBank()) {
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
						...
						player.getTemporaryAttributtes().put("withdrawBoB", Boolean.TRUE);
						player.getPackets().sendInputIntegerScript("Enter Amount:");
					} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON6_PACKET)
						player.getBank().withdrawBoBItem(slotId, Integer.MAX_VALUE);
					else if (packetId == WorldPacketsDecoder.ACTION_BUTTON7_PACKET)
					...
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
						...
						player.getTemporaryAttributtes().put("slotBank", slotId);
						player.getTemporaryAttributtes().put("withdrawItem", Boolean.TRUE);
						player.getPackets().sendInputIntegerScript("Enter Amount:");
					} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON6_PACKET)
				...
				if (player.isOnBoBBank()) {
					if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
						...
						player.getBank().depositBoBItem(slotId, Integer.MAX_VALUE, true);
					else if (packetId == WorldPacketsDecoder.ACTION_BUTTON7_PACKET)
				...
				player.setBoBBank(false);
			else if (componentId == 346)
				player.setBoBBank(true);
		} else if (interfaceId == 1218) {
			if (componentId == 73)
				player.closeInterfaces();
			...
			if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET)
				...
				if (bob == null)
					player.getPackets().sendGameMessage("You can't store items to this familiar.");
				else
					player.getFamiliar().getBob().takeBob();
			} else if (componentId == 43)
				player.getFamiliar().renewFamiliar();
			...
			if (componentId == 46) {
				player.getDialogueManager().sendConfirmDialogue(9, new ConfirmDialogue() {
					...
					@Override
					public void process(int option) {
						if (option == 1 && !player.isInLegacyCombatMode())
							player.switchLegacyCombatMode();
					}
			...
			if (componentId == 102)
				...
				player.setOnDifferentExamineTab(false);
				player.getInterfaceManager().setInterface(true, 1560, 7, 1831);
				player.getPackets().sendOtherPlayerOnIComponent(1831, 10, player.getExaminedPlayer());
			} else if (componentId == 36 || componentId == 45 || componentId == 54) {
				player.setOnDifferentExamineTab(true);
				player.getInterfaceManager().removeInterface(1831);
			} else if (componentId == 90) {
				player.getInterfaceManager().setInterface(true, 1560, 7, 1831);
				player.getPackets().sendOtherPlayerOnIComponent(1831, 10, player.getExaminedPlayer());
			}
			...
			if (componentId == 95)
				...
				player.getDialogueManager().startDialogue("OpenURLPrompt", "cpanel");
		} else if (interfaceId == 1406) { // report
											// bug
			if (componentId == 15)
				ReportAbuse.reportAPlayer(player, null);
			else if (componentId == 23)
				ReportAbuse.reportABug(player);
		} else if (interfaceId == 1468) { // chat
											// expand
			if (componentId == 1)
			...
			if (componentId == 62 || componentId == 96) {
				...
				player.getPackets().sendIComponentText(1036, 13, "Send help request to Staff Member");
				player.getPackets().sendIComponentText(1036, 18, "Yes");
			} else if (componentId == 114 || componentId == 254 && packetId == 7) {
			...
			if (componentId >= 5 && componentId <= 9) {
				...
				player.stopAll();
		} else if (interfaceId == 105 || interfaceId == 107 || interfaceId == 1666 || interfaceId == 651)
		...
		} else if (interfaceId == 1251) {
			if (componentId == 53)
				player.getInterfaceManager().removeInterface(1251);
		} else if (interfaceId == 1311)
			player.getCosmeticsManager().handleButtons(componentId, slotId, itemId, packetId);
		else if (interfaceId == 1450) {
			if (componentId == 1) {
				player.getActionbar().useAbility(new HealAbilityShortcut(1), packetId);
			}
			if (componentId == 0 && (slotId == 7 || slotId == 8))
				player.getCombatDefinitions().setStrengthMenu(slotId == 8 ? 1 : 0);
		} else if (interfaceId == 1453) {
			if (componentId == 7 && (slotId == 7 || slotId == 8))
				player.getCombatDefinitions().setDefenceMenu(slotId == 8 ? 1 : 0);
			...
			if (componentId == 8)
				...
				player.getAreaLooting().deleteItem(slotId, player.getAreaLooting().getItem(slotId));
			else if (componentId == 21) {
				player.getAreaLooting().lootItems();
			}
			...
			if (componentId >= 11 && componentId <= 180)
				player.getActionbar().pushShortcut1((componentId - 11) / 13, packetId);
			...
			if (componentId >= 8 && componentId <= 177)
				player.getActionbar().pushShortcut2((componentId - 8) / 13, packetId);
			else if (componentId == 0) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
			...
			if (componentId >= 7 && componentId <= 176)
				player.getActionbar().pushShortcut3((componentId - 7) / 13, packetId);
			...
			if (componentId >= 7 && componentId <= 176)
				player.getActionbar().pushShortcut4((componentId - 7) / 13, packetId);
			else if (componentId == 188) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
			...
			if (componentId == 78)
				...
				player.decreaseCapIcon();
			else if (componentId == 97)
				player.increaseCapIcon();
			else if (componentId == 118)
				player.decreaseCapIcon2();
			else if (componentId == 123)
				player.increaseCapIcon2();
			else if (componentId == 101)
				player.setCapIcon(slotId);
			else if (componentId == 127)
				player.setCapIcon2(slotId);
			else if (componentId == 162)
				player.switchRetroCapes();
			...
			if (componentId == 32)
				...
				player.getVarsManager().sendVar(4738, 7);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 62) {
				player.getVarsManager().sendVar(4738, 0);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 66) {
				player.getVarsManager().sendVar(4738, 1);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 70) {
				player.getVarsManager().sendVar(4738, 2);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 74) {
				player.getVarsManager().sendVar(4738, 3);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 78) {
				player.getVarsManager().sendVar(4738, 4);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 82) {
				player.getVarsManager().sendVar(4738, 5);
				player.getPackets().sendIComponentSettings(1702, 96, 0, 20, 2);
			} else if (componentId == 36)
				...
				player.switchTogglePlayerNotification();
			else if (componentId == 48)
	...
	public static boolean wear(Player player, int slotId, int itemId, boolean fromBank) {
		...
		if (targetSlot == Equipment.SLOT_POCKET)
			targetSlot = 15;
		if (itemId == 4084)
			targetSlot = 3;
		if (targetSlot == -1) {
		...
		if (isTwoHandedWeapon && player.getEquipment().getItem(Equipment.SLOT_SHIELD) != null
			...
			player.getPackets().sendGameMessage("You Cannot carry anything else. Do you Even Lift Bro?");
			return false;
		}
		...
		if (requiriments != null) {
			for (int skillId : requiriments.keySet()) {
				...
				if (player.getSkills().getLevelForXp(skillId) < level) {
					if (hasRequiriments)
						player.getPackets().sendGameMessage("Get Gud M8, Level dat shit up!");
		...
		if (player.getInterfaceManager().containsInterface(1463))
			player.getEquipment().refreshEquipmentInterfaceBonuses();
		player.getPackets().sendSoundEffect(item.getDefinitions().getCSOpcode(118), 0, 1);
		player.getPackets().sendExecuteScript(6989); // forces refresh item in
		// actionbar for
		// wear/remove option
		return true;
	...
	public static void openItemsKeptOnDeath(Player player) {
		player.getInterfaceManager().sendCentralInterface(1626); // 876 17 ->
																	// 1626
	...
	public static void sendItemsKeptOnDeath(Player player, boolean wilderness) {
		...
		text.append("<br>").append("<br>").append("Carried wealth:").append("<br>")
				...
				.append("<br>")
				.append("<br>").append("Risked wealth:").append("<br>").append(riskedWealth / 10 > Integer.MAX_VALUE
						? "Too high!" : Utils.getFormattedNumber((int) riskedWealth / 10))
				.append("<br>").append("<br>");
		if (wilderness) {
