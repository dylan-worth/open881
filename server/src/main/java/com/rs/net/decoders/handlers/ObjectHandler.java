...
import com.rs.game.player.actions.skills.divination.ConvertAction;
...
import com.rs.game.player.actions.skills.mining.JemMining;
import com.rs.game.player.actions.skills.mining.Mining;
import com.rs.game.player.actions.skills.mining.Mining.RockDefinitions;
import com.rs.game.player.actions.skills.mining.MiningBase;
import com.rs.game.player.actions.skills.runecrafting.SiphionActionNodes;
...
public final class ObjectHandler {
	...
	private static void handleOption1(final Player player, final WorldObject object) {
		...
		if ((object.getId() == 36895 && objectDef.containsOption("Search") || ((object.getId() == 36897 && objectDef.containsOption("Search"))))) {
			...
			switch (random) {
				...
				case 1:
					...
					break;
				case 2:
					player.getPackets().sendGameMessage("You search the haystack and find something.");
					break;
			}
		}
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (TrapAction.isTrap(player, object, id) || TrapAction.isTrap(player, object))
					...
					player.getPackets().sendCSVarInteger(3713,
							player.getInventory().containsItem(ConvertAction.getEnergyForMemory(nextMemory),
									nextMemory < 11 ? 10 : 5) ? 1 : 0);
				...
				else if (id == 86743 && x == 4191 && y == 6303) // staff zone
					// leave portal
					Magic.sendNormalTeleportSpell(player, 1, 0, Settings.START_PLAYER_LOCATION);
				else if (id == 92415) // elf city dung portal
					Magic.sendTeleportSpell(player, 13652, 13654, 2602, 2603, 1, 0, new WorldTile(3447, 3694, 0), 10,
					...
					if (unlockable && bit == 0)
						player.getDialogueManager().startDialogue("SkillPortalSecondD");
					else if (bit == 1)
						player.getDialogueManager().startDialogue("SimpleMessage", "This option hasn't been added yet.");
					else if (bit == 2)
					...
					if (bit == 0)
						player.getDialogueManager().startDialogue("PvMPortalD");
					else
						player.getDialogueManager().startDialogue("ProceedAnywayD");
				} else if (id == 92239) {
					int bit = player.getVarsManager().getBitValue(25054);
					if (bit == 0) {
						player.getDialogueManager().startDialogue("SkillPortalD");
				...
				else if (id == 10230 && x == 1911 && y == 4367) // daga kings
					// entrance
				...
				// entrana dungeon
					...
					player.getActionManager()
							.setAction(new Woodcutting(object, TreeDefinitions.STRAIT_VINE_COLLECTABLE));// start
					// of
					// jadinkos
				else if (id == 12291)
				...
				// start of death platue
					...
					player.getDialogueManager().startDialogue("WarningD", DoomsayerManager.NORMAL_WARNING, 4,
							"Are you sure you want to climb down?", "Yes, I know it may be dangerous", new Runnable() {
					...
					player.useStairs(-1, new WorldTile(player.getX() == 3239 ? player.getX() + 3 : player.getX() - 3,
							player.getY(), player.getPlane()), 1, 2);
				} else if (id == 10137) {
				...
				// start forinthry dungeon
					...
					player.getDialogueManager().startDialogue("WarningD", DoomsayerManager.NORMAL_WARNING, 33, null,
							null, new Runnable() {
								@Override
								public void run() {
									player.lock();
										...
										public void run() {
											if (count == 0) {
												player.setNextFaceWorldTile(
														new WorldTile(object.getX() - 1, object.getY(), 0));
				...
				// lighthouse dungeon
							...
							null, null, new Runnable() {
								...
								public void run() {
									...
									final WorldTile toTile = new WorldTile(
											object.getRotation() == 3 || object.getRotation() == 1 ? object.getX() - 1
													: player.getX(),
											object.getRotation() == 0 || object.getRotation() == 2 ? object.getY() + 2
													: player.getY(),
											object.getPlane());
					...
					switch (objectDef.name.toLowerCase()) {
						case "loom":
							player.getDialogueManager().startDialogue("LoomingD", CreateCape.MILESTONE_CAPE_10);
							break;
						case "spinning wheel":
							player.getDialogueManager().startDialogue("SpinningD", CreateSpin.BALL_OF_WOOL);
							break;
						case "robust glass machine":
							player.getDialogueManager().startDialogue("RobustGlassD", CreateGlass.CRYSTAL_GLASS);
							break;
						case "singing bowl":
							CreateCrystal crystal = CreateCrystal.getBar(player);
							player.getDialogueManager().startDialogue("BowlSingingD", object, crystal);
							break;
						case "range":
						case "cooking range":
						case "stove":
						...
						case "closed chest":
							if (objectDef.containsOption(0, "Open")
									&& (ObjectDefinitions.getObjectDefinitions(object.getId() + 1).name.toLowerCase()
									.equals("open chest"))) {
						...
						case "altar":
						case "chaos altar":
							if (objectDef.containsOption(0, "Recharge") || objectDef.containsOption(0, "Pray")
			...
			public void run() {
				...
				if (player.getTreasureTrailsManager().useObject(object))
					...
					if (player.getSkills().getLevelForXp(Skills.SUMMONING) < 60) {
						player.getPackets()
								.sendGameMessage("You need a summoning level of 60 to go through this portal.");
						return;
					}
					player.lock();
					...
					switch (objectDef.name.toLowerCase()) {
						...
						case "flax":
							if (objectDef.containsOption(1, "Pick") && player.getInventory().addItem(1779, 1)) {
								player.setNextAnimation(new Animation(827));
								player.lock(2);
									World.removeObjectTemporary(object, Utils.random(0, 2000));
	...
	private static void handleOptionExamine(final Player player, final WorldObject object) {
		player.getPackets().sendObjectMessage(0, 15263739, object, ObjectExamines.getExamine(object), true);
		player.getPackets().sendResetMinimapFlag();
		if (player.getRights() == 2 || player.getRights() == 4)
			player.getPackets()
					.sendGameMessage("object id : " + object.getId() + ", " + object.getX() + ", " + object.getY()
							+ ", " + object.getPlane() + ", " + object.getType() + ", " + object.getRotation() + ", "
							+ object.getDefinitions().name + ", config= " + object.getDefinitions().configId
							+ ", configByFile= " + object.getDefinitions().configFileId + "animId= "
		...
		if (Settings.DEBUG) {
			...
			player.getPackets().sendGameMessage("Examine: " + object.getDefinitions().name + " (id: " + object.getId()
	...
	public static boolean handleGate(Player player, WorldObject object, int delay) {
		if (World.isSpawnedObject(object))
			return false;
	...
	public static void handleItemOnObject(final Player player, final WorldObject object, final int interfaceId,
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				else if ((itemId == 345 || itemId == 347) && (objectDef.containsOption(0, "Chop down"))) {
					player.getPackets().sendGameMessage("This is not the mightiest tree in the forest");
				}
				...
				//Axe on trees:
					...
					} else if (AccessorySmithing.isEmptyRing(itemId) && object.getX() == 3047
							&& object.getY() == 4493) {
					...
					switch (objectDef.name.toLowerCase()) {
						...
						case "anvil":
							...
							for (int index = 0; index < Smithing.BARS[0].length; index++) {
								if (Smithing.BARS[0][index] == item.getId()) {
									player.getDialogueManager().startDialogue("SmithingD", object, index, false);
									break;
								}
							}
							break;
						...
						case "furnace":
						case "lava furnace":
							if (item.getId() == 2357) {// TODO implement accesory
								// smithing into
