...
public final class LoginPacketsEncoder extends Encoder {
	...
	public void sendLobbyDetails(Player player) {
		...
		stream.writeByte(player.isDonator() ? 0x2 : 0); // 0x1 - if members, 0x2
		// - subscription
		...
		if (player.getLastGameIp() != null) {
			...
			stream.writeInt((Integer.parseInt(ipSplit[0]) << 24) + (Integer.parseInt(ipSplit[1]) << 16) + (Integer.parseInt(ipSplit[2]) << 8) + Integer.parseInt(ipSplit[3])); // ip
			// part
		} else {
			stream.writeInt(0);
		}
