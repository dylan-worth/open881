...
import com.rs.game.activities.Instance;
...
public final class World {
	...
	private static void addWorldAnnouncementTask() {
		if (Settings.ANNOUNCEMENT_TEXTS.length > 0) {
			GameExecutorManager.slowExecutor.scheduleWithFixedDelay(new Runnable() {
				...
				public void run() {
					try {
						World.sendNews(Settings.ANNOUNCEMENT_TEXTS[Utils.random(Settings.ANNOUNCEMENT_TEXTS.length)],
								World.GAME_NEWS);
					} catch (Throwable e) {
						Logger.handle(e);
					}
				...
				public void run() {
					try {
						for (Player player : World.getPlayers()) {
							if (player == null || player.isLobby() || !player.isRunning()
									|| player.hasVotedInLast12Hours())
								continue;
						}
					} catch (Throwable e) {
						Logger.handle(e);
	...
	private static final void addRestoreSpecialAttackTask() {
		...
		GameExecutorManager.fastExecutor.schedule(new TimerTask() {
			...
			public void run() {
				try {
					for (Player player : getPlayers()) {
						if (player == null || player.isDead() || !player.isRunning() || player.getCombatDefinitions()
	...
	private static final void addRestoreHitPointsTask() {
		GameExecutorManager.fastExecutor.schedule(new TimerTask() {
			...
			public void run() {
				try {
					for (Player player : getPlayers()) {
						if (player == null || player.isDead() || !player.isRunning() || player.isUnderCombat()
								|| cycle % player.getHealRestoreRate() != 0)
							continue;
						player.restoreHitPoints();
					}
					for (NPC npc : npcs) {
						if (npc == null || npc.isDead() || npc.hasFinished() || npc.isUnderCombat()
				...
				try {
					for (Player player : getPlayers()) {
						...
						if (player.getNextRunDirection() == -1) {
							...
							int r = weight >= 270 || player.getEffectsManager().hasActiveEffect(EffectType.MORRIGAN_AXE)
									? 1 : (int) (10 - (weight / (weight < 0 ? 10 : 30)));
							if (cycle % r != 0)
								continue;
	...
	public static final NPC spawnNPC(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea,
		...
		if (id >= 5533 && id <= 5558)
			...
			n = new Gregorovic(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned, null);
		else if (id == 22444)
			n = new Shadow(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		else if (id == 22450)
			n = new Spirit(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned, null);
		else if (id == 22455)
			...
			n = GodWarsBosses.graardorMinions[(id - 6261) / 2] = new GodWarMinion(id, 0, tile, mapAreaNameHash,
			...
			n = GodWarsBosses.armadylMinions[(id - 6223) / 2] = new GodWarMinion(id, 1, tile, mapAreaNameHash,
					canBeAttackFromOutOfArea, spawned);
		else if (id == 6203)
			n = new KrilTstsaroth(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		else if (id == 6204 || id == 6206 || id == 6208)
			n = GodWarsBosses.zamorakMinions[(id - 6204) / 2] = new GodWarMinion(id, 3, tile, mapAreaNameHash,
					canBeAttackFromOutOfArea, spawned);
		else if (id == 6248 || id == 6250 || id == 6252)
			n = GodWarsBosses.commanderMinions[(id - 6248) / 2] = new GodWarMinion(id, 2, tile, mapAreaNameHash,
					canBeAttackFromOutOfArea, spawned);
		else if (id == 6247)
			...
			n = new ConditionalDeath(new int[] { 13290, 4158 }, null, false, id, tile, mapAreaNameHash,
					canBeAttackFromOutOfArea);
	...
	public static boolean isTileFree(WorldTile tile, int size) {
	...
	public static final boolean removeObjectTemporary(final WorldObject object, int time) {
		removeObject(object);
		GameExecutorManager.slowExecutor.schedule(new Runnable() {
			@Override
			public void run() {
	...
	public static void sendNews(Player from, String message, int type) {
		...
		for (Player p : World.getPlayers()) {
			if (p == null || !p.isRunning()
					|| (type == 2 && p != from && !p.getFriendsIgnores().isFriend(from.getDisplayName())))
				continue;
	...
	public static boolean isSafeZone(WorldTile tile) {
		return (tile.getY() >= 3470 && tile.getY() <= 3513 && tile.getX() >= 3143 && tile.getX() <= 3186)// Grand
				...
				|| (tile.getX() >= 3044 && tile.getY() >= 3482 && tile.getX() <= 3059 && tile.getY() <= 3498
						&& tile.getPlane() == 1) // Top
													// level
													// of
													// Monastery
				...
				// Bank
				|| (tile.getRegionId() == 10658) || (tile.getRegionId() == 11927)
				|| (tile.getRegionId() == 13625 || tile.getRegionId() == 13626)
				|| (tile.getX() >= 2952 && tile.getY() >= 3220 && tile.getX() <= 2959 && tile.getY() <= 3227) // ridmigton
		// poh
		;
	}
	...
	public static final boolean isMultiArea(WorldTile tile) {
		...
		return (destX >= 3462 && destX <= 3511 && destY >= 9481 && destY <= 9521 && tile.getPlane() == 0) // kalphite
				...
				|| (destX >= 2863 && destX <= 2878 && destY >= 5350 && destY <= 5372) || TormentedDemon.atTD(tile) // Tormented
																													// demon's
																													// area
	...
	public static void removeTemporaryObject(WorldObject object, long time, boolean b) {
