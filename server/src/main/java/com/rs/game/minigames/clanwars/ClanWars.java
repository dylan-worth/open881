...
public final class ClanWars implements Serializable {
	...
	public static void sendConfig(Player player, Player other, int configId, int value) {
		...
		if (resetAccept) {
			player.getVarsManager().sendVarBitOld(5293, 0);
			other.getVarsManager().sendVarBitOld(5293, 0);
		}
		player.getVarsManager().sendVarBitOld(configId, value);
		other.getVarsManager().sendVarBitOld(configId, value);
	}

	public static boolean hasMemberItems(Player player) {
		List<Item> collective = new LinkedList<Item>();
		Collections.addAll(collective, player.getEquipment().getItems().toArray());
		Collections.addAll(collective, player.getInventory().getItems().toArray());
	...
	private void sendVictoryConfiguration(Player p) {
		switch (victoryType) {
		case -1:
			p.getVarsManager().sendVarBitOld(5280, 0);
			break;
		case 25:
			p.getVarsManager().sendVarBitOld(5280, 1);
			break;
		case 50:
			p.getVarsManager().sendVarBitOld(5280, 2);
			break;
		case 100:
			p.getVarsManager().sendVarBitOld(5280, 3);
			break;
		case 200:
			p.getVarsManager().sendVarBitOld(5280, 4);
			break;
		case 400:
			p.getVarsManager().sendVarBitOld(5280, 5);
			break;
		case 750:
			p.getVarsManager().sendVarBitOld(5280, 6);
			break;
		case 1_000:
			p.getVarsManager().sendVarBitOld(5280, 7);
			break;
		case 2_500:
			p.getVarsManager().sendVarBitOld(5280, 8);
			break;
		case 5_000:
			p.getVarsManager().sendVarBitOld(5280, 9);
			break;
		case 10_000:
			p.getVarsManager().sendVarBitOld(5280, 10);
			break;
		case -2:
			p.getVarsManager().sendVarBitOld(5280, 15);
			break;
		}
	}
	...
	private void sendTimeConfiguration(Player p) {
		switch (timeLeft) {
		case 500:
			p.getVarsManager().sendVarBitOld(5281, 1);
			break;
		case 1_000:
			p.getVarsManager().sendVarBitOld(5281, 2);
			break;
		case 3_000:
			p.getVarsManager().sendVarBitOld(5281, 3);
			break;
		case 6_000:
			p.getVarsManager().sendVarBitOld(5281, 4);
			break;
		case 9_000:
			p.getVarsManager().sendVarBitOld(5281, 5);
			break;
		case 12_000:
			p.getVarsManager().sendVarBitOld(5281, 6);
			break;
		case 15_000:
			p.getVarsManager().sendVarBitOld(5281, 7);
			break;
		case 18_000:
			p.getVarsManager().sendVarBitOld(5281, 8);
			break;
		case 24_000:
			p.getVarsManager().sendVarBitOld(5281, 9);
			break;
		case 30_000:
			p.getVarsManager().sendVarBitOld(5281, 10);
			break;
		case 36_000:
			p.getVarsManager().sendVarBitOld(5281, 11);
			break;
		case 48_000:
			p.getVarsManager().sendVarBitOld(5281, 12);
			break;
		case -1:
			p.getVarsManager().sendVarBitOld(5281, 0);
			break;
		}
	}
	...
	public void sendInterface(final Player p, final Player other) {
		...
		p.getVarsManager().sendVarBitOld(5291, 0);
		p.getVarsManager().sendVarBitOld(5292, 0);
		p.getVarsManager().sendVarBitOld(5293, 0);
			...
			GameExecutorManager.slowExecutor.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						int[] regionPos = MapUtils.convert(MapUtils.Structure.CHUNK, MapUtils.Structure.REGION, newCoords);
						World.executeAfterLoadRegion(regionPos[0], regionPos[1], regionPos[0] + width / 8, regionPos[1] + height / 8, 0, 10000, new Runnable() {
							...
							public void run() {
								...
								GameExecutorManager.fastExecutor.scheduleAtFixedRate(timer = new ClanWarsTimer(ClanWars.this), 600, 600);
	...
	public static void enter(Player p, String target) {
		...
		if (p2 == null || p2.getCurrentFriendsChat() == null || p2.getCurrentFriendsChat().getClanWars() == null) {
	...
	public static boolean enter(Player p, Player friend, boolean viewing) {
		...
		final ClanWars c = hasWar ? friend.getCurrentFriendsChat().getClanWars() : null;
		...
		if (hasWar) {
			...
			if (c.firstTeam == friend.getCurrentFriendsChat()) {
				if (viewing)
					p.setNextWorldTile(c.getBaseLocation().transform(c.getAreaType().getFirstDeathOffsetX(), c.getAreaType().getFirstDeathOffsetY(), 0));
				else
				...
				if (viewing)
					p.setNextWorldTile(northEast.transform(c.getAreaType().getSecondDeathOffsetX(), c.getAreaType().getSecondDeathOffsetY(), 0));
				else
			...
			p.getPackets().sendMusicEffect(c.timer.getMusicEffect());
			return true;
		}
		return false;
	}
	...
	public void leaveFC(Player p) {
		if (p.getControlerManager().getControler() instanceof WarControler && p.isCanPvp()) {
			p.lock(3);
			if (firstPlayers.contains(p))
				p.setNextWorldTile(getBaseLocation().transform(getAreaType().getFirstDeathOffsetX(), getAreaType().getFirstDeathOffsetY(), 0));
			else {
				WorldTile northEast = getBaseLocation().transform(getAreaType().getNorthEastTile().getX() - getAreaType().getSouthWestTile().getX(), getAreaType().getNorthEastTile().getY() - getAreaType().getSouthWestTile().getY(), 0);
				p.setNextWorldTile(northEast.transform(getAreaType().getSecondDeathOffsetX(), getAreaType().getSecondDeathOffsetY(), 0));
			}
			p.setCanPvp(false);
			p.reset();
			updateWar();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (Player player : firstPlayers) {
					...
					player.getPackets().sendMusicEffect(firstType < 4 ? 293 : firstType < 8 ? 292 : 294);
				}
				for (Player player : secondPlayers) {
					leave(player, NORMAL);
					player.getInterfaceManager().sendCentralInterface(790);
					...
					player.getPackets().sendMusicEffect(secondType < 4 ? 293 : secondType < 8 ? 292 : 294);
				}
	...
	public void useViewingOrb(Player player) {
		if (ended) {
			player.getPackets().sendGameMessage("You cannot use the orb while the war is ending!");
			return;
		}
		if (views == null) {
			views = new WorldTile[5];
			int width = (areaType.getNorthEastTile().getX() - areaType.getSouthWestTile().getX());
			int height = (areaType.getNorthEastTile().getY() - areaType.getSouthWestTile().getY());
