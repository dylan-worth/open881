...
public final class ClanWarsTimer extends TimerTask {// this is the class for
	...
	public void run() {
		try {
			if (!started) {
				if (startTicks-- == 8) {
					...
					for (Player player : clanWars.getFirstPlayers())
						player.getPackets().sendMusicEffect(getMusicEffect());
			...
			if (clanWars.getPlayersInside(false) == 0 || clanWars.getPlayersInside(true) == 0) {
	...
	public void refresh(Player p, boolean firstTeam) {
		...
		p.getPackets().sendCSVarInteger(263, clanWars.getKills() >> (firstTeam ? 0 : 24) & 0xFFFF);
		p.getPackets().sendCSVarInteger(264, clanWars.getKills() >> (firstTeam ? 24 : 0) & 0xFFFF);
