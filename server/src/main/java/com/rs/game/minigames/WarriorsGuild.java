...
public class WarriorsGuild extends Controller {
	...
	public static void init() {
		GameExecutorManager.fastExecutor.schedule(WarriorsGuild.timer = new WarriorTimer(), 600, 600);
	}
	...
	*/
	...
	public static boolean canEnter(Player player) {
		if (player.getSkills().getLevelForXp(Skills.STRENGTH) + player.getSkills().getLevelForXp(Skills.ATTACK) < 130)
			return false;
		if (player.getPrayer().isAncientCurses() && player.getPrayer().hasPrayersOn()) {
			player.getPrayer().closeAllPrayers();
			player.getPackets().sendGameMessage("You cannot use curses inside the warrior's guild.");
		}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		if (interfaceId == 411) {
			...
			if (componentId == 22) {
				...
				player.getVarsManager().sendVarBitOld(8668, 5);
			} else if (componentId == 23) {
				cyclopseOption = STRENGTH;
				player.getVarsManager().sendVarBitOld(8668, 3);
			} else if (componentId == 24) {
				cyclopseOption = COMBAT;
				player.getVarsManager().sendVarBitOld(8668, 4);
			} else if (componentId == 26) {
				cyclopseOption = DEFENCE;
				player.getVarsManager().sendVarBitOld(8668, 2);
			} else if (componentId == 25) {
				cyclopseOption = ATTACK;
				player.getVarsManager().sendVarBitOld(8668, 1);
			} else if (componentId == 3) {
				cyclopseOption = ALL;
			} else if (componentId == 22) {
				player.getVarsManager().sendVarBitOld(8668, 0);
				cyclopseOption = -1;
			}
			if (componentId == 44) {
	...
	public void prepareShotput(final byte stage, final boolean is18LB) {
		...
		if ((player.getSkills().getLevel(Skills.STRENGTH) / 100.0) > Math.random()) {
	...
	private void balanceKeg(final WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				kegCount++;
				player.getVarsManager().sendVarBitOld(object.getDefinitions().configFileId, 1);
	...
	private void resetKegBalance() {
		...
		kegTicks = 0;
		for (int i = 0; i < 6; i++)
			player.getVarsManager().sendVarBitOld(2252 + i, 0);
	}
