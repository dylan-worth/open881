...
public class DuelArena extends Controller {
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		synchronized (this) {
			...
			player.getVarsManager().sendVarOld(286, 0);
			player.getTemporaryAttributtes().put("firstScreen", true);
			player.getInterfaceManager().sendCentralInterface(ifFriendly ? 637 : 631);
					...
					if (controler instanceof DuelArena) {
						...
						if (stage != DuelStage.DONE) {
							...
							if (!targetConfiguration.removeEquipment()) {
								targetConfiguration.closeDuelInteraction(DuelStage.NO_SPACE);
								return;
							}
							beginBattle(true);
	...
	private void openConfirmationScreen(boolean ifFriendly) {
		player.getInterfaceManager().sendCentralInterface(ifFriendly ? 639 : 626);
	...
	public void endDuel(final Player victor, final Player loser, boolean isDraw) {
		...
		if (rules != null && !rules.hasRewardGiven()) {
			...
			Logger.globalLog(victor.getUsername(), victor.getSession().getIP(), new String(" completed the duel with " + loser.getUsername() + " previous items are: " + Arrays.toString(victor.getDuelRules().getStake().getShiftedItem()) + " new items are " + Arrays.toString(loser.getDuelRules().getStake().getShiftedItem())));
		...
		victor.getPackets().sendGameMessage(isDraw ? "The battle has ended in a draw." : "congratulations! You easily defeated " + loser.getDisplayName() + ".");
	...
	private static void battleTeleport(Player player, Player target) {
		...
		final WorldTile CENTER = rules.getRule(24) ? POSSIBLE_TILE_CENTRE[1][0] : rules.getRule(6) ? POSSIBLE_TILE_CENTRE[2][Utils.random(POSSIBLE_TILE_CENTRE[2].length)] : POSSIBLE_TILE_CENTRE[0][Utils.random(POSSIBLE_TILE_CENTRE[0].length)];
		if (noMovement) {
	...
	private void end(int type) {
		if (isDueling) {
			...
			if (type == LOGOUT || type == DUEL_END_LOSE || type == TELEPORT)
				endDuel(target, player, bothDead || GameLauncher.shutdown || GameLauncher.delayedShutdownStart != 0);
			...
			synchronized (target.getControlerManager().getControler()) {
				...
				case 631:
					...
					case 53:
						...
						player.getTemporaryAttributtes().put(Key.DUEL_COIN_WITHDRAWL, this);
						return true;
					case 56: // no range
					...
					case 59: // fun wep
						if (rules.getRule(15)) {// no wep
							player.getPackets().sendGameMessage("You can't have no weapons and fun weapons enabled.");
							return false;
						}
						boolean canContinue = false;
						for (int weapon : FUN_WEAPONS) {
							if (player.getEquipment().getWeaponId() == weapon || player.getInventory().containsItem(weapon, 1)) {
								canContinue = true;
								break;
							}
						}
						if (!canContinue) {
							player.getPackets().sendGameMessage("You and your opponent both need a fun weapon in order to have fun weapons enabled.");
							return false;
						}
					...
					case 64: // no movement
						if (rules.getRule(6)) {
							player.getPackets().sendGameMessage("You can't have no movement and obstacles enabled.");
							return false;
						}
