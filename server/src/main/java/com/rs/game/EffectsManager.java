			...
			if (action == BUFF) {
				...
				if (effect.type == EffectType.CONCENTRATED_BLAST) {
					if (effect.cycle == 9 || effect.cycle == 7 || effect.cycle == 5) {
						...
						PlayerCombat.delayHit(target, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, PlayerCombat.getHit(player, target, true, Combat.MAGIC_TYPE, 1.0, effect.cycle == 8 ? 0.75 : effect.cycle == 4 ? 0.82 : 0.87, false, true, false));
					...
					if (effect.cycle == 9 || effect.cycle == 7 || effect.cycle == 5 || effect.cycle == 3) {
						...
						Hit hit = PlayerCombat.getHit(player, target, true, Combat.MAGIC_TYPE, 1.0, 1.88, false, true, false);
						if (effect.cycle == 9 && hit.getDamage() > 0)
							target.setBoundDelay(10);// Six seconds
						PlayerCombat.delayHit(target, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, hit);
					}
				} else if (effect.type == EffectType.SNIPE) {
					if (effect.cycle == 1) {
						Entity[] targets = (boolean) effect.args[2] ? PlayerCombat.getMultiAttackTargets(player, target) : new Entity[] { target };
						for (Entity t : targets) {
							Projectile projectile = World.sendProjectileNew(t == target ? player : target, t, (int) effect.args[0], 41, 39, 10, 5, 0, 95);
							Hit hit = PlayerCombat.getHit(player, t, true, Combat.RANGE_TYPE, 1.15, 1.25 + Utils.random(0.94), false, true, false);
							PlayerCombat.delayHit(t, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, hit);
					...
					if (effect.cycle > 1) {
						Entity[] targets = (boolean) effect.args[3] ? PlayerCombat.getMultiAttackTargets(player, target) : new Entity[] { target };
						...
						for (Entity t : targets) {
							...
							Hit hit = PlayerCombat.getHit(player, t, true, Combat.RANGE_TYPE, 1.0, 0.94, false, true, false);
							if (effect.cycle == 9 && hit.getDamage() > 0)
					...
					if (effect.cycle == 8 || effect.cycle == 6 || effect.cycle == 4 || effect.cycle == 2) {
						...
						for (Entity t : targets) {
							Projectile projectile = World.sendProjectileNew(t == target ? player : target, t, (int) effect.args[0], 41, 39, 10, 5, 0, 95);
							for (Entity t2 : PlayerCombat.getMultiAttackTargets(player, t, 0, 9, true)) {
								Hit hit = PlayerCombat.getHit(player, t2, true, Combat.RANGE_TYPE, 1.0, 1.50 + (double) effect.args[1], false, true, false);
								...
								PlayerCombat.delayHit(t2, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, hit);
							}
						}
					}
					...
					if (effect.cycle == 8 || effect.cycle == 6 || effect.cycle == 4 || effect.cycle == 2) {
						for (Entity t : PlayerCombat.getMultiAttackTargets(player, target, 1, 9, true)) {
							if (!((boolean) effect.args[0]) && t != target)
								continue;
							Hit hit = PlayerCombat.getHit(player, t, true, Combat.MELEE_TYPE, 1.0, 0.94, false, true, false);
							if (t == target)
					...
					if (effect.cycle == 6 || effect.cycle == 4 || effect.cycle == 1) {
						Hit hit = PlayerCombat.getHit(player, target, true, Combat.MELEE_TYPE, 1.0, 0.75 + (double) effect.args[0], false, true, false);
					...
					if (effect.cycle == 7 || effect.cycle == 5 || effect.cycle == 3 || effect.cycle == 1) {
						Hit hit = PlayerCombat.getHit(player, target, true, Combat.MELEE_TYPE, 1.0, 2.19, false, true, false);
						PlayerCombat.delayHit(target, 0, hit);
					}
					...
					if (effect.cycle == 9 || effect.cycle == 8 || effect.cycle == 5 || effect.cycle == 4) {
						...
						Hit hit = PlayerCombat.getHit(player, target, true, Combat.MELEE_TYPE, 1.0, 1.88, false, true, false);
						PlayerCombat.delayHit(target, 0, hit);
					}
					...
					if (effect.cycle == 8 || effect.cycle == 6 || effect.cycle == 4 || effect.cycle == 2) {
						...
						for (Entity t : PlayerCombat.getMultiAttackTargets(player, target, 1, 9, true)) {
							...
							if (nextAngle != angle)
								continue;
							Hit hit = PlayerCombat.getHit(player, t, true, Combat.MELEE_TYPE, 1.0, 1.10 + (double) effect.args[0], false, true, false);
