...
public abstract class Entity extends WorldTile {
	...
	public void removeHitpoints(final Hit hit) {
		if (isDead() || hit.getLook() == HitLook.ABSORB_DAMAGE || hit.getLook() == HitLook.HEALED_DAMAGE)
			return;
	...
	public Entity getCurrentFaceEntity() {
		return lastFaceEntity >= 0 ? (lastFaceEntity >= 32768 ? World.getPlayers().get(lastFaceEntity - 32768)
				: World.getNPCs().get(lastFaceEntity)) : null;
	}
	...
	public boolean addWalkStep(int nextX, int nextY, int lastX, int lastY, boolean check) {
		...
		if (dir == -1)
			return false;
		if (check && !World.checkWalkStep(getPlane(), lastX, lastY, dir, getSize()) || (this instanceof NPC
				&& !canWalkNPC(getX() + Utils.DIRECTION_DELTA_X[dir], getY() + Utils.DIRECTION_DELTA_Y[dir])))// double
			// check
			// must
			// be
			// done
	...
	public boolean restoreHitPoints() {
		 ...
		 */if (hitpoints < maxHp) {
			if (this instanceof Player && World.isSafeZone(this))
				setHitpoints(maxHp - hitpoints < 1000 ? maxHp : (hitpoints + 1000));
			else
	...
	public void updateAngle(WorldTile base, int sizeX, int sizeY) {
		...
		direction = deltaX != 0 || deltaY != 0
				? (int) (Math.atan2((double) deltaX, (double) deltaY) * 2607.5945876176133) & 0x3FFF : 0;
	}
	...
	public boolean isBoundImmune() {
		...
		return effectsManager.hasActiveEffect(EffectType.FREEDOM)
				|| effectsManager.hasActiveEffect(EffectType.BOUND_IMMUNITY) || getSize() > 2;
	}
	...
	public void sendSoulSplit(final Hit hit, final Entity user) {
		...
		if (hit.getDamage() > 0) {
			Projectile projectile = World.sendProjectileNew(this, user, 2263, 11, 11, 30, -1, 20, 0);
			if (this instanceof Player)
				((Player) Entity.this).getPrayer().drainPrayer(hit.getDamage() / 85);
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (user.isDead())
						return;
					user.heal((int) (hit.getDamage() > 2000 ? (200 + ((hit.getDamage() - 2000) * 0.05))
							: hit.getDamage() * 0.1), 0, 0, false);
				}
