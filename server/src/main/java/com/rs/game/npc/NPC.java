...
public class NPC extends Entity implements Serializable {
	...
	public boolean needMasksUpdate() {
		return super.needMasksUpdate() || refreshHeadIcon || nextSecondaryBar != null || nextTransformation != null
				|| getCustomName() != null
								...
								if (can) {
									...
									if (getMapAreaNameHash() != -1) {
										...
										if ((walkType & FLY_WALK) != 0)
											...
											Entity.findBasicRoute(this,
													new WorldTile(getX() + moveX, getY() + moveY, getPlane()), 10,
													true);
	...
	public void drop() {
		...
		if (players == null || players.size() == 1) {
			...
			for (Drop drop : dropL) {
				...
				/*
				 * if(hasHerbicide) { final Herbs herb =
				 * HerbCleaning.getHerb(drop.getItemId()); if(herb != null &&
				 * killer.getSkills().getLevel(Skills.HERBLORE) >=
				 * herb.getLevel()) { killer.getSkills().addXp(Skills.HERBLORE,
				 * herb.getExperience()*2); continue; } }
				 */
			...
			for (Drop drop : dropL) {
				...
				luckyPlayer.getPackets()
						.sendGameMessage("<col=00FF00>You received: " + item.getAmount() + " " + item.getName() + ".");
				for (Player p2 : players) {
					if (p2 == luckyPlayer)
						continue;
					p2.getPackets().sendGameMessage("<col=66FFCC>" + luckyPlayer.getDisplayName() + "</col> received: "
	...
	public Item sendDrop(Player player, Drop drop) {
		...
		try {
			...
			writer.write("[" + dateFormat.format(cal.getTime()) + ", IP: " + player.getSession().getIP() + "] got "
					+ ItemDefinitions.getItemDefinitions(drop.getItemId()).getName() + " (" + drop.getItemId()
					+ ") drop from " + getName() + ".");
			writer.newLine();
			writer.flush();
			writer.close();
			...
			if (checkPlayers) {
				...
				if (playerIndexes != null) {
					for (int playerIndex : playerIndexes) {
						...
						if (player == null || player.getCutscenesManager().hasCutscene()
								|| !player.clientHasLoadedMapRegion() || player.getPlane() != getPlane()
								|| player.isDead() || player.hasFinished() || !player.isRunning()
						...
						if (checkNPCs) {
							...
							if (familiar == null || familiar.isDead() || familiar.isFinished()
									|| !Utils.isOnRange(getX(), getY(), size, familiar.getX(), familiar.getY(),
											familiar.getSize(),
			...
			if (checkNPCs) {
				...
				if (npcsIndexes != null) {
					for (int npcIndex : npcsIndexes) {
						...
						if (npc == null || npc instanceof Familiar || npc.getPlane() != getPlane() || npc == this
								...
								|| /*
									 * ((!isAtMultiArea() ||
									 * !npc.isAtMultiArea()) &&
									 * npc.getAttackedBy() != this &&
									 * npc.getAttackedByDelay() >
	...
	public void sendKillCountData() {
		...
		if (npcName.equalsIgnoreCase("Rorarius") || npcName.equalsIgnoreCase("Gladius")
				|| npcName.equalsIgnoreCase("Capsarius") || npcName.equalsIgnoreCase("Scutarius")) {
			npcName = "Ascension creatures";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else if (npcName.equalsIgnoreCase("Mutated jadinko baby") || npcName.equalsIgnoreCase("Mutated jadinko male")
				|| npcName.equalsIgnoreCase("Mutated jadinko guard")) {
			npcName = "Mutated jadinkos";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else if (npcName.equalsIgnoreCase("Ganodermic beast") || npcName.equalsIgnoreCase("Ganodermic runt")
				|| npcName.equalsIgnoreCase("Fungal mage") || npcName.equalsIgnoreCase("Fungal rodent")
				|| npcName.equalsIgnoreCase("Infested axe") || npcName.equalsIgnoreCase("Grifolaroo")
				|| npcName.equalsIgnoreCase("Grifolapine")) {
			npcName = "Polypore creatures";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else if (npcName.equalsIgnoreCase("Force muspah") || npcName.equalsIgnoreCase("Throwing muspah")
				|| npcName.equalsIgnoreCase("Bladed muspah")) {
			npcName = "Muspah";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else if (npcName.equalsIgnoreCase("Ice nihil") || npcName.equalsIgnoreCase("Blood nihil")
				|| npcName.equalsIgnoreCase("Smoke nihil") || npcName.equalsIgnoreCase("Shadow nihil")) {
			npcName = "Nihil";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else if (npcName.equalsIgnoreCase("Dagannoth Rex") || npcName.equalsIgnoreCase("Dagannoth Prime")
				|| npcName.equalsIgnoreCase("Dagannoth Supreme")) {
			npcName = "Dagannoth Kings";
			killer.slayerCreatures.put(npcName,
					(killer.slayerCreatures.get(npcName) == null ? 1 : (killer.slayerCreatures.get(npcName) + 1)));
		} else
			killer.slayerCreatures.put(npcName, (killer.slayerCreatures.get(npcName) == null ? 1
					: ((Integer) killer.slayerCreatures.get(npcName) + 1)));
	}

	private void sendLootBeam(Player player, NPC npc, ItemDefinitions drop) {
		if (drop.getValue() >= player.getLootBeamValue() && player.getLootBeamOption() != 0) {
			player.getPackets().sendGameMessage("<col=E89002>" + player.getLootBeamMessage());
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (loop == 0) {
						player.getPackets().sendGraphics(new Graphics(player.getLootBeamGFX(drop)),
								new WorldTile(tile));
					} else if (loop == 20)
