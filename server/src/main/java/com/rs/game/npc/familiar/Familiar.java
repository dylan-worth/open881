...
public abstract class Familiar extends NPC implements Serializable {
	...
	public boolean canAttack(Entity target) {
		...
		return !target.isDead() && owner.getControlerManager().canAttack(target);
	}
	...
	public void sendFollowerDetails() {
		...
		owner.getPackets().sendIComponentText(662, 17, owner.getFamiliar().getName().toLowerCase());
		owner.getVarsManager().sendVar(1784, owner.getFamiliar().getId());
		owner.getVarsManager().sendVar(1831, owner.getFamiliar().getPouch().getRealPouchId());
		owner.getVarsManager().sendVar(1786, (int) (owner.getFamiliar().getPouch().getPouchTime() / 468.75));
		...
		if (!owner.getFamiliar().isBoB(owner)) 
			owner.getPackets().sendHideIComponent(662, 33, true);
		refreshSpecialEnergy();
		switchOrb(true);
		refreshSpecialEnergy();
		sendTimeRemaining();
		owner.getPackets().sendIComponentSettings(1430, 30, -1, 65535, 2);
		owner.getPackets().sendIComponentSettings(1506, 16, -1, 65535, 2);
		owner.getPackets().sendCSVarInteger(1436, 1);
		owner.getVarsManager().sendVarBit(6051, getSpecialAmount());
		owner.getPackets().sendCSVarString(2516, getSpecialName());
		owner.getPackets().sendCSVarString(2517, getSpecialDescription());
		unlockOrb();
	}
	...
	public void dissmissFamiliar(boolean logged) {
		...
		if (!logged && !isFinished()) {
			...
			owner.getPackets().sendIComponentSettings(1506, 16, -1, 65535, 0);
			owner.getPackets().sendIComponentSettings(1430, 30, -1, 65535, 0);
			if (bob != null)
				bob.dropBob();
		}
	}
	...
	public boolean isBoB(Player player) {
		if (player.getFamiliar().getId() == 6873 || player.getFamiliar().getId() == 6815
				|| player.getFamiliar().getId() == 6794 || player.getFamiliar().getId() == 6867)
			return true;
		return false;
	}
	...
	public boolean hasSpecialOn() {
		if (owner.getTemporaryAttributtes().remove("FamiliarSpec") != null) {
			...
			if (!withinDistance(owner, 16)) {
				owner.getPackets()
						.sendGameMessage("Your familiar is too far away to use that scroll, or it cannot see you.");
				return false;
			}
