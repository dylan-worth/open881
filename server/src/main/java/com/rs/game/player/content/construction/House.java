...
import com.rs.game.npc.construction.Guard;
...
public class House implements Serializable {
	...
	public void previewRoom(RoomReference reference, boolean remove) {
		...
		if (reference.plane == 0) {
			for (int x = 0; x < 8; x++) {
				for (int y = 0; y < 8; y++) {
					WorldObject objectR = new WorldObject(HouseConstants.EMPTY_SPACE_ID, 10, reference.rotation, boundX + x, boundY + y, reference.plane);
					if (remove)
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				WorldObject[] objects = region.getAllObjects(look & 0x3, (realChunkX & 0x7) * 8 + x, (realChunkY & 0x7) * 8 + y);
				if (objects != null) {
					for (WorldObject object : objects) {
						if (object == null)
							continue;
						...
						for (int y = 0; y < 8; y++) {
							...
							if (objects != null) {
								skip: for (WorldObject object : objects) {
									if (object == null)
										continue;
									if (object.getDefinitions().containsOption(4, "Build") || object.getDefinitions().name.equals("Habitat space")) {
										if (isDoor(object)) {
