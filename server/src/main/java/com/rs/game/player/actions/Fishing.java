...
public class Fishing extends Action {
	...
	public enum FishingSpots {
		...
		BAIT4(800, 1, 1585, 313, new Animation(622), Fish.LAVA_EEL),
	...
	private int getFishingDelay(Player player) {
		...
		int randomAmt = Utils.random(0,4);
	...
	private void addFish(Player player) {
		...
		if (player.getFamiliar() != null) {
			if (Utils.random(50) == 0 && getSpecialFamiliarBonus(player.getFamiliar().getId()) > 0) {
