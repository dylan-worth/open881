...
public final class DominionTower implements Serializable {
	...
	public void openModes() {
		...
		if (progress == 256) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You have some dominion fa" + "ctor which you must exchange before", "starting another match.");
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					...
					player.getVarsManager().sendVarOld(1241, 1);
					...
					player.getPackets().sendCameraPosition(Cutscene.getX(player, getBaseX() + 32), Cutscene.getY(player, getBaseY() + 38), 1800, 6, 6);
				} else if (count == 1) {
					...
					player.getPackets().sendCameraPosition(Cutscene.getX(player, getBaseX() + 42), Cutscene.getY(player, getBaseY() + 37), 1800, 6, 6);
				} else if (count == 4) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					...
					player.getPackets().sendCameraLook(Cutscene.getX(player, getBaseX() + 35), Cutscene.getY(player, getBaseY() + 28), 800);
					player.getPackets().sendCameraPosition(Cutscene.getX(player, getBaseX() + 42), Cutscene.getY(player, getBaseY() + 37), 2500, 6, 6);
	...
	public void selectBoss() {
		if (nextBossIndex < 0) {
			...
			if (nextBossIndex < 0)
				nextBossIndex = 1;
			player.getPackets().sendExecuteScriptReverse(5435, 0, nextBossIndex);
		} else
			player.getPackets().sendExecuteScriptReverse(5435, 1, nextBossIndex);
	}
	...
	public void openRewardsChest() {
		...
		for (int i = 0; i < rewards.getSize(); i++) {
			...
			if (item.getId() == 22358 || item.getId() == 22362 || item.getId() == 22366)
				...
				item.setAmount(item.getAmount() * Settings.getDropQuantityRate());
			rewards.set(i, item);
		}
		progress = 0;
		dominionFactor = 0;
