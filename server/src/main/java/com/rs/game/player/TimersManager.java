...
public class TimersManager implements Serializable {
	...
	public static enum RecordKey {
		...
		ANGEL_OF_DEATH(20, true, false, 18),
		QUEEN_BLACK_DRAGON(21, true, false, 24),
		TELOS(22, false, true, 34),
		THE_TWIN_FURIES(23, false, true, 32),
		FIGHT_CAVES(24, true, false, 32),
		VINDICTA_GORVEK(25, false, true, 34),
		VORAGO(26, true, true, 25),
		YAKAMARU(27, false, true, 26);

		private boolean hasHM, hasFastestKill;
		private int index, lodestoneComponent;

		private RecordKey(int index, boolean hasHM, boolean hasFastestKill, int lodestoneComponent) {
			this.index = index;
			this.hasHM = hasHM;
	...
	private static class BossRecord implements Serializable {
		...
		/*
		 * private BossRecord(int totalKills, int totalHmKills, int fastestKill) { this.totalKills = totalKills;
		 * this.totalHmKills = totalHmKills; this.fastestKill = fastestKill; }
		 */
	}
	...
	public void removeTimer(RecordKey key, boolean hm) {
		...
		if (key != null) {
			...
			record.fastestKill = record.fastestKill == 0 || record.fastestKill > currentTime ? currentTime : record.fastestKill;
			if (key.hasHM && hm)
				record.totalHmKills++;
			else
				record.totalKills++;
	...
	public void increaseKills(RecordKey key, boolean hm) {
		if (isActive() || key.hasFastestKill) // shouldnt be used with this if a
			// timer boss
			return;
		BossRecord record = getRecord(key);
		if (record == null) {
	...
	public void sendBossKills() {
		...
		player.getVarsManager().sendVarBit(22946, moleRecord != null ? moleRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22947, moleRecord != null ? moleRecord.totalHmKills : 0);
		BossRecord kbdRecord = getRecord(RecordKey.KING_BLACK_DRAGON);
		player.getVarsManager().sendVarBit(22948, kbdRecord != null ? kbdRecord.totalKills : 0);
		BossRecord barrowsRecord = getRecord(RecordKey.THE_BARROWS_BROTHERS);
		player.getVarsManager().sendVarBit(22949, barrowsRecord != null ? barrowsRecord.totalKills : 0);
		BossRecord chaosElementalRecord = getRecord(RecordKey.CHAOS_ELEMENTAL);
		player.getVarsManager().sendVarBit(22950, chaosElementalRecord != null ? chaosElementalRecord.totalKills : 0);
		BossRecord kalphiteQueenRecord = getRecord(RecordKey.KALPHITE_QUEEN);
		player.getVarsManager().sendVarBit(22951, kalphiteQueenRecord != null ? kalphiteQueenRecord.totalKills : 0);
		BossRecord fightCavesRecord = getRecord(RecordKey.FIGHT_CAVES);
		player.getVarsManager().sendVarBit(22952, fightCavesRecord != null ? fightCavesRecord.totalKills : 0);
		BossRecord corperealBeastRecord = getRecord(RecordKey.CORPOREAL_BEAST);
		player.getVarsManager().sendVarBit(22953, corperealBeastRecord != null ? corperealBeastRecord.totalKills : 0);
		BossRecord draganothKingsRecord = getRecord(RecordKey.DAGANNOTH_KINGS);
		player.getVarsManager().sendVarBit(22954, draganothKingsRecord != null ? draganothKingsRecord.totalKills : 0);
		BossRecord qbdRecord = getRecord(RecordKey.QUEEN_BLACK_DRAGON);
		player.getVarsManager().sendVarBit(22955, qbdRecord != null ? qbdRecord.totalKills : 0);
		BossRecord czRecord = getRecord(RecordKey.COMMANDER_ZILYANA);
		player.getVarsManager().sendVarBit(22956, czRecord != null ? czRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22957, czRecord != null ? czRecord.totalHmKills : 0);
		BossRecord generalGraadorRecord = getRecord(RecordKey.GENERAL_GRAARDOR);
		player.getVarsManager().sendVarBit(22958, generalGraadorRecord != null ? generalGraadorRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22959, generalGraadorRecord != null ? generalGraadorRecord.totalHmKills : 0);
		BossRecord kreeAraRecord = getRecord(RecordKey.KREE_ARRA);
		player.getVarsManager().sendVarBit(22960, kreeAraRecord != null ? kreeAraRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22961, kreeAraRecord != null ? kreeAraRecord.totalHmKills : 0);
		BossRecord krilTsutsarothRecord = getRecord(RecordKey.KRIL_TSUTSAROTH);
		player.getVarsManager().sendVarBit(22962, krilTsutsarothRecord != null ? krilTsutsarothRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22963, krilTsutsarothRecord != null ? krilTsutsarothRecord.totalHmKills : 0);
		BossRecord kilnRecord = getRecord(RecordKey.FIGHT_KILN);
		player.getVarsManager().sendVarBit(22964, kilnRecord != null ? kilnRecord.totalKills : 0);
		BossRecord kalphiteKingRecord = getRecord(RecordKey.KALPHITE_KING);
		player.getVarsManager().sendVarBit(22965, kalphiteKingRecord != null ? kalphiteKingRecord.totalKills : 0);
		BossRecord legionesRecord = getRecord(RecordKey.LEGIONES);
		player.getVarsManager().sendVarBit(22966, legionesRecord != null ? legionesRecord.totalKills : 0);
		BossRecord nexRecord = getRecord(RecordKey.NEX);
		player.getVarsManager().sendVarBit(22967, nexRecord != null ? nexRecord.totalKills : 0);
		BossRecord riseOfTheSixRecord = getRecord(RecordKey.THE_BARROWS_RISE_OF_THE_SIX);
		player.getVarsManager().sendVarBit(22968, riseOfTheSixRecord != null ? riseOfTheSixRecord.totalKills : 0);
		...
		player.getVarsManager().sendVarBit(22970, voragoRecord != null ? voragoRecord.totalKills : 0);
		player.getVarsManager().sendVarBit(22971, voragoRecord != null ? voragoRecord.totalHmKills : 0);

	}

	// TODO tracking at slayer itself
	public void sendSlayerCreatureKills() {
		...
		player.getVarsManager().sendVarBit(22940, 0);// Glaciors
	...
	public void teleportToLodestone() {
		if (player.getVarsManager().getValue(BEAST_MENU_VAR) != 0)
			return;
		RecordKey key = (RecordKey) player.getTemporaryAttributtes().remove(Key.BOSS_INFO);
		if (key == null)
			return;
	...
	public void openBossInfo(int index) {
		if (index >= RecordKey.values().length || index < 0 || player.getVarsManager().getValue(BEAST_MENU_VAR) != 0)
			return;
		...
		player.getPackets().sendCSVarInteger(4486, record == null ? 0 : record.totalKills);
		player.getPackets().sendCSVarInteger(4487, !key.hasHM ? -1 : record == null ? 0 : record.totalKills);
		player.getPackets().sendCSVarInteger(4488, !key.hasFastestKill ? -1 : record == null ? 0 : record.fastestKill * 30);

		GeneralRequirementMap data = GeneralRequirementMap.getMap(ClientScriptMap.getMap(9031).getIntValue(index));
		player.getPackets().sendCSVarInteger(4484, NPCDefinitions.getNPCDefinitions(data.getIntValue(1347)).renderEmote);
