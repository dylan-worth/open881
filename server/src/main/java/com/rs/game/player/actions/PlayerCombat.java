...
public class PlayerCombat extends Action {
	...
	public boolean process(Player player) {
		...
		if (target.isDead() || target.hasFinished()) {
			...
			if (mainHandDelay > 0 || offHandDelay > 0) {
				 ...
				 * (player.getCombatDefinitions().getType(Equipment.SLOT_SHIELD) != Combat.MELEE_TYPE &&
				 * player.getEquipment().hasOffHand() && !player.getEquipment().hasShield())) {
	...
	public static boolean isWithinDistance(Player player, Entity target) {
		// player is walking to atm
		if (!player.clipedProjectile(target, hasMeleeHand(player) && !checkAttackPathAsRange(target)) || !Utils.isOnRange(player, target,
				getAttackRange(player)
						// correct extra distance for walk. no glitches
						// this way ^^. even if frozen
						+ (player.hasWalkSteps() && target.hasWalkSteps() ? (player.getRun() && target.getRun() ? 2 : 1) : 0))
				|| Utils.colides(player, target)) // doesnt let u attack when u
			// under / while walking out,
			// remove this check if u want
			return false;
		return true;
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if (player.hasFinished() || target.isDead() || target.hasFinished() || (target instanceof NPC && ((NPC) target).isCantInteract()))
								return;
							effect.execute(player, target, hitTarget, target == PlayerCombat.this.target, spell);
						}
	...
	private int attackRange(Player player, boolean mainHand) {
		...
		case Combat.BOLT_STYLE:
			...
			if (defs.id == 15241 && Utils.random(player.getSkills().getLevel(Skills.FIREMAKING) << 1) == 0) {// handcannon
				// exeption
	...
	private int attackMelee(Player player, boolean mainHand) {
		...
		/*
		 * int animClientCycles = attackAnim.getDefinitions().getEmoteClientCycles(); int worldCycleDelay =
		 * animClientCycles / 60; int clientCycleDelay = ((worldCycleDelay + 1) * 60 - animClientCycles) - 10; if
		 * (clientCycleDelay < 0) //some weaps are faster than cycle lol, those few dont need delay(such as new ags
		 * emote) clientCycleDelay = 0;
		 * 
	...
	public int handleAbilityAttack(final Player player) {
		...
		case ActionBar.MAGIC_ABILITY_SHORTCUT:
			...
			case 5:// COMBUST
				...
				final int minumumDamage = getMaxHit(player, target, true, Combat.MAGIC_TYPE, 1.0, true), maximumDamage = getMaxHit(player, target, true, Combat.MAGIC_TYPE, 1.0 + Utils.random(.88), true), damage = Utils.random(minumumDamage, maximumDamage) / 5;
				target.setNextGraphics(new Graphics(targetGFX, 0, 130));
				WorldTasksManager.schedule(new WorldTask() {
		...
		case ActionBar.MELEE_ABILITY_SHORTCUT:
			...
			case 5:// SMASH
				...
				target.getEffectsManager().startEffect(new Effect(EffectType.PROTECTION_DISABLED, 8));// 5
				// Seconds
				break;
			case 3:// SEVER
			...
			case 7:// SLAUGHTER
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						if (player.isStunned())// One of the logistics to avoid
							// it is to insta stun.
							return;
			...
			case 14:// BALANCED_STRIKE
				double playerPercent = player.getHitpoints() / (double) player.getMaxHitpoints(), targetPercent = target.getHitpoints() / (double) target.getMaxHitpoints();
				int damage = 0;
	...
	public int handleSpecialAttack(final Player player) {
		...
		if (spec != null) {
			...
			if (spec.effect != null) {
				...
				for (final Entity target : targets) {
					int time = spec.effect.executeAttack(player, target, target == PlayerCombat.this.target, spec);
					if (targetGfx != null)
	...
	private static enum SpecialAttack {
		...
		, PENANCE_TRIDENT(new int[] { 1409 }, 23996, new SpecialAttackEffect() {
			...
			@Override
			public int executeAttack(final Player player, final Entity target, boolean mainTarget, SpecialAttack attack) {
				player.getEffectsManager().startEffect(new Effect(EffectType.INCREASE_CRIT_CHANCE, 3, 10));// 10%
				// crit
				// chance
	...
	public static Hit getHit(Player player, Entity target, boolean mainHand, int attackType, double accuracyMultiplier, double damageMultiplier, boolean spec, boolean ability, boolean ignoreDefence) {
		...
		if (Utils.random(100) <= (effects.hasActiveEffect(EffectType.INCREASE_CRIT_CHANCE) ? (int) effects.getEffectForType(EffectType.INCREASE_CRIT_CHANCE).getArguments()[0] : 5)) { // 5%
			// chance
			// crit
			hit.setCriticalMark();
	...
	public static int getMaxHit(Player player, Entity target, boolean mainHand, int attackType, double damageMultiplier, boolean ability) {
		...
		// System.out.println("maxhit: " + maxHit + ", with crit: " + (maxHit *
		// 1.25));
	...
	// abilities.
	/*
	 * TODO Black mask, Focus sight, Hexcrest, Full slayer helmet and Slayer helmet boost your ability damage by
	 * 12.5% when on a Slayer task. Salve amulet and Salve amulet (e) boosts your ability damage by respectively
	 * 15% and 20% against undead creatures. Ferocious ring boosts ability damage by 4% when on a task in Kuradal's
	 * Dungeon. Balmung boosts ability damage by 45% against Dagannoths. Bane ammunition increases ability damage
	 * by 18.75% against appropriate monsters. TokKul-Zo increases ability damage by 10% against TzHaar creatures.
	 * Keris increases damage by 33% against Kalphites, and occasionally increases it by 200%. Brine sabre and
	 * Brackish blade increase damage by 40% against 'rum'-pumped crabs. Silverlight and Darklight double ability
	 * damage against demons. Using the portal to the King Black Dragon in the Lava Maze increases ability damage
	...
	private static int getMaxHit(Player player, boolean mainHand, boolean ability) {
		...
		return (int) ((ability ? player.getCombatDefinitions().getAbilitiesDamage() : player.getCombatDefinitions().getHandDamage(!mainHand)) * dmgMultiplier * (mainHand ? 1 : 0.5) * 0.1); // x
		// 0.1
		// to
		// reduce
		// dmg
			...
			public void run() {
				for (Hit hit : hits) {
					...
					if (hit.getLook() != HitLook.MAGIC_DAMAGE) {
						...
						/*
						 * Effect pois = player.getEffectsManager().getEffectForType( EffectType.WEAPON_POISON); if
						 * (pois != null && Utils.random(8) == 0) { Effect anti =
						 * target.getEffectsManager().getEffectForType( EffectType.ANTIPOISON); if (anti != null) {
						 * anti.setCycle(anti.getCycle() - 16); target.getEffectsManager().refreshBuffEffect(anti);
						 * } else { EffectsManager.makePoisoned(target, getMaxHit(player, )); } }
						 */
					...
					if (player.getPrayer().usingPrayer(1, 30))
	...
	public static void setWeaponDelay(Player player, boolean mainHand) {
		...
		if (player.getCombatDefinitions().getType(Equipment.SLOT_WEAPON) != player.getCombatDefinitions().getType(Equipment.SLOT_SHIELD) && player.getEquipment().hasOffHand() && !player.getEquipment().hasShield())
			nextHitMin += 1;
		if (player.getEffectsManager().hasActiveEffect(EffectType.ADRENALINE_GAIN_DECREASE)) {// Reduces
			// speed
			// by
			// two
			delay *= 2;
	...
	public static int getAttackAnimation(Player player, boolean mainHand) {
		...
		return item.getDefinitions()
				.getCombatOpcode(mainHand ? (legacy ? 4385 : 2914) : (legacy ? 4389 : 2831)/* 4369 : 4373 */);
	}
	 ...
	 * not in cache sadly
	 * 
	 * private Graphics getLoadWeaponGraphic(int weaponId, int arrowId) { //TODO knives //darts if (weaponId ==
	 * 806) return new Graphics(232, 0, 96); if (weaponId == 807) return new Graphics(233, 0, 96); if (weaponId ==
	 * 808) return new Graphics(234, 0, 96); if (weaponId == 3093) return new Graphics(273, 0, 96); if (weaponId ==
	 * 809) return new Graphics(235, 0, 96); if (weaponId == 810) return new Graphics(236, 0, 96); if (weaponId ==
	 * 811) return new Graphics(237, 0, 96); if (weaponId == 11230) return new Graphics(1123, 0, 96); return null;
	 * }
	 */
