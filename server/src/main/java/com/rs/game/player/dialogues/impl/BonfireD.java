...
import com.rs.game.player.dialogues.impl.skills.ChooseAToolD;
...
public class BonfireD extends Dialogue {
	...
	public void start() {
		...
		for (int i = 0; i < 15; i++)
			player.getPackets().sendCSVarInteger(1703 + i, i >= ids.length ? -1 : ids[i]);
		player.getPackets().sendIComponentText(1179, 0, "Which logs do you want to add to the bonfire?");
