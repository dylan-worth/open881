...
public abstract class Dialogue {
	...
	public static final int ANGRY = 9790, ASKING = 9829, BLANK = 9772, UPSET = 9776, MILDLY_ANGRY = 9784, VERY_ANGRY = 9792, MANIAC = 9800, NOT_TALKING_JUST_LISTENING = 9804, PLAIN_TALKING = 9808, UNSURE = 9836, LISTENS_THEN_LAUGHS = 9840, GOOFY_LAUGH = 9851, THINKING_THEN_TALKING = 9859, NONONO = 9844, SAD_SHOCKED = 9776;
	...
	public static boolean sendEntityDialogueNoContinue(Player player, int type, String title, int entityId, int animationId, String... texts) {
		...
		/*
		 * player.getInterfaceManager().sendDialogueInterface(1192); player.getPackets().sendIComponentText(1192,
		 * 4, title); player.getPackets().sendIComponentText(1192, 3, text);
		 * player.getPackets().sendEntityOnIComponent(type == IS_PLAYER, entityId, 1192, 11); if (animationId !=
		 * -1) player.getPackets().sendIComponentAnimation(animationId, 1192, 11);
		 */
		return true;
	}
	...
	public boolean sendEntityDialogues(short interId, String[] talkDefinitions, byte type, int entityId, int animationId) {
		...
		for (int childOptionId = 0; childOptionId < componentOptions.length; childOptionId++)
			player.getPackets().sendIComponentText(interId, componentOptions[childOptionId], talkDefinitions[childOptionId]);
		if (type == IS_ITEM)
	...
	public boolean sendEntityDialogue(int type, String title, int entityId, int animationId, String... texts) {
		...
		if (type == IS_NPC) {
			...
			player.getInterfaceManager().sendDialogueInterface(1184);
			/*
			 * for (int i = 0; i < 3; i++) player.getPackets().sendHideIComponent(1184, 14 + i, true);
			 */
			player.getPackets().sendIComponentText(1184, 10, title);
			player.getPackets().sendIComponentText(1184, 9, text);
	...
	public boolean sendEntityDialogue(short interId, String[] talkDefinitions, byte type, int entityId, int animationId) {
