...
public class GodWars extends Controller {
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 57225) {
			...
			int requiredKc = player.isExtremeDonator() ? 9 : player.isDonator() ? 10 : 15;
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (loop == 0) {
						...
						player.getControlerManager().forceStop();
						player.getControlerManager().startControler("DeathEvent", CHAMBER_TELEPORTS[(sector * 2) + 1], player.hasSkull());
					} else if (loop == 4) {
