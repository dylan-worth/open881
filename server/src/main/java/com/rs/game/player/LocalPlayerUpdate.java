...
public final class LocalPlayerUpdate {
	...
	private boolean needsRemove(Player p) {
		...
		return p != player && (!p.getCutscenesManager().showYourselfToOthers() || p.hasFinished()
				|| !(player.withinDistance(p, player.hasLargeSceneView() ? 182 : 14)
						&& /*
	...
	private boolean needsAdd(Player p) {
		return p != null && p.getCutscenesManager().showYourselfToOthers() && p.isRunning()
				&& (player.withinDistance(p, player.hasLargeSceneView() ? 182 : 14)
						&& /*
							 * ( player . hasLargeSceneView ( ) ||
							 */player.getMapRegionsIds().contains(p.getRegionId()))
				&& localAddedPlayers < MAX_PLAYER_ADD && p.clientHasLoadedMapRegion();
	}
	...
	private void updateRegionHash(OutputStream stream, int lastRegionHash, int currentRegionHash,
		...
		if (lastRegionX == currentRegionX && lastRegionY == currentRegionY) {
			...
			stream.writeBits(20,
					(yOffset & 0xff) + ((xOffset & 0xff) << 8) + (planeOffset << 16) + (currentMovementType << 18));
		}
	}
		...
		for (int i = 0; i < localPlayersIndexesCount; i++) {
			...
			if (needsRemove(p)) {
				...
				boolean needAppearenceUpdate = needAppearenceUpdate(p.getIndex(),
						p.getAppearence().getMD5AppeareanceDataHash());
				boolean needIconsUpdate = needIconsUpdate(p.getIndex(), p.getAppearence().getMD5IconsDataHash());
				boolean needUpdate = p.needMasksUpdate() || needAppearenceUpdate || needIconsUpdate
						|| player.getCombatDefinitions().isNeedTargetReticuleUpdate(p);
				if (needUpdate)
				...
				if (p.hasTeleported() || p.getNextWalkDirection() != -1) {
					...
					if (Math.abs(p.getX() - p.getLastWorldTile().getX()) < 16 // 14
						...
						if (yOffset < 0)
							yOffset += 32;
						stream.writeBits(15,
								yOffset + (xOffset << 5) + ((planeOffset & 0x3) << 10) | movementType << 12);// 4
																												// forces
																												// setmovementtype
																												// to
																												// teleport
					...
					for (int i2 = i + 1; i2 < localPlayersIndexesCount; i2++) {
						...
						if (needsRemove(p2) || p2.hasTeleported() || p2.getNextWalkDirection() != -1
								|| (p2.needMasksUpdate()
										|| needAppearenceUpdate(p2.getIndex(),
												p2.getAppearence().getMD5AppeareanceDataHash())
										|| needIconsUpdate(p2.getIndex(), p2.getAppearence().getMD5IconsDataHash())))
							break;
						skip++;
					}
					skipPlayers(stream, skip);
	...
	private void appendUpdateBlock(Player p, OutputStream data, boolean needAppearenceUpdate, boolean needIconsUpdate,
			boolean added) {
		int mask = 0;
		...
		if ((added || p.getNextFaceWorldTile() != null)
				&& (p.getNextRunDirection() == -1 && p.getNextWalkDirection() == -1 && p.getNextForceMovement() == null
						&& p.getNextFaceEntity() < 0 && !(added && p.getLastFaceEntity() != -1))) {
			mask |= 0x10;
		}
	...
	private void applyColourMask(Player p, OutputStream data) {
		...
		data.writeByte128(colours & 0xFF);
		data.write128Byte(colours >> 8 & 0xFF);
		data.writeByte128(colours >> 16 & 0xFF);
		data.write128Byte(colours >> 24 & 0xFF);
		data.writeShort128(color.getDelay());
		data.writeShortLE(color.getDuration());
	}
	...
	private void applyHitsMask(Player p, OutputStream data) {
		...
		for (Hit hit : p.getNextHits()) {
			...
			if (hit.missed() && !interactingWith) {
				...
				data.write128Byte(hit.getDamageDisplay(player));
			} else {
	...
	private void applyClanMemberMask(Player p, OutputStream data) {
		data.writeByteC((player.getClanManager() != null && player.getClanManager().isMemberOnline(p)) ? 1 : 0);
	}
	...
	private void applyGraphicsMask3(Player p, OutputStream data) {
		...
		data.writeIntLE(p.getNextGraphics3().getSettingsHash());
	...
	private void applyAnimationMask(Player p, OutputStream data) {
		...
		data.writeByte128(p.getNextAnimation().getDelay());
	}
	...
	private void applyAppearanceMask(Player p, OutputStream data) {
		...
		System.out.println(renderData.length);
	...
	private void applyForceMovementMask(Player p, OutputStream data) {
		data.writeByte(p.getNextForceMovement().getToFirstTile().getX() - p.getX());
		data.write128Byte(p.getNextForceMovement().getToFirstTile().getY() - p.getY());
		data.write128Byte(p.getNextForceMovement().getToSecondTile() == null ? 0
		...
		data.writeByte(p.getNextForceMovement().getToFirstTile().getPlane());																	// 876
		data.writeByte(p.getNextForceMovement().getToSecondTile().getPlane());
