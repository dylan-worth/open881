	...
	private void handleUpgrade(int prevAmount, int newAmount) {
		if (!getDonatorTitle(prevAmount).equalsIgnoreCase(
			...
			player.getBank().addItem(new Item(ItemIdentifiers.KEEPSAKE_KEY, 1),
					true);
			player.getPackets().sendGameMessage("A dragon keepsake key has been added to your bank!");
			World.sendNews(player.getDisplayName() + " has just upgraded " + (player.getAppearence().isMale() ? "his" : "her") + " donator status from " + getDonatorTitle(prevAmount).toLowerCase() + " to " + getDonatorTitle(newAmount).toLowerCase() + "!", 0);
		}
	}
