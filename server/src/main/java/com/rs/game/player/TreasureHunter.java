...
import com.rs.game.player.content.treasurehunter.Prize;
import com.rs.game.player.content.treasurehunter.Rewards;
...
public class TreasureHunter implements Serializable {
	...
	private int earnedKeys, dailyKeys, boughtKeys;
	...
	public void processClick(int packetId, int interfaceId, int componentId,
		...
		case 1253:
			switch (componentId) {
			case 788:
				player.getInterfaceManager().removeWindowInterface(415);
				break;
			case 34:
					...
					if (bank || !player.getInventory().hasFreeSlots()) {
						...
						try {
							...
							writer.write("[" + dateFormat2.format(cal2.getTime()) + ", IP: "
									+ player.getSession().getIP() + ", inventory] got " + item.getName() + " ("
	...
	public int allKeys() {
		return dailyKeys + boughtKeys + earnedKeys;
	}
...
//				Requirement.TREASURE_HUNTER, 1);
	...
	public void sendReward(Prize prize, int chestIndex, boolean instant,
		...
		player.getVarsManager().setVarBit(20735, 1291); //20736
	...
	public void openSpinInterface(boolean force, boolean instant, boolean first) {
		...
		player.stopAll();
		player.getInterfaceManager().setWindowInterface(415, 1253); //415 1253
		player.getPackets().sendIComponentText(1253, 25, "no current event");
		open(instant, first);
	}

	public void open(boolean instant, boolean first) {
		player.getPackets().sendCSVarInteger(1800, getAllKeys());//brought keys count
		player.getPackets().sendCSVarInteger(4082, frozenHearts);//fixed frozen heart count
		player.getPackets().sendCSVarInteger(1928, 76643); // enabling colorful
		...
		player.getPackets().sendIComponentText(1253, 445, "Earn spins by skilling, participating activities, donating, and more!");
...
//		player.getPackets().sendHideIComponent(1253, 25, true);
	...
	public void giveDailyKeys() {
		if (Utils.currentTimeMillis() - delay < (24 * 60 * 60 * 1000))
			return;
		delay = Utils.currentTimeMillis();
		...
		if (dailyKeys > previous) {
			player.getPackets().sendGameMessage("<col=FF0000>You have been awarded " + (dailyKeys - previous) + " Treasure Hunter keys for today");
		}
	}
	...
	public void setHearts(int frozenHearts) {
