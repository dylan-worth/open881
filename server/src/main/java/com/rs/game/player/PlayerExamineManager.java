...
public class PlayerExamineManager implements Serializable {
	...
	public void openExamineSettings() {
		if (player.getInterfaceManager().containsScreenInterface()
			...
			player.getPackets()
					.sendGameMessage("Please finish what you are doing before opening the examine settings interface.");
			return;
		}
		player.stopAll();
	...
	public void openExamineDetails(Player target) {
		...
		if (!privacyOn) {
			...
			player.getVarsManager().sendVarBit(30202, target.getSkills().getLevel(Skills.INVENTION));
			player.getVarsManager().sendVarBit(30203, target.getSkills().getLevelForXp(Skills.INVENTION));
			player.getVarsManager().sendVar(4934, (int) target.getSkills().getXp(Skills.ATTACK));
			player.getVarsManager().sendVar(4935, (int) target.getSkills().getXp(Skills.STRENGTH));
			player.getVarsManager().sendVar(4936, (int) target.getSkills().getXp(Skills.DEFENCE));
			player.getVarsManager().sendVar(4937, (int) target.getSkills().getXp(Skills.RANGE));
			player.getVarsManager().sendVar(4938, (int) target.getSkills().getXp(Skills.PRAYER));
			player.getVarsManager().sendVar(4939, (int) target.getSkills().getXp(Skills.MAGIC));
			player.getVarsManager().sendVar(4940, (int) target.getSkills().getXp(Skills.RUNECRAFTING));
			player.getVarsManager().sendVar(4941, (int) target.getSkills().getXp(Skills.CONSTRUCTION));
			player.getVarsManager().sendVar(4942, (int) target.getSkills().getXp(Skills.DUNGEONEERING));
			player.getVarsManager().sendVar(4943, (int) target.getSkills().getXp(Skills.HITPOINTS));
			player.getVarsManager().sendVar(4944, (int) target.getSkills().getXp(Skills.AGILITY));
			player.getVarsManager().sendVar(4945, (int) target.getSkills().getXp(Skills.HERBLORE));
			player.getVarsManager().sendVar(4946, (int) target.getSkills().getXp(Skills.THIEVING));
			player.getVarsManager().sendVar(4947, (int) target.getSkills().getXp(Skills.CRAFTING));
			player.getVarsManager().sendVar(4948, (int) target.getSkills().getXp(Skills.FLETCHING));
			player.getVarsManager().sendVar(4949, (int) target.getSkills().getXp(Skills.SLAYER));
			player.getVarsManager().sendVar(4950, (int) target.getSkills().getXp(Skills.HUNTER));
			player.getVarsManager().sendVar(4951, (int) target.getSkills().getXp(Skills.DIVINATION));
			player.getVarsManager().sendVar(4952, (int) target.getSkills().getXp(Skills.MINING));
			player.getVarsManager().sendVar(4953, (int) target.getSkills().getXp(Skills.SMITHING));
			player.getVarsManager().sendVar(4954, (int) target.getSkills().getXp(Skills.FISHING));
			player.getVarsManager().sendVar(4955, (int) target.getSkills().getXp(Skills.COOKING));
			player.getVarsManager().sendVar(4956, (int) target.getSkills().getXp(Skills.FIREMAKING));
			player.getVarsManager().sendVar(4957, (int) target.getSkills().getXp(Skills.WOODCUTTING));
			...
			player.getVarsManager().sendVar(6100, (int) target.getSkills().getXp(Skills.INVENTION));

			player.getVarsManager().sendVar(4960, target.isInLegacyCombatMode() ? 1 : 2);// Enables
			// something
		...
		// option
		player.setExaminedPlayer(target);
		System.out.println(player.isOnDifferentExamineTab());
		player.getPackets().sendOtherPlayerOnIComponent(1831, 10, target);
		if (player.isOnDifferentExamineTab())
	...
	public void closeExamineDetails() {
		for (int x = 1556; x < 1561; x++){
		player.getInterfaceManager().removeInterface(x);
