...
public class DungeonManager {
	...
	public boolean hasKey(KeyDoors key) {
		if(Settings.DEBUG) Logger.log(this, "Checking for key "+key.toString());
	...
	public void loadRoom(final RoomReference reference) {
		if(Settings.DEBUG) Logger.log(this, "Loading room...");
	...
	public void openRoom(final Room room, final RoomReference reference, final VisibleRoom visibleRoom) {
		if(Settings.DEBUG) Logger.log(this, "openning room...");
		...
		if(Settings.DEBUG) Logger.log(this, "Loading player to map region...");
		for (Player player : party.getTeam()) {
		...
		if(Settings.DEBUG) Logger.log(this, "Finished loading player to map region...");
		World.executeAfterLoadRegion(regionId, new Runnable() {
			...
			public void run() {
				if (isDestroyed())
					return;
				if(Settings.DEBUG) Logger.log("DungeonManager", "Run open room events...");
				room.openRoom(DungeonManager.this, reference);
				visibleRoom.openRoom();
				if(Settings.DEBUG) Logger.log("DungeonManager", "Loading doors...");
				...
				if(Settings.DEBUG) Logger.log("DungeonManager", "Set resources...");
				if (room.getRoom().allowResources())
					setResources(room, reference, toChunkX, toChunkY);
				if(Settings.DEBUG) Logger.log("DungeonManager", "Set keys...");
				if (room.getDropId() != -1)
	...
	public void setDoor(RoomReference reference, int lockObjectId, int doorObjectId, int rotation) {
		if(Settings.DEBUG) Logger.log(this, "Set door: "+" lockID = "+lockObjectId+", DoorID = "+doorObjectId);
		if (lockObjectId != -1) {
	...
	public void sendSettings(Player player) {
		...
		Effect nextOvlEffect = previousOvlEffect == null ? null : new Effect(EffectType.OVERLOAD, previousOvlEffect.getCycle());
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			if (party.getSize() == DungeonConstants.TEST_DUNGEON) {
				player.getPackets().sendGameMessage("Moving next floor. Enjoy the xp.");
				player.getSkills().addXp(Skills.DUNGEONEERING, 500000, true);
				player.getDungManager().addTokens(50000);
			...
			player.getVarsManager().sendVarBitOld(7550, player.getDungManager().getCurrentProgres());
			player.getVarsManager().sendVarBitOld(7551, player.getDungManager().getPreviousProgress());
			double averageXP = (floorXP + prestigeXP) / 2;
			if (party.getTeam().size() == 1 && party.getSize() != DungeonConstants.LARGE_DUNGEON)
				averageXP *= 1.5;
			if (player.isDonator())
				averageXP *= player.isExtremeDonator() ? 1.15 : 1.1;
	...
	public void setDungeon() {
		if(Settings.DEBUG) Logger.log(this, "Setting Dungeon...");
		key = party.getLeader() + "_" + keyMaker.getAndIncrement();
		if(Settings.DEBUG) Logger.log(this, "Dungeon key = "+key);
		cachedDungeons.put(key, this);
		if(Settings.DEBUG) Logger.log(this, "Dungeon " +key+ " successfully added to the dungeon cache");
		for (Player player : party.getTeam())
	...
	public void load() {
		if(Settings.DEBUG) Logger.log(this, "Load dungeon...");
		party.lockParty();
		...
		GameExecutorManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					if(Settings.DEBUG) Logger.log("DungeonManager", "start Loading Dungeon...");
					clearKeyList();
					
					
					// generates dungeon structure
					if(Settings.DEBUG) Logger.log("DungeonManager", "Creating Dungeon...");
					...
					if(Settings.DEBUG) Logger.log("DungeonManager", "Find enpty map area for the dungeon...");
					boundChuncks = MapBuilder.findEmptyChunkBound(dungeon.getMapWidth() * 2, (dungeon.getMapHeight() * 2));
					// reserves all map area
					if(Settings.DEBUG) Logger.log("DungeonManager", "Reserve map area for the dungeon...");
	...
	public void openMap(Player player) {
			...
			for (int y = 0; y < visibleMap[x].length; y++) {
				if (visibleMap[x][y] != null) { // means exists
					...
					if (room.hasNorthDoor() && visibleMap[x][y + 1] == null) {
						...
						player.getPackets().sendExecuteScriptReverse(3278, protocol, getMapIconSprite(DungeonConstants.SOUTH_DOOR, highLight), y + 1, x);
					}
					if (room.hasSouthDoor() && visibleMap[x][y - 1] == null) {
						Room unknownR = getRoom(new RoomReference(x, y - 1));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScriptReverse(3278, protocol, getMapIconSprite(DungeonConstants.NORTH_DOOR, highLight), y - 1, x);
					}
					if (room.hasEastDoor() && visibleMap[x + 1][y] == null) {
						Room unknownR = getRoom(new RoomReference(x + 1, y));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScriptReverse(3278, protocol, getMapIconSprite(DungeonConstants.WEST_DOOR, highLight), y, x + 1);
					}
					if (room.hasWestDoor() && visibleMap[x - 1][y] == null) {
						Room unknownR = getRoom(new RoomReference(x - 1, y));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScriptReverse(3278, protocol, getMapIconSprite(DungeonConstants.EAST_DOOR, highLight), y, x - 1);
					}
				}
			}
		}
		...
		for (Player p2 : party.getTeam()) {
			...
			player.getPackets().sendExecuteScriptReverse(3279, p2.getDisplayName(), protocol, index++, reference.getY(), reference.getX());
		}
	}
