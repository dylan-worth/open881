...
public final class Inventory implements Serializable {
	...
	public void unlockInventory(boolean menu) {
		player.getPackets().sendIComponentSettings(1473, 5, -1, 65535, 2097152);
		player.getPackets().sendIComponentSettings(1473, 5, 0, 27, 15302030);
		player.getPackets().sendIComponentSettings(1473, 68, 0, 5, 1422);
		player.getPackets().sendIComponentSettings(1473, 93, 0, 4, 14);
	}
	...
	public boolean addItem(Item item) {
		if (item == null || item.getId() < 0 || item.getAmount() < 0 || !Utils.itemExists(item.getId()) || !player.getControlerManager().canAddInventoryItem(item.getId(), item.getAmount()))
			return false;
		Item[] itemsBefore = items.getItemsCopy();
		if (!items.add(item)) {
	...
	public void sendExamine(int slotId) {
		...
		player.getPackets().sendInventoryMessage(0, slotId, ItemExamines.getExamine(item) + (ItemConstants.isTradeable(item) ? "<br>GE guide price: " + GrandExchange.getPrice(item.getId()) + " gp" : ""));
	}
