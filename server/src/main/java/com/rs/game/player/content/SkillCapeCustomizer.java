...
public final class SkillCapeCustomizer {
	...
	public static void startCustomizing(Player player, int itemId) {
		...
		int[] skillCape = itemId == 20767 || itemId == 32151 ? player.getMaxedCapeCustomized() : player.getCompletionistCapeCustomized();
		player.getInterfaceManager().sendCentralInterface(20);
		for (int i = 0; i < 4; i++)
			player.getVarsManager().sendVarBit(1039 + i, skillCape[i]);
		player.getPackets().sendIComponentModel(20, 104, player.getAppearence().isMale() ? ItemDefinitions.getItemDefinitions(itemId).getMaleWornModelId1() : ItemDefinitions.getItemDefinitions(itemId).getFemaleWornModelId1());
	}
	...
	public static void handleSkillCapeCustomizerColor(Player player, int colorId) {
		...
		if (part == null)
			return;
		int[] skillCape = capeId == 20767 || capeId == 32151 ? player.getMaxedCapeCustomized() : player.getCompletionistCapeCustomized();
