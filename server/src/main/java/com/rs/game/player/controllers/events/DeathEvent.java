...
public class DeathEvent extends Controller {
	...
	public void lockInterfaces(boolean lock) {
		...
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.ACTIVE_TASK_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.INVENTORY_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.EQUIPMENT_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.PRAYER_BOOK_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.MAGIC_ABILITIES_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.MELEE_ABILITIES_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.RANGE_ABILITIES_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.DEFENCE_ABILITIES_TAB, lock);
		player.getInterfaceManager().sendLockGameTab(InterfaceManager.EMOTES_TAB, lock);
	}
	...
	public void sendProtectedItems() {
		for (int i = 0; i < getProtectSlots(); i++)
			player.getVarsManager().sendVarBitOld(9222 + i, i >= slots[0].length ? -1 : slots[0][i]);
	}
