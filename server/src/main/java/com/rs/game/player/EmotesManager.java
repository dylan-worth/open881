...
public final class EmotesManager implements Serializable {
	...
	public void useBookEmote(int slotId, int packetId) {
		...
		case 37:// Skill Capes
			...
			case 32054:
			case 32055:
				player.getDialogueManager().startDialogue("SimpleMessage", "Not letting you do this, bitch - miles M");
				break;
			case 19709:
			case 19710:
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (step == 0) {
							...
							World.sendGraphics(player, new Graphics(2777),
									new WorldTile(player.getX() - 1, player.getY() - 1, player.getPlane()));
							...
							World.sendGraphics(player, new Graphics(2778),
									new WorldTile(player.getX() + 1, player.getY() - 1, player.getPlane()));
							player.setNextFaceWorldTile(
							...
							World.sendGraphics(player, new Graphics(2779),
									new WorldTile(player.getX(), player.getY() - 1, player.getPlane()));
							World.sendGraphics(player, new Graphics(2780),
									new WorldTile(player.getX(), player.getY() + 1, player.getPlane()));
							player.setNextAnimation(new Animation(14610));
							GameExecutorManager.slowExecutor.schedule(new Runnable() {
								@Override
								public void run() {
									player.setNextGraphics(new Graphics(2783));
								}
		...
		case 48:// Puppet Master
			...
			break;
		case 49:// Task Master
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 15034 : 15033));
		...
		case 94:// Troubadour dance
			...
			break;
		case 95:// Evil Laugh
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 15536 : 15535));
			break;
		case 96:// Golf Clap
			player.setNextAnimation(new Animation(15520));
			break;
		case 97:// LOLcano
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 15533 : 15532));
		...
		case 100:// Your Dead
			...
			break;
		case 101:// Scream
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 15527 : 15526));
			break;
		case 102:// Tornado
		...
		case 103:// Chaotic Cookery
			...
			break;
		case 104:// ROFLcopter
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 16374 : 16373));
		...
		case 119:// crab transform
			player.setNextAnimation(new Animation(!player.getAppearence().isMale() ? 17213 : 17212));
		...
		case 184:// Cracker Pull
			...
			break;
		default:
			player.getPackets().sendGameMessage("Animation at slot " + slotId + " has not been added yet.");
			break;
		}
		if (!isDoingEmote())
			setNextEmoteEnd();
	...
	void unlockEmotesBook() {
		//player.getPackets().sendUnlockIComponentOptionSlots(590, 8, 0, 196, 0, 1);
		//player.getPackets().sendUnlockIComponentOptionSlots(590, 13, 0, 11, 2);
		player.getPackets().sendIComponentSettings(590, 4, 0, 213, 8388614);
		player.getPackets().sendExecuteScript(4717, 38666248, 38666247, 38666249, 3874);
		player.getPackets().sendExecuteScript(4717, 94240781, 94240780, 94240782, 3874);
