...
public class SerenStoneCleansing extends Action {
    ...
    public static boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.PRAYER) < 70) {
			player.getPackets().sendGameMessage("You need a Prayer level of at least 70 to do this activity.");
		    return false;
		}
		if (!player.getInventory().containsItems(new Item(32615, 1))) {
    ...
    public int processWithDelay(Player player) {
		ticks --;
		if (!player.getInventory().containsItems(new Item(32615, 1))) {
			player.getPackets().sendGameMessage("You have no more cleansing crystals to work with.");
			this.stop(player);
		}
