...
public class SlayerManager implements Serializable {
	...
	private void addPoints() {
		...
		pointsIncreased *= Settings.getDropQuantityRate();
				...
				if (removePoints(100)) {
					for (int index = 0; index < 6; index++) {
						...
						if (task == null) {
							...
							player.getPackets().sendGameMessage(
									"You have cancled the task " + currentTask.getName() + " permenantly.");
							skipCurrentTask(true);
							sendSlayerInterface(ASSIGNMENT_INTERFACE);
							return;
						}
	...
	public Object[] calculateTask() {
		...
		while (true) {
			...
			return new Object[] { task, (int) Utils.random(currentMaster.getTasksRange()[0] * task.getTaskFactor(),
					currentMaster.getTasksRange()[1] * task.getTaskFactor()) };
		}
	}

	private static boolean hasRequirement(Player p, SlayerTask task) {
		if (task == SlayerTask.SHADOW_WARRIOR && !p.getQuestManager().completedQuest(Quests.LEGENDS_QUEST)
				|| task == SlayerTask.SKELETAL_WYVERN
						&& !p.getQuestManager().completedQuest(Quests.ELEMENTAL_WORKSHOP_I))
			return false;
		if (p.getSlayerManager().getCurrentMaster() == SlayerMaster.KURADAL) {
			if ((task == SlayerTask.ICE_STRYKEWYRM && !p.isCompletedFightCaves()
					&& !p.getSlayerManager().hasLearnedStykes())
					|| (task == SlayerTask.AQUANITE && !p.getSlayerManager().hasLearnedAquanite()))
				return false;
		}
	...
	public void checkCompletedTask(int damageAdmitted, int otherDamageAdmitted) {
		...
		if (currentTaskCount + otherSocialCount == maximumTaskCount) {
			if (socialPlayer != null)
				socialPlayer.getPackets().sendGameMessage(
						"<col=008000>You have finished your slayer task, talk to a slayer master for a new one.</col>");
			updateInterface();
			resetTask(true, true);
			player.getPackets()
					.sendGameMessage("<col=008000>You've completed " + player.getSlayerManager().getCompletedTasks()
							+ " tasks in a row and gain " + player.getSlayerManager().getEarnedPoints()
	...
	public void checkKillsLeft() {
		...
		if (socialPlayer != null) {
			player.getPackets().sendGameMessage("Your partner's current assignment is: " + currentTask.getName()
					+ "; only " + player.getSlayerManager().getCount() + " more to go.");
	...
	public void updateInterface() {
		...
		player.getPackets().sendCSVarString(4967,
				player.getSlayerManager().getCurrentTask() == null ? ""
						: getCount() >= 1 ? getCurrentTask().toString().toLowerCase().replace("_", " ") + "s"
								: getCurrentTask().toString().toLowerCase().replace("_", " "));
	}
}