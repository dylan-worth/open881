...
public final class Drinkables {
	...
	public static enum Drink {
		...
		ANTIPOISON_DOUBLE_PLUS_Flask(new int[] { 23591, 23593, 23595, 23597, 23599, 23601 }, Effects.ANTIPOISON_DOUBLE_PLUS),
	...
	private enum Effects {
		...
		WEAK_STAT_CURE_POTION() {
			...
			public void extra(Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.FIRE_IMMUNITY, 500));
		...
		REGULAR_STAT_CURE_POTION() {
			@Override
			public void extra(Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.SUPER_FIRE_IMMUNITY, 1000));
		...
		STRONG_STAT_CURE_POTION() {
			@Override
			public void extra(Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.SUPER_FIRE_IMMUNITY, 2000));
		...
		ZAMORAK_BREW(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE) {
			...
			public boolean canDrink(Player player) {
				...
				player.getPrayer().restorePrayer((int) ((player.getPrayer().getPrayerpoints() * 1.08) + 20));
				return true;
			}
		},
		...
		SANFEW_SERUM(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.MAGIC, Skills.RANGE, Skills.AGILITY, Skills.COOKING, Skills.CRAFTING, Skills.FARMING, Skills.FIREMAKING, Skills.FISHING, Skills.FLETCHING, Skills.HERBLORE, Skills.MINING, Skills.RUNECRAFTING, Skills.SLAYER, Skills.SMITHING, Skills.THIEVING, Skills.WOODCUTTING, Skills.SUMMONING) {
			...
			@Override
			public void extra(Player player) {
				player.getPrayer().restorePrayer((int) ((int) ((player.getSkills().getLevelForXp(Skills.PRAYER) * 0.25 * 10) + 80) * player.getAuraManager().getPrayerPotsRestoreMultiplier()));
		...
		ANTI_FIRE() {
			...
			public void extra(final Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.FIRE_IMMUNITY, 600));
			}
		},
		SUPER_ANTI_FIRE() {
			@Override
			public void extra(final Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.SUPER_FIRE_IMMUNITY, 600));
			}
		},
		...
		SUPER_SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {
		...
		OVERLOAD() {
			...
			@Override
			public void extra(final Player player) {
				Effect currentEffect = player.getEffectsManager().getEffectForType(EffectType.OVERLOAD);
				if (currentEffect == null || currentEffect != null && currentEffect.getCycle() < 480) {
				...
				player.getEffectsManager().startEffect(new Effect(EffectType.OVERLOAD, 501));
			}
		},
		SUPER_PRAYER() {
			@Override
			public void extra(Player player) {
				player.getPrayer().restorePrayer((int) ((int) (70 + (player.getSkills().getLevelForXp(Skills.PRAYER) * 3.5)) * player.getAuraManager().getPrayerPotsRestoreMultiplier()));
			}
		},
		PRAYER_RENEWAL() {
			@Override
			public void extra(Player player) {
				player.getEffectsManager().startEffect(new Effect(EffectType.PRAYER_RENEWAL, 500));
			}
		},
		...
		RESTORE_POTION(Skills.ATTACK, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {
	...
	public static int mixPot(Player player, Item fromItem, Item toItem, int fromSlot, int toSlot, boolean single) {
		if (single) {
			if (fromItem.getId() == VIAL || toItem.getId() == VIAL) {
				Drink pot = getDrink(fromItem.getId() == VIAL ? toItem.getId() : fromItem.getId());
	...
	public static boolean drink(Player player, Item item, int slot) {
		...
		if (dosesLeft == 0 && drink.isFlask())
			...
			player.getInventory().getItems().set(slot, new Item(dosesLeft > 0 ? drink.getIdForDoses(dosesLeft) : item.getDefinitions().isDungeoneeringItem() ? DUNGEONEERING_VIAL : drink.isPotion() ? VIAL : getReplacedId(drink), 1));
			player.getInventory().refresh(slot);
		}
		...
		player.setNextAnimationNoPriority(new Animation((player.getCombatDefinitions().isCombatStance() && !player.isInLegacyCombatMode()) ? 18003 : 18000));
		if (drink.isFlask() || drink.isPotion()) {
			...
			player.getPackets().sendGameMessage(dosesLeft == 0 ? "You have finished your " + (drink.isFlask() ? "flask and the glass shatters to pieces." : drink.isPotion() ? "potion." : item.getName().toLowerCase() + ".") : "You have " + dosesLeft + " dose of potion left.", true);
		}
		return true;
	}
	...
	public static void applyOverLoadEffect(Player player) {
		...
		int realLevel = player.getSkills().getLevelForXp(Skills.ATTACK);
		int level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.ATTACK, (int) (level + 3 + (realLevel * 0.15)));

		actualLevel = player.getSkills().getLevel(Skills.STRENGTH);
		realLevel = player.getSkills().getLevelForXp(Skills.STRENGTH);
		level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.STRENGTH, (int) (level + 3 + (realLevel * 0.15)));

		actualLevel = player.getSkills().getLevel(Skills.DEFENCE);
		realLevel = player.getSkills().getLevelForXp(Skills.DEFENCE);
		level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.DEFENCE, (int) (level + 3 + (realLevel * 0.15)));

		actualLevel = player.getSkills().getLevel(Skills.MAGIC);
		realLevel = player.getSkills().getLevelForXp(Skills.MAGIC);
		level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.MAGIC, (int) (level + 3 + (realLevel * 0.15)));

		actualLevel = player.getSkills().getLevel(Skills.RANGE);
		realLevel = player.getSkills().getLevelForXp(Skills.RANGE);
		level = actualLevel > realLevel ? realLevel : actualLevel;
		player.getSkills().set(Skills.RANGE, (int) (level + 3 + (realLevel * 0.15)));
	}
