package com.rs.game.player.actions.skills.divination;
...
public class HarvestAction extends Action {
	...
	private static final double[] EXPERIENCE = { /* Regular Wisps */1, 2, 3, 4, 5, 6, 7, 8, 8.5, 9, 10, 11, 12,
	...
	public int processWithDelay(Player player) {
		...
		/*
		 * player.getSkills().addXp(Skills.DIVINATION, xp); player.getInventory().addItem(new Item(29312 + xp, 1));
		 * int gfxId = 4235, memoryId = 29383+xp, rate = 12-(player.getSkills().getLevel(Skills.DIVINATION) / 10);
		 * if(Utils.random(2) == 1) player.getInventory().addItem(new Item(memoryId+(enriched ? 11 : 0), 1)); else
		 * if(Utils.random(rate) == 1 && wisp.getOriginalId() != 18150) { gfxId++;
		 * player.getInventory().addItem(memoryId + 11, 1); } player.setNextGraphics(new Graphics(gfxId));
		 */
		return 2;
	}
