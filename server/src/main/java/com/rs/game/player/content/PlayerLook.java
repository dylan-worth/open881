...
public final class PlayerLook {
	...
	public static void openCharacterCustomizing(Player player, boolean create) {
		...
		player.getPackets().sendExecuteScript(7936, new Object[] { 1 }); // confirm																// button
		if (!create)
			return;
	...
	public static void openMageMakeOver(Player player) {
		...
		player.getVarsManager().sendVarBitOld(6098, player.getAppearence().isMale() ? 0 : 1);
		player.getVarsManager().sendVarBitOld(6099, player.getAppearence().getSkinColor());
