...
public class Wilderness extends Controller {
	...
	public void dropArtefact(Player killer) {
		String[] keys = new String[] { killer.getUsername() + "@" + player.getUsername(), killer.getSession().getIP() + "@" + player.getSession().getIP(), killer.getLastGameMAC() + "@" + player.getLastGameMAC() };
