...
public class FriendsIgnores {
	...
	public void initialize() {
		LoginClientChannelManager.sendReliablePacket(
				LoginChannelsPacketEncoder.encodePlayerFriendIgnoreSendAll(player.getUsername()).getBuffer());
	}
	...
	public void setPmStatus(int pmStatus, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_PMSTATUS, pmStatus)
					.getBuffer());
	}
	...
	public void setFcName(String newName, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCNAME, fcName).getBuffer());
	...
	public void setFcJoinReq(int newReq, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCJOINREQ, fcJoinReq)
					.getBuffer());
		if (!player.isLobby() && player.getInterfaceManager().containsInterface(1108))
	...
	public void setFcTalkReq(int newReq, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCTALKREQ, fcTalkReq)
					.getBuffer());
		if (!player.isLobby() && player.getInterfaceManager().containsInterface(1108))
	...
	public void setFcKickReq(int newReq, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCKICKREQ, fcKickReq)
					.getBuffer());
		if (!player.isLobby() && player.getInterfaceManager().containsInterface(1108))
	...
	public void setFcLootShareReq(int newReq, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCLSREQ, fcLootshareReq)
					.getBuffer());
		if (!player.isLobby() && player.getInterfaceManager().containsInterface(1108))
	...
	public void setFcCoinshare(boolean coinshare, boolean updateAccount) {
		...
		if (updateAccount)
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
					.encodeAccountVarUpdate(player.getUsername(), LoginProtocol.VAR_TYPE_FCCSSTATE, fcCoinshare ? 1 : 0)
					.getBuffer());
	}
	...
	public void sendPrivateMessage(String displayName, String message) {
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
				.encodePlayerSendPrivateMessage(player.getUsername(), displayName, message).getBuffer());
	}
	...
	public void sendPrivateMessage(String displayName, QuickChatOptionDefinition option, long[] qcData) {
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
				.encodePlayerSendPrivateMessage(player, displayName, option, qcData).getBuffer());
	}
	...
	public void addFriend(String displayName) {
		LoginClientChannelManager
				.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFriendIgnoreOperation(player.getUsername(),
						displayName, LoginProtocol.FRIENDIGNORE_OP_FADD).getBuffer());
	}
	...
	public void removeFriend(String displayName) {
		...
		LoginClientChannelManager
				.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFriendIgnoreOperation(player.getUsername(),
						displayName, LoginProtocol.FRIENDIGNORE_OP_FREMOVE).getBuffer());
	}
	...
	public void addIgnore(String displayName, boolean temp) {
		LoginClientChannelManager
				.sendReliablePacket(LoginChannelsPacketEncoder
						.encodePlayerFriendIgnoreOperation(player.getUsername(), displayName,
								temp ? LoginProtocol.FRIENDIGNORE_OP_IADDTMP : LoginProtocol.FRIENDIGNORE_OP_IADD)
						.getBuffer());
	}
	...
	public void removeIgnore(String displayName) {
		...
		LoginClientChannelManager
				.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFriendIgnoreOperation(player.getUsername(),
						displayName, LoginProtocol.FRIENDIGNORE_OP_IREMOVE).getBuffer());
	}
	...
	public void changeRank(String displayName, int newRank) {
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder
				.encodePlayerFriendIgnoreSetRank(player.getUsername(), displayName, newRank).getBuffer());
	}
	 ...
	 */
	public void updateFriend(boolean isStatusUpdate, String displayName, String previousDisplayName, int world,
		...
		player.getPackets().appendFriend(friendsUpdatePacket, isStatusUpdate, displayName, previousDisplayName, world,
				fcRank, worldName);
	}
	...
	public void handleFriendListButtons(int interfaceId, int componentId) {
		if ((interfaceId == 1441 && componentId == 28) || (interfaceId == 550 && componentId == 25)
				|| (interfaceId == 550 && componentId == 41) || (interfaceId == 235 && componentId == 16))
			openInputFriend(false);
		else if ((interfaceId == 1441 && componentId == 22) || (interfaceId == 550 && componentId == 33)
				|| (interfaceId == 235 && componentId == 10))
			openInputFriend(true);
		else if ((interfaceId == 1441 && componentId == 10) || (interfaceId == 550 && componentId == 74))
			openInputIgnore(false);
		else if ((interfaceId == 1441 && componentId == 4) || (interfaceId == 550 && componentId == 82))
	...
	public void handleFriendChatButtons(int interfaceId, int componentId, int packetId) {
		if (interfaceId == 1427 || interfaceId == 1109 || interfaceId == 1472 || interfaceId == 1468) {
			if ((interfaceId == 1427 && componentId == 10) || (interfaceId == 1109 && componentId == 10)
					|| (interfaceId == 1472 && componentId == 228) || (interfaceId == 1468 && componentId == 2)) {
				...
				if (player.getCurrentFriendsChat() == null) {
					player.getPackets()
							.sendGameMessage("You need to be in a Friends Chat channel to activate LootShare.");
					player.refreshLootShare();
					return;
				}
				player.getCurrentFriendsChat().toogleLootshare(player);
			} else if ((interfaceId == 1427 && componentId == 4) || (interfaceId == 1472 && componentId == 231)
				...
				player.stopAll();
				openFriendChatSetup();
			} else if ((interfaceId == 1427 && componentId == 37) || (interfaceId == 1109 && componentId == 40)) {
				if (player.getCurrentFriendsChat() == null)
					return;
				openKickFriendChat();
			}
	...
	public void refreshChatName() {
		player.getPackets().sendIComponentText(1108, 9, fcName == null ? "Chat disabled" : fcName);
	}
