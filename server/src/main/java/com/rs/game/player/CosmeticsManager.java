...
public class CosmeticsManager implements Serializable {
	...
	public transient Item[] cosmeticPreview;
	...
	public void init() {
		...
		player.getPackets().sendItems(675, keepSakeItems.toArray(new Item[keepSakeItems.size()]));
	}
	...
	public void open(CosmeticType type) {
		for (int x = 153; x < 172; x++) {
			for (int x2 = 174; x < 176; x++) {
				...
				player.getPackets().sendIComponentSettings(1311, x2, 0, 500, 10);
			}
		}
	...
	player.getPackets().sendIComponentText(1311, 424, "");
		...
		for (int i = 0; i < MENUS_COMPONENT_IDS.length; i++) {
			for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(1311); x++) {

			}
            player.getPackets().sendUnlockIComponentOptionSlots(1311, MENUS_COMPONENT_IDS[i], 0, 1000, 0, 1);
		}
		System.out.println("fixed appearence1");
		if (type == CosmeticType.WARDROBE) {
			System.out.println("fixed appearence2");
	...
	private void sendPreviewTitle(int titleId) {
		...
		player.getPackets().sendExecuteScript(6453, Appearence.isTitleAfterName(titleId) ? 0 : 1,
				title == null ? "" : title);
	}
	...
	private void setCosmetic(ClientScriptMap map, int slot, boolean preview) {
		System.out.println("setcosmetic");
		...
		if (item.getDefinitions().equipSlot != -1
				&& (player.getAppearence().isMale() ? item.getDefinitions().getMaleWornModelId1() == -1
	...
	public void setCosmetic(Item item, boolean preview) {
	...
	public void setCosmetic1(Item item, boolean preview) {
		cosmeticPreview = player.getEquipment().getCosmeticItems().getItemsCopy();
		int slot = 1;
		cosmeticPreview[slot] = null;
		if (item.getDefinitions().equipSlot != -1
	...
	private static final int[] MENUS_COMPONENT_IDS = { 153, 154, 155, 156, 158, 159, 160, 161, 162, 163, 164, 165, 166,
			167, 168, 169, 170, 171, 172, 173, 174, 175, 324 };
	...
	private Integer[] getCosmeticIds(CosmeticType type, int menuId) {
		...
		for (int index = 0; index < getCosmeticsIds(type).getSize(); index++) {
			...
			if (data.getIntValue(2532) == menuId && ((showingAllItems && data.getIntValue(2547) == 0)
					|| isAutoUnlocked(data.getId()) || unlockedCosmetics.contains(data.getId())
							&& hasCosmeticVar(data.getId(), data.getIntValue(2546) == 1))) {
				ids.add(index);
			}
		}
	...
	public void handleButtons(int componentId, int slotId, int slotId2, int packetId) {
		if (componentId == 515)
			...
			if (id == null) {
				Item item = (Item) player.getTemporaryAttributtes().remove("Keepsake");
				if (item != null) {
			...
			int index = (int) player.getTemporaryAttributtes().remove("KeepSakeIndex");
			...
			if (!player.getInventory().addItem(item)) {
				...
				player.getPackets().sendIComponentText(1311, 653, "Success! Your item has been added to your bank.");
				return;
			}
			player.getPackets().sendIComponentText(1311, 653, "Success! Your item has been added to your inventory.");
			revert();
		} else {
			for (int menuId = 0; menuId < MENUS_COMPONENT_IDS.length; menuId++) {
				if (MENUS_COMPONENT_IDS[menuId] == componentId) {
					...
					int index = slotId / 3;
					if (type == CosmeticType.WARDROBE) {
						player.getPackets().sendGameMessage("" + slotId + " , menuid " + menuId);
						if (menuId == 19) {
						...
						if (menuId == 21) {// clear all
							if (index == 0)
								...
								clearCosmetic(packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET,
										Equipment.SLOT_HAT);
							else if (index == 2)
								clearCosmetic(packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET,
										Equipment.SLOT_CAPE);
						...
						int dataId = ClientScriptMap.getMap(ClientScriptMap.getMap(8464).getIntValue(menuId))
								.getIntValue(slotId / 3);
						if (dataId == -1)
							return;
						GeneralRequirementMap data = GeneralRequirementMap.getMap(dataId);
						System.out.println("dataId=" + dataId + ", MenuId= " + menuId + ", index = " + index);
						if (!(isAutoUnlocked(data.getId())
								|| hasCosmeticVar(data.getId(), data.getIntValue(2546) == 1)))
							player.getPackets().sendGameMessage(
									"This cosmetic var is not currently added. Name: " + data.getValue(2533));
						...
						if (index >= listIds.length) // cant happen
							return;
						GeneralRequirementMap data = getCosmeticData(type, listIds[index]);
						player.getPackets().sendGameMessage("menu= " + menuId + " ,type= " + type + " , index= " + index
								+ " , emoteId= " + data.getIntValue(2535));
	...
	private void spawnPet(int dataId) {
		int growth = 2;
		GeneralRequirementMap data = GeneralRequirementMap.getMap(ClientScriptMap.getMap(7225).getIntValue(dataId));
		int npcId = (growth == 2 || data.getIntValue(3150) == 0) ? data.getIntValue(3152)
				: growth == 1 ? data.getIntValue(3150) : data.getIntValue(3148);
		World.spawnNPC(npcId, new WorldTile(player.getX(), player.getY(), player.getPlane()), 0, false);
	}
	...
	private void sendPreviewPet(int dataId) {
		...
		RenderAnimDefinitions rDef = RenderAnimDefinitions
				.getRenderAnimDefinitions(NPCDefinitions.getNPCDefinitions(npcId).renderEmote);
		int emoteId = rDef.walkAnimation;
		int zoom = ndef.size * 100;
		int height1 = 100;
		player.getPackets().sendGameMessage("" + emoteId + " " + NPCDefinitions.getNPCDefinitions(npcId).renderEmote);
		player.getPackets().sendExecuteScript(2716, dataId);
		player.getVarsManager().sendVar(3472,
				(Integer) NPCDefinitions.getNPCDefinitions(
						data.getIntValue(3150) == 0 ? data.getIntValue(3152) : data.getIntValue(3148)).clientScriptData
								.get(75));
		player.getPackets().sendExecuteScript(7422, npcId, emoteId, color, zoom, height1);
		if (data.getIntValue(3150) == 0)
			player.getPackets().sendExecuteScript(7425, "<col=E5B051><shad>" + ndef.getName() + "</col></shad>");
		else {
			String[] name = ndef.getName().split(" ");
			String sufix = growth == 0 ? "Baby" : growth == 1 ? "Adolescent" : "Adult";
			player.getPackets().sendExecuteScript(7424, sufix,
					name.length > 2 ? (name[0] + " " + name[1] + (dataId == 7013 ? (" " + name[2]) : "")) : name[0]);
		}
	}
	...
	public void buy(int id, CosmeticType type) {
		...
		this.open(CosmeticType.WARDROBE);// try this
		 player.getInterfaceManager().closeMenu();
		 player.getPackets().sendGameMessage("Bought cosmetic, please reopen the interface again.");
		player.lock(5);
		return;
		 }
	...
	public void apply(int id) {
		...
		if (type == CosmeticType.TITLE) {
			if (unlockedCosmetics.contains(id)) {
				...
				player.getPackets()
						.sendGameMessage(player.getAppearence().getTitleId() == 0 ? "Your title has been cleared."
								: "Your title has been changed to: " + player.getAppearence().getTitle() + ".");
				buy(id, type);
			}
	...
	private static void setVars() {
		cosmeticsVars1.put(11113, 859); //Supposed to be 860
	...
	public boolean keepSakeItem(Item itemUsed, Item itemUsedWith) {
		if (itemUsed.getId() != 25430 && itemUsedWith.getId() != 25430)
			return false;
		if (itemUsed.getId() == 25430 && itemUsedWith.getId() == 25430)
			return false;
		Item keepSakeKey = itemUsed.getId() == 25430 ? itemUsed : itemUsedWith;
		Item keepSakeItem = itemUsed.getId() == 25430 ? itemUsedWith : itemUsed;
		if (keepSakeItem == null || keepSakeKey == null)
			return false;
		if (keepSakeItems.size() >= 20) {
			player.getPackets().sendGameMessage("You can only keep sake 20 items.");
			return true;
		}
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void run(int interfaceId, int componentId) throws ClassNotFoundException {
				...
				case 2:
					sendOptionsDialogue(
							"Item to store: " + "<col=116190><shad>" + keepSakeItem.getName() + "</col></shad>",
							"Store this item in my Dragon Keepsake Box.", "Quit.");
					stage = 3;
					break;
				case 3:
					switch (componentId) {
					case OPTION_1:
						...
						player.getPackets().sendItems(675, keepSakeItems.toArray(new Item[keepSakeItems.size()]));
						player.getInventory().deleteItem(25430, 1);
	...
	public Item[] getCosmeticPreview(){
		return cosmeticPreview;
	}
	
	public void setCosmeticPreview(Item[] item){
		this.cosmeticPreview = item;
	}
}