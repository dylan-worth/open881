...
public class Magic {
	...
	public static final boolean hasInfiniteRunes(int runeId, int... itemIds) {
		for (int id : itemIds) {
			...
			if (defs.getCSOpcode(972) == 1 && runeId == AIR_RUNE || defs.getCSOpcode(973) == 1 && runeId == WATER_RUNE
	...
	public static void handleSpellOnItem(Player player, int spellId, byte slot) {
		...
		int spellBook = getSpellBook(data);// spellbook 3 means shared by all
		// books
		if (spellBook != 3 && spellBook != player.getCombatDefinitions().getSpellBook() || !hasLevel(player, data))
			return;
				...
				if (target.getId() == required[0].getId()) {
					if (player.getSkills().getLevel(Skills.SMITHING) >= bar.getLevelRequired()) {
						if (player.getInventory().containsItems(required)) {
							if (!Magic.checkRunes(player, true, NATURE_RUNE, 1, FIRE_RUNE, 4))
								return;
							player.lock(3);
							...
							player.getInterfaceManager().openGameTab(InterfaceManager.MAGIC_ABILITIES_TAB);
						} else
						...
						player.getPackets()
								.sendGameMessage("You need a Smithing level of at least " + bar.getLevelRequired()
										+ " to smelt " + bar.getProducedBar().getDefinitions().getName());
					}
				}
			}
		...
		case 47: // high alch
			...
			if (player.getDungManager().isInside()) {
				player.getInventory().addItem(
						new Item(18201, (int) (target.getDefinitions().getValue() * (highAlch ? 0.6D : 0.3D))));
			} else {
				player.getInventory().addItemMoneyPouch(
						new Item(995, (int) (target.getDefinitions().getValue() * (highAlch ? 0.6D : 0.3D))));
			}
		...
		case 139: // plank make
			...
			if (plank == null) {
				player.getPackets()
						.sendGameMessage("You can only convert plain, oak, teak and mahogany logs into planks.");
				return;
			}
	...
	public static final void handleSpellOnEntity(final Player player, int spellId, Entity target) {
		...
		case 107:// stat spy npc
			...
			player.getPackets().sendIComponentText(522, 3,
					"Creature's Max Hit: " + npc.getMaxHit(npc.getAttackStyle()));
			player.getPackets().sendIComponentText(522, 4,
					(player.getSlayerManager().isValidTask(npc.getName()) ? "Valid Slayer Task" : ""));
			player.setNextAnimation(new Animation(4413));
		...
		case 122:// stat spy
			...
			for (int i = 0; i < stat_spy_skills.length; i++) {
				player.getPackets().sendIComponentText(523, stat_spy_skills[i] == Skills.DIVINATION ? 110 : (1 + i * 4),
						"" + p2.getSkills().getLevel(stat_spy_skills[i]));
				player.getPackets().sendIComponentText(523, stat_spy_skills[i] == Skills.DIVINATION ? 111 : (2 + i * 4),
	...
	private static final int[] stat_spy_skills = { Skills.ATTACK, Skills.HITPOINTS, Skills.MINING, Skills.STRENGTH,
			...
			Skills.CONSTRUCTION, Skills.HUNTER, Skills.SUMMONING, Skills.DUNGEONEERING, Skills.DIVINATION };
	...
	public static final void handleSpell(Player player, int spellId, int packetId) {
		...
		case 111: // moonclan teleport
			sendLunarTeleportSpell(player, 69, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1,
					EARTH_RUNE, 2);
			break;
		case 113: // ourania teleport
			sendLunarTeleportSpell(player, 71, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1,
					EARTH_RUNE, 6);
			break;
		case 116: // south falador teleport
			sendLunarTeleportSpell(player, 72, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1,
					AIR_RUNE, 2);
			break;
		...
		case 121: // barbarian teleport
			sendLunarTeleportSpell(player, 75, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1,
					FIRE_RUNE, 3);
			break;
		case 123: // North Ardroudge teleport
			sendLunarTeleportSpell(player, 76, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1,
					WATER_RUNE, 5);
			break;
		case 127: // Khazard teleport
			sendLunarTeleportSpell(player, 78, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 2,
					WATER_RUNE, 4);
			break;
		case 137: // Fishing guild teleport
			sendLunarTeleportSpell(player, 85, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3,
					WATER_RUNE, 8);
			break;
		case 140: // Catherbay teleport
			sendLunarTeleportSpell(player, 87, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3,
					WATER_RUNE, 10);
			break;
		case 143: // Ice Plateau teleport
			sendLunarTeleportSpell(player, 89, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3,
					WATER_RUNE, 8);
			break;
		case 148: // Throheim teleport
			sendLunarTeleportSpell(player, 92, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3,
					WATER_RUNE, 10);
			break;
		case 105: // bake pie
		...
		case 144:
		case 149:
			int index = spellId == 112 ? 0
					: spellId == 118 ? 1
							: spellId == 124 ? 2
									: spellId == 128 ? 3
											: spellId == 138 ? 4 : spellId == 142 ? 5 : spellId == 144 ? 6 : 7;
			if (!checkRunes(player, data, true))
				return;
			...
			for (int regionId : player.getMapRegionsIds()) {
				...
				for (Integer playerIndex : playersIndexes) {
					...
					if (p2 == null || p2.isDead() || p2.hasFinished() || !p2.isRunning()
							...
							|| p2.getInterfaceManager().containsScreenInterface() || !p2.withinDistance(player, 5))
						continue;
		...
		case 119:// TODO cure group
			...
			break;
		case 83: // paddewa
			sendAncientTeleportSpell(player, 54, getSpellXP(data), getSpellTeleport(data), LAW_RUNE, 2, FIRE_RUNE, 1,
					AIR_RUNE, 1);
			break;
		case 86: // senntisten
		...
		case 92: // lassar
			...
			break;
		case 95: // dare
			sendAncientTeleportSpell(player, 78, getSpellXP(data), getSpellTeleport(data), LAW_RUNE, 2, FIRE_RUNE, 3,
					AIR_RUNE, 2);
			break;
		case 98: // care
		...
		case 53:// bones to peaches
			...
			if (!checkRunes(player, true, NATURE_RUNE, bones_to_peaches ? 2 : 1, EARTH_RUNE, bones_to_peaches ? 4 : 2,
					WATER_RUNE, bones_to_peaches ? 4 : 2))
				return;
			int bones = 0;
			for (int i = 0; i < 28; i++) {
		...
		case 156: // crossbow bolt enchant
			...
			break;
		case 19: // mobi
			sendNormalTeleportSpell(player, 10, getSpellXP(data), getSpellTeleport(data), LAW_RUNE, 1, WATER_RUNE, 1,
					AIR_RUNE, 1);
			break;
		case 28: // varrock
			sendNormalTeleportSpell(player, 25, getSpellXP(data), getSpellTeleport(data), FIRE_RUNE, 1, AIR_RUNE, 3,
					LAW_RUNE, 1);
			break;
		case 31: // lumby
			sendNormalTeleportSpell(player, 31, getSpellXP(data), getSpellTeleport(data), EARTH_RUNE, 1, AIR_RUNE, 3,
					LAW_RUNE, 1);
			break;
		case 34: // fally
			sendNormalTeleportSpell(player, 37, getSpellXP(data), getSpellTeleport(data), WATER_RUNE, 1, AIR_RUNE, 3,
					LAW_RUNE, 1);
			break;
		case 36: // house
			sendNormalTeleportSpell(player, 40, getSpellXP(data), null, LAW_RUNE, 1, AIR_RUNE, 1, EARTH_RUNE, 1);
			break;
		case 39: // camelot
			sendNormalTeleportSpell(player, 45, getSpellXP(data), getSpellTeleport(data), AIR_RUNE, 5, LAW_RUNE, 1);
			break;
		case 45: // ardy
			sendNormalTeleportSpell(player, 51, getSpellXP(data), getSpellTeleport(data), WATER_RUNE, 2, LAW_RUNE, 2);
			break;
		case 50: // watch
			sendNormalTeleportSpell(player, 58, getSpellXP(data), new WorldTile(2549, 3114, 2), EARTH_RUNE, 2, LAW_RUNE,
					2);
			break;
		case 57: // troll
		...
		case 170: // godswars
			if (!player.isDonator()) {
				player.getDialogueManager().startDialogue("SimpleMessage",
						"You need to be donator to use this feature.");
				return;
			}
			sendNormalTeleportSpell(player, 61, getSpellXP(data), getSpellTeleport(data), FIRE_RUNE, 2, LAW_RUNE, 2);
			break;
		case 64: // ape
			sendNormalTeleportSpell(player, 64, getSpellXP(data), getSpellTeleport(data), FIRE_RUNE, 2, WATER_RUNE, 2,
					LAW_RUNE, 2, 1963, 1);
			break;
		}
	...
	public static final void processDungSpell(Player player, int spellId, int slot, int packetId) {
		 ...
		 * !player.getInventory().hasFreeSlots()) { player.getPackets().
		 * sendGameMessage("Not enough space in your inventory."); return; } if
	...
	public static void useVecnaSkull(Player player) {
		...
		if (time != null && time >= currentTime) {
			...
			player.getPackets().sendGameMessage(
					"The skull has not yet regained its mysterious aura. You will have to wait another " + minutes
							+ " minutes.");
			return;
		}
	...
	public static void useEctoPhial(final Player player, Item item) {
		...
		public static void resourcesTeleport(final Player player, final WorldTile tile, int level) {
	    	...
	    	if (player.getSkills().getLevelForXp(Skills.DUNGEONEERING) < level) {
	    		player.getPackets().sendGameMessage("You need a Dungeoneering level of at least "+level+" to enter this dungeon.");
	    		return;
	    	}
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
		    		...
		    		player.setNextFaceWorldTile(new WorldTile(
		    				tile.getX(), tile.getY() - 1, tile.getPlane()));
		    		player.setDirection(6);
