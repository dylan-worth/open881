...
public class ClansManager {
	...
	public static void viewClanmateDetails(Player player, ClanMember member) {
		...
		player.getPackets().sendCSVarInteger(1565, member.isBanFromKeep() ? 1 : 0);
		player.getPackets().sendCSVarInteger(1566, member.isBanFromCitadel() ? 1 : 0);
		player.getPackets().sendCSVarInteger(1567, member.isBanFromIsland() ? 1 : 0);
		player.getPackets().sendCSVarInteger(1568, member.firstWeek() ? 1 : 0);
		player.getPackets().sendCSVarString(2521, Utils.formatPlayerNameForDisplay(member.getUsername()));
		player.getPackets().sendExecuteScript(4319);
	}

	public static void handleClanChatButtons(Player player, int interfaceId, int componentId, int slotId) {
		if (interfaceId == 1110 || interfaceId == 1440 || interfaceId == 234 || interfaceId == 233) {
			if ((interfaceId == 1110 && componentId == 118) || (interfaceId == 1440 && componentId == 109)
					|| (interfaceId == 234 && componentId == 34))
				ClansManager.joinClanChatChannel(player);
			if ((interfaceId == 1110 && componentId == 126) || (interfaceId == 1440 && componentId == 117)
					|| (interfaceId == 234 && componentId == 28)) {
				//ClanVexillum.openClanDetails(player);
			}
			if ((interfaceId == 1110 && componentId == 142) || (interfaceId == 1440 && componentId == 133)
					|| (interfaceId == 234 && componentId == 16))
				ClansManager.openClanSettings(player);
			if ((interfaceId == 1110 && componentId == 59) || (interfaceId == 1440 && componentId == 41))
				ClansManager.joinGuestClanChat(player);
			if ((interfaceId == 1110 && componentId == 20) || (interfaceId == 1440 && componentId == 23)
					|| (interfaceId == 233 && componentId == 10))
				ClansManager.banPlayer(player);
			if ((interfaceId == 1110 && componentId == 28) || (interfaceId == 1440 && componentId == 31)
					|| (interfaceId == 233 && componentId == 4))
				ClansManager.unbanPlayer(player);
			else if ((interfaceId == 1110 && componentId == 34) || (interfaceId == 1440 && componentId == 14))
				ClansManager.unbanPlayer(player, slotId);
			else if ((interfaceId == 1110 && componentId == 159) || (interfaceId == 1440 && componentId == 149)
	...
	public static void unlockClanBanList(Player player, boolean menu) {
		player.getPackets().sendUnlockIComponentOptionSlots(menu ? 1440 : 1110, menu ? 14 : 34, 0, 200, 0);
	}
	...
	public static void openClanMottifInterface(Player player) {
		...
		for (int i = 0; i < manager.clan.getMottifColors().length; i++)
			player.getVarsManager().sendVar(2067 + i, manager.clan.getMottifColors()[i]);
	}
	...
	public void setMottifColor(Player player, int color, int part) {
		synchronized (this) {
			player.getVarsManager().sendVar(2067 + part, clan.getMottifColors()[part] = color);
	...
	public void sendQuickMessage(Player player, QuickChatMessage message) {
		synchronized (this) {
			...
			int rights = player.getRights();
			for (Player p2 : channelPlayers)
				p2.getPackets().receiveClanChatQuickChatMessage(membersOnline.contains(p2), displayName, rights, message);
		}
	}
	...
	private void setClanVars() {
		...
		if (clan.isRecruiting() || clan.isClanTime() || clan.getWorldId() != 0 || clan.getClanFlag() != 0)
			clanVars.put(3, (clan.isRecruiting() ? 1 : 0) | (clan.isClanTime() ? 1 : 0) << 1 | clan.getWorldId() << 2
					| clan.getClanFlag() << 10);
		...
		synchronized (cachedClans) {
			...
			if (!created) { // not loaded
				if (!SerializableFilesManager.containsClan(clanName)) {
					...
					player.getPackets().sendIComponentText(1110, 136,
							"Could not find a clan named " + clanName + ". Please check the name and try again.");
					if (player.getInterfaceManager().containsInterface(1440))
						player.getPackets().sendIComponentText(1440, 90,
								"Could not find a clan named " + clanName + ". Please check the name and try again.");
					return false;
				}
	...
	public static void openClanSettings(Player player) {
		...
		player.getPackets().sendIComponentSettings(1096, 47, 0,
				Clan.MAX_MEMBERS, 2); // unlocks
		// clan
		// mates
		// select member rank
		player.getPackets().sendIComponentSettings(1096, 282, 0, 125,
				0);
		// select member profission
		player.getPackets().sendIComponentSettings(1096, 268, 0, 500,
				0);

		player.getPackets().sendIComponentSettings(1096, 296, 0, 200,
				0); // unlocks
		// world
		// id
		// setting
		player.getPackets().sendIComponentSettings(1096, 246, 0, 144,
				0); // unlocks
		// timezone
		// setting
	...
	public static void invite(Player player, Player p2) {
		...
		if (p2.getInterfaceManager().containsScreenInterface() || p2.getControlerManager().getControler() != null
