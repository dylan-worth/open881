...
public class GrandExchange {
	...
	private static void findBuyerSeller(Offer offer) {
		while (!offer.isCompleted()) {
			...
			for (Offer o : OFFERS.values()) {
				if (o.isBuying() == offer.isBuying() || o.getId() != offer.getId()
						|| o.isCompleted() || (offer.isBuying() && o.getPrice() > offer.getPrice())
	...
	private static void findBuyerSeller1(Offer offer) {
		while (!offer.isCompleted()) {
			Offer bestOffer = null;
			if (GrandExchange.getPrice(offer.getId()) <= 5000) {
				if ((offer.isBuying() && offer.getPrice() >= GrandExchange.getPrice(offer.getId()))
						|| (!offer.isBuying() && offer.getPrice() <= GrandExchange.getPrice(offer.getPrice())))
					bestOffer = new Offer(offer.getId(), offer.getAmount(), GrandExchange.getPrice(offer.getId()),
							!offer.isBuying());
			}
			if (bestOffer == null)
				break;
