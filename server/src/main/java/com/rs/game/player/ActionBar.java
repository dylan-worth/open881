...
import com.rs.game.npc.others.GuthixBlessing;
...
public class ActionBar implements Serializable {
	...
	public static final int ITEM_SHORTCUT = 0, MELEE_ABILITY_SHORTCUT = 1, STRENGTH_ABILITY_SHORTCUT = 2,
			RANGED_ABILITY_SHORTCUT = 5, DEFENCE_ABILITY_SHORTCUT = 3, HEAL_ABILITY_SHORTCUT = 4,
			MAGIC_ABILITY_SHORTCUT = 6, PRAYER_SHORTCUT = 7;

	public static final int REGENERATION_VARBIT_ID = 24939;

	public static final int[] COMBAT_SETTINGS_ACTIONBAR_COMPONENTS = { 16, 29, 42, 55, 68, 81, 94, 107, 120, 133, 146 };
	...
	public static GeneralRequirementMap getAbilityData(int type, int abilityId) {
		int id = ClientScriptMap.getMap(CS_DATA_ID[type - 1]).getIntValue(abilityId);
	...
	public static class RangeAbilityShortcut extends Shortcut {
		...
		public boolean queue(Player player) {
			GeneralRequirementMap data = GeneralRequirementMap
					.getMap(ClientScriptMap.getMap(CS_DATA_ID[getType() - 1]).getIntValue(abilityId));
	...
	public void setShortcut1(int index, Shortcut shortcut) {
		...
		if (shortcuts[bitValue - 1][index] != null && shortcuts[bitValue - 1][index].getType() == shortcut.getType()
				&& shortcuts[bitValue - 1][index].getId(player) == shortcut.getId(player)) {
			return;
		}
		if (getQueueShortcutIndex() == index)
			removeQueueShortcut();
	...
	public void setupBar() {
		...
		int mainAbilityType = type == Combat.MELEE_TYPE ? MELEE_ABILITY_SHORTCUT
				: type == Combat.RANGE_TYPE ? RANGED_ABILITY_SHORTCUT
						: type == Combat.MAGIC_TYPE ? MAGIC_ABILITY_SHORTCUT : -1;

		int secondAbilityType = type == Combat.MELEE_TYPE ? STRENGTH_ABILITY_SHORTCUT : -1;
		int shortcutCount = 0;
		if (mainAbilityType != -1) {
			for (Long abilityId : ClientScriptMap.getMap(CS_DATA_ID[mainAbilityType - 1]).getValues().keySet()) {

				if (mainAbilityType == MELEE_ABILITY_SHORTCUT && abilityId == 13
						|| (mainAbilityType == MAGIC_ABILITY_SHORTCUT && !isMagicAbility(abilityId.intValue())))
					continue;
				if (!hasRequiriments(abilityId.intValue(), getAbilityData(mainAbilityType, abilityId.intValue()), false,
						true))
					continue;
				shortcuts[bitValue][shortcutCount++] = mainAbilityType == MELEE_ABILITY_SHORTCUT
						? new MeleeAbilityShortcut(abilityId.intValue())
						: mainAbilityType == RANGED_ABILITY_SHORTCUT ? new RangeAbilityShortcut(abilityId.intValue())
		...
		if (secondAbilityType != -1 && shortcutCount < 12) {
			for (Long abilityId : ClientScriptMap.getMap(CS_DATA_ID[secondAbilityType - 1]).getValues().keySet()) {
				if (!hasRequiriments(abilityId.intValue(), getAbilityData(secondAbilityType, abilityId.intValue()),
						false, true))
					continue;

				shortcuts[currentBar][shortcutCount++] = new StrengthAbilityShortcut(abilityId.intValue());
				if (shortcutCount == 12)
					break;
			}
		}
		...
		if (mainAbilityType != -1) {
			for (Long abilityId : ClientScriptMap.getMap(CS_DATA_ID[mainAbilityType - 1]).getValues().keySet()) {
				...
				if (!hasRequiriments(abilityId.intValue(), getAbilityData(mainAbilityType, abilityId.intValue()), false,
						true))
					continue;
				shortcuts[currentBar][shortcutCount++] = mainAbilityType == MELEE_ABILITY_SHORTCUT
						? new MeleeAbilityShortcut(abilityId.intValue())
						: mainAbilityType == RANGED_ABILITY_SHORTCUT ? new RangeAbilityShortcut(abilityId.intValue())
								: new MagicAbilityShortcut(abilityId.intValue());
				if (shortcutCount == 12)
	...
	public void processShortcut(Shortcut shortcut, int packetId, int shortcutIndex, boolean queue) {
		if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
			if (shortcut.getType() == MAGIC_ABILITY_SHORTCUT && shortcut.getId(player) == 8
					&& player.getEffectsManager().hasActiveEffect(EffectType.DETONATE)) {
				if (queue)
					removeQueueShortcut();
				...
				Effect effect = player.getEffectsManager().getEffectForType(EffectType.DETONATE);
				int ticks = effect.getArguments() == null ? 10 : ((int) effect.getArguments()[0] / 5);
				for (Entity e : PlayerCombat.getMultiAttackTargets(player, target, 3, 9))
					PlayerCombat.delayHit(e, 0, PlayerCombat.getHit(player, e, true, Combat.MAGIC_TYPE, 1.0,
							1.5 + (ticks / 5), false, true, false));
				player.getEffectsManager().removeEffect(EffectType.DETONATE);
				return;
			} else if (shortcut.getType() == HEAL_ABILITY_SHORTCUT && shortcut.getId(player) == 15) {
				if (!queue) {
					...
					if (target != null) {
						...
						if (target instanceof Player) {
							...
							canSiphon = playerTarget.getCombatDefinitions().getSpecialAttackPercentage() > player
									.getCombatDefinitions().getSpecialAttackPercentage();
						}
						if (!canSiphon) {
			...
			if (shortcut.getType() == HEAL_ABILITY_SHORTCUT && shortcut.getId(player) == 1) {
				if (!queue) {
					if (player.getCombatDefinitions().getSpecialAttackPercentage() == 0) {
						player.getPackets().sendGameMessage("This ability requires adrenaline in order to be used.");
						return;
					}
					if (player.getCombatDefinitions().isCombatStance()) {
						if (player.getVarsManager().getBitValue(REGENERATION_VARBIT_ID) == 1) {
							player.getPackets().sendGameMessage(
									"Regenerate will no longer be activated when you have left combat.");
							removeQueueShortcut();
							return;
						}
						player.getPackets().sendGameMessage("Regenerate will be activated when you have left combat.");
						setQueueShortcut(shortcut, shortcutIndex);
						return;
					}
				} else {
				...
				// turn it on here
				player.getEffectsManager().startEffect(new Effect(EffectType.REGENERATE, 11));
				return;
			} else {
				if (queue) {
					Entity target = player.getCombatDefinitions().getCurrentTarget();
					...
					if (target == null || !PlayerCombat.isWithinDistance(player, target) || player.isStunned())
						return;

					if (player.getCombatDefinitions().getCombatMode() == CombatDefinitions.LEGACY_COMBAT_MODE)
						if ((int) (player.getCombatDefinitions().getMainHandDelay() - Utils.currentWorldCycle()) > 0)
							return;
					...
					return;
						...
						|| shortcut.getType() == RANGED_ABILITY_SHORTCUT || shortcut.getType() == HEAL_ABILITY_SHORTCUT
						|| shortcut.getType() == MAGIC_ABILITY_SHORTCUT) && hasCooldown(shortcut)) {
					if (player.getCombatDefinitions().isAllowAbilityQueueing()) {
						setQueueShortcut(shortcut, shortcutIndex);
						return;
					}
		...
		if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET
			...
			if (shortcut.getType() == ITEM_SHORTCUT) {
				...
				if (defs.isWearItem() && packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					Item item = player.getEquipment().getItem(defs.getEquipSlot());
					if (item != null && item.getId() == shortcut.getId(player)) {
						player.getEquipment().handleEquipment(defs.getEquipSlot(), shortcut.getId(player),
								WorldPacketsDecoder.ACTION_BUTTON1_PACKET);
						return;
					}
				}
				int slotId = player.getInventory().getItems().lookupSlot(shortcut.getId(player));
				if (slotId == -1) {
				...
				case WorldPacketsDecoder.ACTION_BUTTON5_PACKET:
					InventoryOptionsHandler.handleItemOption8(player, slotId, shortcut.getId(player), item);
					break;
		...
		case PRAYER_SHORTCUT:
			if (((PrayerShortcut) shortcut).curse != player.getPrayer().isAncientCurses())
				return;
	...
	private static final int[][] SHARED_CD_MAPS = { { 14682, 14699, 14664, 14664 }, // Binding
			// abilities
	...
	public void setCooldown(int abilityMap, int cooldown) {
		...
		player.getPackets().sendExecuteScript(6570, abilityMap, (int) currentCycle, ((int) currentCycle) + cooldown, 1,
				1);
		...
		if (abilityStyle > 0) {
								...
								: abilityStyle == 3 ? Skills.RANGE
										: abilityStyle == 4 ? Skills.MAGIC
												: abilityStyle == 5 ? Skills.DEFENCE
														: abilityStyle == 6 ? Skills.HITPOINTS : -1;
				if (skill != -1) {
					if (player.getSkills().getLevel(skill) < level) {
						if (warn)
							player.getPackets().sendGameMessage("You require level " + level + " "
									+ Skills.SKILL_NAME[skill] + " in order to use this ability.");
						return false;
					}
				}
			}
			if ((abilityStyle == 3 && abilityId == 3) || (abilityStyle == 4 && abilityId == 2)
					|| (abilityStyle == 2 && abilityId == 2) || (abilityStyle == 1 && abilityId == 6))
			...
			if (abilityStyle == 5) {
				...
				if (!hasAbilityStyle(player, abilityStyle, true)) {
					if (warn)
						player.getPackets().sendGameMessage("This ability requires a "
								+ ABILITY_STYLES[abilityStyle - 1] + " weapon in your main hand.");
					return false;
				}
			}
			...
			if (offHandRequired) {
				...
				if (!hasOffHand || !hasAbilityStyle(player, abilityStyle, false)) {
					if (warn)
						player.getPackets().sendGameMessage("You need to be dual wielding "
								+ ABILITY_STYLES[abilityStyle - 1] + " weapons to use this ability.");
					return false;
				}
			}
			...
			if (!setup && abilityType != 0 && player.getCombatDefinitions()
					.getSpecialAttackPercentage() < (abilityType == THRESHOLD_ABILITY ? 50 : 100)) {
				if (warn)
					player.getPackets()
							.sendGameMessage((abilityType == THRESHOLD_ABILITY ? "Threeshold" : "Ultimate")
									+ " abilities require you to have " + (abilityType == THRESHOLD_ABILITY ? 50 : 100)
	...
	private static boolean hasAbilityStyle(Player player, int abilityStyle, boolean mainHand) {
		...
		return ((abilityStyle == 1 || abilityStyle == 2) && handStyle == Combat.MELEE_TYPE)
				...
				|| (abilityStyle == 4 && handStyle == Combat.MAGIC_TYPE) || abilityStyle == 6;
	}
	...
	private void triggerAbility(Shortcut shortcut) {
		...
		if (player.getCombatDefinitions().getCombatMode() == CombatDefinitions.MOMENTUM_COMBAT_MODE)
			player.getCombatDefinitions().setCombatMode(CombatDefinitions.MANUAL_COMBAT_MODE);
		...
		if (bookId == HEAL_ABILITY_SHORTCUT && abilityId == 16) { // special
			...
			if (!weapon.getDefinitions().hasSpecialAttack()) {
				player.getPackets()
						.sendGameMessage("You can only do that with a weapon that can perform a special attack.");
				return;
			}
			setCooldown(14881, 3);
		...
		if (!shortcut.queue(player)) { // code one click abilities here
			...
			case MAGIC_ABILITY_SHORTCUT:
				...
				case 2:// SURGE
					...
					player.setNextForceMovement(new ForceMovement(player, 0, tile, 1,
				...
				case 8:// DETONATE
					...
					player.getEffectsManager().startEffect(new Effect(EffectType.DETONATE, 10, 0));// Six
					// seconds
					break;
				case 10:// METAMORPHISIS
					...
					player.getEffectsManager()
							.startEffect(new Effect(EffectType.METAMORPHISIS, 35, Magic.getSpellType(spellData)));
					break;
				case 164:// SUNSHINE
			...
			case RANGED_ABILITY_SHORTCUT:
				...
				case 3:// ESCAPE
					...
					player.setNextForceMovement(new ForceMovement(player, 0, tile, 1,
							Utils.getAngle(player.getX() - tile.getX(), player.getY() - tile.getY())));
					WorldTasksManager.schedule(new WorldTask() {
				...
				case 14:// DEATH'S SWIFTNESS
					player.getEffectsManager()
							.startEffect(new Effect(EffectType.DEATHS_SWIFTNESS, 50, new WorldTile(player)));
					break;
				}
				break;
			case MELEE_ABILITY_SHORTCUT:
				...
				case 2:// BARGE
					...
					player.setNextForceMovement(new ForceMovement(player, 0, tile, 1, player.getDirection()));
			...
			case STRENGTH_ABILITY_SHORTCUT:
				if (abilityId == 10)
					player.getEffectsManager().startEffect(new Effect(EffectType.BERSERK, 33));
				break;
			case DEFENCE_ABILITY_SHORTCUT:
				Effect effect = null;
				switch (abilityId) {
				case 1:// ANTICIPATION
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.ANTICIPATION, 16));
					break;
				case 2:// FREEDOM
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.FREEDOM, 10));
					break;
				case 7:// REFLECT
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.REFLECT, 17));
					break;
				case 4:// RESONANCE
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.RESONANCE, 10));
					break;
				case 5:// PREPERATION
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.PREPARATION, 10));
					break;
				case 9:// REVENGE
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.REVENGE, 33, 1.0));
					break;
				case 10:// BARRICADE
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.BARRICADE, 17));
					break;
				case 11:// REJUVINATE
					effect = new Effect(EffectType.REJUVINATE, 17, HitLook.HEALED_DAMAGE, null,
							new Double((player.getMaxHitpoints() * 0.4) * 0.05).intValue(), 2, player);
					player.getEffectsManager().startEffect(effect);
					break;
				case 12:// IMMORTALITY
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.IMMORTALITY, 50));
					break;
				case 14:// DEVOTION
					player.getEffectsManager().startEffect(effect = new Effect(EffectType.DEVOTION, 16));
					break;
				}
				assert effect != null;
				player.setAttackingDelay(((effect.getCycle() * 600) - 6000) + Utils.currentTimeMillis());
				break;
			case HEAL_ABILITY_SHORTCUT:
				switch (abilityId) {
				case 2:// INCITE
					if (player.getEffectsManager().hasActiveEffect(EffectType.INCITE)) {
						player.getEffectsManager().removeEffect(EffectType.INCITE);
						break;
					}
					player.getEffectsManager().startEffect(new Effect(EffectType.INCITE, -1));
					player.getPackets().sendGameMessage("Incite is now active.");
					break;
				case 9:// GUTHIX_BLESSING
					player.getEffectsManager().startEffect(new Effect(EffectType.GUTHIX_BLESSING, 17,
							new GuthixBlessing(player, new WorldTile(player))));
					break;
				case 14:// ICE ASYLUM
					player.getEffectsManager().startEffect(
							new Effect(EffectType.ICE_ASYLUM, 33, new WorldTile(player), player.getMaxHitpoints() * 3));
					break;
			...
			case MAGIC_ABILITY_SHORTCUT:
				...
				case 11:// TSUNAMI
					if (!player.withinDistance(target, 5)) {
						player.getPackets().sendGameMessage("There are no valid targets in that direction.");
						return;
					}
					break;
				}
			...
			case MELEE_ABILITY_SHORTCUT:
				...
				case 6:// BACKHAND
					if (!Utils.isOnRange(player, target, 1)) {
						player.getPackets().sendGameMessage("Target is too far away.");
						return;
					}
					break;
				case 12:// METEOR STRIKE
					int maxHP = target.getMaxHitpoints(), currentHP = target.getHitpoints();
		...
		// adrenaline drain
		if (!(abilityType == 0 && player.getCombatDefinitions().getCurrentTarget() == null))
			player.getCombatDefinitions()
					.increaseSpecialAttack(abilityType == 0
							? player.getEffectsManager().hasActiveEffect(EffectType.NATURAL_INSTINCT) ? 16 : 8
							: abilityType == 1 ? -15 : -100);
		...
		if (bookId == DEFENCE_ABILITY_SHORTCUT && abilityId == 14)// Manditory
			// exception
			player.getEffectsManager()
					.processBuffTimer(player.getEffectsManager().getEffectForType(EffectType.DEVOTION), true);
	}

	public void removeBuffs() {
		player.getEffectsManager().removeEffectsWithAction(EffectsManager.COMBO_BUFFS);
	}

	public boolean isComboOn() {
		return player.getEffectsManager().hasActiveEffect(EffectsManager.COMBO_BUFFS);
	}
	...
	private void setQueueShortcut(Shortcut shortcut, int shortcutIndex) {
		if (shortcutIndex != -1 && getQueueShortcutIndex() == shortcutIndex) { 
			removeQueueShortcut();
			return;
		}
		...
		else if (queueShortcut != null && queueShortcut.getType() == HEAL_ABILITY_SHORTCUT
	...
	private void refreshQueueShortcut() {
		player.getVarsManager().forceSendVar(4164, player.getVarsManager().getValue(4164));
		player.getVarsManager().forceSendVar(5861, 1003);
	}

	private void removeQueueShortcut() {
		player.getVarsManager().sendVar(4164, 0);
		if (queueShortcut != null && queueShortcut.getType() == HEAL_ABILITY_SHORTCUT
				&& queueShortcut.getId(player) == 1)
			player.getVarsManager().sendVarBit(REGENERATION_VARBIT_ID, 0);
		queueShortcut = null;
	}
	...
	private static boolean isMagicAbility(int abilityId) {
		// abilty ids here to distinguish from normal spells
		return (abilityId == 7 || abilityId == 1 || abilityId == 12 || abilityId == 6 || abilityId == 165
				|| abilityId == 3 || abilityId == 166 || abilityId == 171 || abilityId == 5 || abilityId == 2
				|| abilityId == 8 || abilityId == 4 || abilityId == 9 || abilityId == 10 || abilityId == 11
	...
	public void processQueuedShortcut() {
		...
		if (queueShortcut != null)
			processShortcut(queueShortcut, WorldPacketsDecoder.ACTION_BUTTON1_PACKET, getQueueShortcutIndex(), true);
			...
			for (int i = 0; i < 9; i++) {
				...
				if (shortcut != null && (shortcut.getType() == MELEE_ABILITY_SHORTCUT
						|| shortcut.getType() == DEFENCE_ABILITY_SHORTCUT
						|| shortcut.getType() == STRENGTH_ABILITY_SHORTCUT
						|| shortcut.getType() == RANGED_ABILITY_SHORTCUT || shortcut.getType() == HEAL_ABILITY_SHORTCUT
						|| (shortcut.getType() == MAGIC_ABILITY_SHORTCUT && isMagicAbility(shortcut.getId(player))))
						&& !hasCooldown(shortcut)) {

					GeneralRequirementMap data = GeneralRequirementMap.getMap(ClientScriptMap
							.getMap(CS_DATA_ID[shortcut.getType() - 1]).getIntValue(shortcut.getId(player)));
					int abilityType = data.getIntValue(2799);
					if (abilityType != 0 || !hasRequiriments(shortcut.getId(player), data, false, false))
						continue;
	...
	private void pushShortcutOnSomething(Shortcut shortcut, Object target) {
		...
		case MAGIC_ABILITY_SHORTCUT:
			if (target instanceof Item) {
				int slotId = player.getInventory().getItems().lookupSlot(((Item) target).getId());
				if (slotId == -1)
					return;
				Magic.handleSpellOnItem(player, shortcut.getId(player), (byte) slotId);
	...
	public void refreshOtherBars() {
		...
		for (int index = 0; index < otherBars.length; index++) {
			...
			if (active) {
				player.getInterfaceManager().setInterface(true, 1477, 54 + (index * 5), 1670 + index);
			}
			player.getPackets().sendExecuteScript(active ? 8310 : 8320, 1032 + index);
	...
	public void unlockOtherComponents(){
		player.getPackets().sendIComponentSettings(1670, 11, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 16, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 24, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 29, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 37, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 42, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 50, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 55, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 63, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 68, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 76, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 81, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 89, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 94, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 102, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 107, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 115, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 120, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 128, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 133, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 141, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 146, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 154, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 159, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 167, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 172, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 180, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1670, 185, -1, -1, 11108350);
		
		player.getPackets().sendIComponentSettings(1673, 7, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 12, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 20, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 25, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 33, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 38, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 46, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 51, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 59, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 64, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 72, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 77, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 85, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 90, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 98, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 103, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 111, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 116, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 124, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 129, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 137, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 142, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 150, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 155, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 163, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 168, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 176, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1673, 181, -1, -1, 11108350);
		
		player.getPackets().sendIComponentSettings(1671, 8, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 99, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 21, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 112, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 34, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 125, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 47, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 138, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 60, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 151, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 73, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 164, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 86, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1671, 177, -1, -1, 11108350);
		
		
		
		player.getPackets().sendIComponentSettings(1672, 7, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 20, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 33, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 46, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 59, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 72, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 85, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 98, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 111, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 124, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 137, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 150, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 163, -1, -1, 11108350);
		player.getPackets().sendIComponentSettings(1672, 176, -1, -1, 11108350);
	}
	...
	private void refreshBlockIncomingShareOffers() {
		player.getVarsManager().sendVarBit(22085, blockIncomingShareOffers ? 1 : 0);
	}
	...
	private void refresh(int index, boolean force) {
		...
		sendShortcutVar(index, shortcut.getType(), shortcut.getId(player), force);
	}
	...
	private void refresh1(int index, boolean force) {
		...
		sendShortcutVar1(index, shortcut.getType(), shortcut.getId(player), force);
	}
	...
	private void refresh2(int index, boolean force) {
		...
		sendShortcutVar2(index, shortcut.getType(), shortcut.getId(player), force);
	}
	...
	private void refresh3(int index, boolean force) {
		...
		sendShortcutVar3(index, shortcut.getType(), shortcut.getId(player), force);
	}
	...
	private void refresh4(int index, boolean force) {
		...
		sendShortcutVar4(index, shortcut.getType(), shortcut.getId(player), force);
	}
	...
	private void sendShortcutVar(int barIndex, int type, int id, boolean force) {
		if (force) { // to save data from being sent every click lol
			player.getVarsManager().forceSendVar(
					barIndex >= 12 ? (4429 + 2 * currentBar) + (barIndex - 12) : ((823 + 12 * currentBar) + barIndex),
					type != 0 ? -1 : id);
			player.getVarsManager().forceSendVar(
					barIndex >= 12 ? (4415 + 2 * currentBar) + (barIndex - 12) : ((739 + 12 * currentBar) + barIndex),
					type != 0 ? (id << 4 | type) : 0);
		} else {
	...
	private void sendShortcutVar1(int barIndex, int type, int id, boolean force) {
		int bitValue = player.getVarsManager().getBitValue(29138);
		if (force) { // to save data from being sent every click lol
			player.getVarsManager().forceSendVar(barIndex >= 12 ? (4429 + 2 * (bitValue - 1)) + (barIndex - 12)
					: ((823 + 12 * (bitValue - 1)) + barIndex), type != 0 ? -1 : id);
			player.getVarsManager().forceSendVar(barIndex >= 12 ? (4415 + 2 * (bitValue - 1)) + (barIndex - 12)
					: ((739 + 12 * (bitValue - 1)) + barIndex), type != 0 ? (id << 4 | type) : 0);
		} else {
			player.getVarsManager().sendVar(barIndex >= 12 ? (4429 + 2 * (bitValue - 1)) + (barIndex - 12)
					: ((823 + 12 * (bitValue - 1)) + barIndex), type != 0 ? -1 : id);
			player.getVarsManager().sendVar(barIndex >= 12 ? (4415 + 2 * (bitValue - 1)) + (barIndex - 12)
					: ((739 + 12 * (bitValue - 1)) + barIndex), type != 0 ? (id << 4 | type) : 0);
		}
	}
	...
	public void unlockShortcuts(boolean menu) {
		for (int i = 0; i < shortcuts[currentBar].length; i++) {
			player.getPackets().sendIComponentSettings(menu ? 1436 : 1430, (menu ? 16 : 57) + i * 13, -1, 1,
					!lockedBar || menu ? 11108350 : 2195454);
		}
		for (int i = 0; i < shortcuts[currentBar].length; i++) {
			player.getPackets().sendIComponentSettings(1670, 11 + i * 13, -1, 1, 2195454);
			player.getPackets().sendIComponentSettings(1671, 8 + i * 13, -1, 1, 2195454);
			player.getPackets().sendIComponentSettings(1672, 7 + i * 13, -1, 1, 2195454);
			player.getPackets().sendIComponentSettings(1673, 7 + i * 13, -1, 1, 2195454);
		}
		player.getPackets().sendIComponentSettings(1430, 30, -1, 65535, 2);
	}
	...
	private static boolean canSurge(Player player) {
		...
		boolean surgeDisabled = c != null && (c instanceof BorkController || c instanceof DuelArena
				|| c instanceof DTControler || c instanceof FightCaves || c instanceof FightKiln
				|| c instanceof HouseControler || c instanceof StealingCreationController
				|| c instanceof BossInstanceController || c instanceof Wilderness);
		for (int regionId : disabledRegions)
			if (player.getRegionId() == regionId)
				surgeDisabled = true;
		if (surgeDisabled)
			player.getPackets()
					.sendGameMessage("You are in an area which is preventing you from performing that ability.");
	...
	private static WorldTile getSurgeTile(Player player, int start, int end, boolean increment, Entity target) {
		...
		for (int steps = start; increment ? steps < end : steps > end; steps += (increment ? 1 : -1)) {
			WorldTile step = new WorldTile(player.getX() + (dirs[0] * steps), player.getY() + (dirs[1] * steps),
					player.getPlane());
			if (target != null
					&& Utils.colides(target.getX(), target.getY(), target.getSize(), step.getX(), step.getY(),
							player.getSize())
					|| !player.clipedProjectile(step, true)
					|| !World.isTileFree(step.getPlane(), step.getX(), step.getY(), player.getSize()))
				break;
			lastStep = step;
		}
		return lastStep;
