...
public class Trade {
	...
	// for implementing loan
	// player loan cid - 38
	// other player loan cid - 39
	public static final int PLAYER_OFFER_COMPONENT_ID = 26;
	public static final int OTHER_OFFER_COMPONENT_ID = 29;
	...
	public void openTrade(final Player target) {
		synchronized (this) {
			...
			//getPackets().sendCSVarString(2519, Speedbabe22);
			sendInterItems();
			sendOptions();
			sendTradeModified();
	...
	public void removeItem(final int slot, int amount) {
		/*DEBUG*/
		/*player.getInterfaceManager().removeInterface(336);
		player.getInterfaceManager().setInterface(false, 1477, PLAYER_ITEMS_COMPONENT_ID_T, 336);
		Logger.log("Debug components trade", "Now items on "+PLAYER_ITEMS_COMPONENT_ID_T);
		sendOptions();
		PLAYER_ITEMS_COMPONENT_ID_T++;
	...
	public void addItem(Item item) {
		if(Settings.DEBUG) Logger.log(this, "Player "+player.getDisplayName()+" Added "+item.getName());
		synchronized (this) {
	...
	public void addItem(ItemsContainer<Item> nitems) {
		
		synchronized (this) {
			Item[] itemsBefore = items.getItemsCopy();
			for (int i = 0; i < nitems.getSize(); i++)
			{
				Item item = nitems.get(i);
				if (item == null || !isTrading())
					continue;
				if (!ItemConstants.isTradeable(item)) {
					player.getPackets().sendGameMessage("That item isn't tradeable.");
	...
	public void sendOptions() {
		// ComponentsID (Interface 335)
		// 19 - player items box GUI
		// 20 - other player items box GUI
		// 22 - Accept button GUI
		// 61 - Accept button
		// 24 - player items
		// 27 - other player items
		// 67 - Decline button
		// 25 - the menu above
		// 74 - Put all items button
		// 81 - Put money button
		...
		player.getPackets().sendInterSetItemsOptionsScript(335, PLAYER_OFFER_COMPONENT_ID, 90, 4, 7, "Remove", "Remove-5", "Remove-10", "Remove-All", "Remove-X", "Value");
		player.getPackets().sendInterSetItemsOptionsScript(335, OTHER_OFFER_COMPONENT_ID, 90, true, 4, 7, "Value");
		player.getPackets().sendIComponentSettings(335, PLAYER_OFFER_COMPONENT_ID, 0, 27, 1150); //876
		player.getPackets().sendIComponentSettings(335, OTHER_OFFER_COMPONENT_ID, 0, 27, 1026); //876
		player.getPackets().sendIComponentSettings(336, 0, 0, 27, 1278);
		//player.getPackets().sendIComponentSettings(335, 61, -1, -1, 1026);//876
		//player.getPackets().sendIComponentSettings(335, 67, -1, -1, 1026);//876
		//player.getPackets().sendIComponentSettings(335, 74, -1, -1, 1026);
	}
	...
	public void refreshFreeInventorySlots() {
		...
		target.getPackets().sendIComponentText(335, 9, "has " + (freeSlots == 0 ? "no" : freeSlots) + " free" + "<br>inventory slots");
	}
