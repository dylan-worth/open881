...
public final class Skills implements Serializable {
	...
	public void unlockSkills(boolean menu) {
		player.getPackets().sendIComponentSettings(menu ? 320 : 1466, menu ? 9 : 7, 0, 27, 2);
	}
	...
	public int getLevelForXp(int skill) {
		return getLevelForXp(xp[skill], skill == DUNGEONEERING || skill == INVENTION || skill == SLAYER ? 120 : 99);
	}
	...
	public double addXp(int skill, double exp, boolean forceRSXp) {
		...
		player.getVarsManager().sendVar(2044, (int) (bonus * 10));
	...
	public int getIconValue(int skill) {
		...
		if (skill == Skills.RANGE)
			return 8388611 + (8388608 * (getLevel(Skills.RANGE) - 1));
		...
		if (skill == Skills.SUMMONING)
			...
			return 8388635 + (8388608 * (getLevel(Skills.INVENTION) - 1));
		}
		return 27;
	}
