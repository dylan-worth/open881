...
public final class Commands {
	...
	public static boolean processPunishmentCommand(final Player player, String[] cmd, boolean console,
		...
		case "openhistory":
			...
			return true;
		case "unlock":
			for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(105); x++) {
				player.getPackets().sendUnlockIComponentOptionSlots(105, x, -1, 0, 0, 1);
				// player.getPackets().sendIComponentSettings(105, x, 0, 500,
				// 2);
			}
			return true;
		case "teletome":
		...
		if (clientCommand) {
			...
			case "kkkk":
				try {
					int x = 0;
					for (int i = x; i < 10; i++) {
						player.getPackets().sendCSVarInteger(1928, i);
						Thread.sleep(1000);
			...
			case "keys":
				player.getTreasureHunter().setBoughtKeys(150);
				break;
			case "th":
//				try {
//					for (int i = 410; i < 417; i++) {
				player.getInterfaceManager().setWindowInterface(415, 1253);
//						Thread.sleep(2500);
//						System.out.println(i);
//					}
...
//				}
				return true;
			case "rank":
				player.getDonationManager().increaseDonationAmount(19);
				return true;
			case "hideall":
				player.getInventory().addItem(new Item(1517, 1001));
				return true;
			case "dmg":
				for (NPC npcs : World.getNPCs()){
					if (npcs.getId() == 19464)
						npcs.applyHit(new Hit(player, Integer.parseInt(cmd[1]), HitLook.REGULAR_DAMAGE));
				}
				return true;
				case "treasurehunter":
				case "sof":
					if (player.getUsername().equalsIgnoreCase("timelord")
							|| player.getUsername().equalsIgnoreCase("dylanpage95")
				...
				case "devnpc":
					...
					return true;
			case "controller":
				System.out.println(player.getControlerManager().getControler());
				return true;
			case "skybox":
				player.getPackets().sendSkyBox(Integer.parseInt(cmd[1]));
				return true;
			case "helwyr":
				HelwyrInstance instance = new HelwyrInstance(player, 600, 10, -1, -1, 22438, false);
				instance.constructInstance();
				return true;
			case "vindicta":
				VindictaInstance instance1 = new VindictaInstance(player, 600, 10, -1, -1, 22460, false);
				instance1.constructInstance();
				return true;
			case "greg":
				GregorovicInstance instance2 = new GregorovicInstance(player, 600, 10, -1, -1, 22443, false);
				instance2.constructInstance();
				return true;
			case "twins":
				TwinFuriesInstance instance3 = new TwinFuriesInstance(player, 600, 10, -1, -1, 22455, false);
				instance3.constructInstance();
				return true;
			case "hidebutton":
				player.getPackets().sendHideIComponent(1487, Integer.parseInt(cmd[1]), true);
				System.out.println(Utils.getInterfaceDefinitionsComponentsSize(1487));
				return true;
			case "unlockclan":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(1096); x++)
					player.getPackets().sendIComponentSettings(1096, x, 0, 500, Integer.parseInt(cmd[1]));
				return true;
			case "spawnrandom":
				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++)
					player.getInventory().addItem(new Item(x, 1));
				return true;
			case "unlockbars":
				player.getActionbar().unlockOtherComponents();
				return true;
			case "dia":
				player.getDialogueManager().startDialogue("Mandrith");
				return true;
			case "scene":
				System.out.println("lol");
				player.getPackets().sendGameScene(true);
				return true;
			case "setting":
				player.getPackets().sendIComponentSettings(1430, 30, -1, 65535, 0);
				return true;
			case "settingsfamiliar":
				player.getPackets().sendIComponentSettings(1430, 30, -1, 65535, 2);
				player.getPackets().sendCSVarInteger(1436, 1);
				player.getPackets().sendCSVarString(2516, "Howl");
				player.getPackets().sendCSVarString(2517, "Makes NPCs flee for a short period.");
				player.getPackets().sendHideIComponent(662, 8, true);
				player.getPackets().sendHideIComponent(662, 33, true);
				player.getPackets().sendHideIComponent(662, 175, false);
				player.getPackets().sendHideIComponent(662, 49, true);
				return true;
			case "sizee":
				player.getPackets().sendOtherPlayerOnIComponent(1831, Integer.parseInt(cmd[1]), player);
				return true;
			case "placeit":
				player.getInterfaceManager().setInterface(true, 1560, Integer.parseInt(cmd[1]), 1831);
				return true;
			case "dragit":
				player.getInterfaceManager().setInterface(true, 1477, Integer.parseInt(cmd[1]), 1490);
				return true;
			case "examine":
				...
				System.out.println(Utils.getInterfaceDefinitionsComponentsSize(1558));
				return true;
			case "playercomps":
				player.getPackets().sendOtherPlayerOnIComponent(1558, Integer.parseInt(cmd[1]), player);
				return true;
			case "unlockprayers":
				player.getVarsManager().sendVar(3275, Integer.MAX_VALUE);
				return true;
			case "dump":
				IComponentDefinitions.getInterface(1458);
				return true;
			case "playerhead":
				player.getPackets().sendPlayerHeadOnIComponent(1369, Integer.parseInt(cmd[1]));
				return true;
			case "playercomp":
				player.getPackets().sendOtherPlayerOnIComponent(1558, Integer.parseInt(cmd[1]), World.getPlayer("tom"));
				return true;
			case "entityinter":
				player.getPackets().sendEntityInterface(player.getCombatDefinitions().getCurrentTarget(), true, 1477,
						422, 1490);
				return true;
			case "unlockall":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(320); x++)
					player.getPackets().sendIComponentSettings(1466, x, 0, 26, 2);
				return true;
			case "model":
				// for (int x = Integer.parseInt(cmd[1]); x <
				// Integer.parseInt(cmd[2]); x++)
				player.getPackets().sendIComponentModel(20, Integer.parseInt(cmd[1]),
						player.getAppearence().isMale()
								? ItemDefinitions.getItemDefinitions(20767).getMaleWornModelId1()
								: ItemDefinitions.getItemDefinitions(20767).getFemaleWornModelId1());
				return true;
			case "compsettings":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(1477); x++)
					player.getPackets().sendIComponentSettings(1477, x, 0, 50, 2);
				return true;
			case "compsettings3":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(105); x++)
					player.getPackets().sendUnlockIComponentOptionSlots(105, x, -1, 0, 0, 1);
			...
			case "getest":
				...
				return true;
			case "compsettings1":
				player.getPackets().sendIComponentSettings(1674, 126, 0, 8, 2359296);
				player.getPackets().sendIComponentSettings(1674, 127, 0, 8, 2);
				player.getPackets().sendIComponentSettings(1674, 100, 0, 8, 2359296);
				player.getPackets().sendIComponentSettings(1674, 101, 0, 8, 2);
				return true;
			case "adrenaline":
				player.getCombatDefinitions().increaseSpecialAttack(Integer.parseInt(cmd[1]));
				return true;
			case "closeinter":
				player.getPackets().closeInterface(Integer.parseInt(cmd[1]));
				return true;
			case "playerinter":
				System.out.println("lol");
			...
			case "appearence":
				...
				return true;
			case "retrocapes":
				System.out.println(player.getRetroCapes());
				return true;
			case "spritecomp":
				System.out.println("lol");
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(1663); x++)
					player.getPackets().sendSpriteOnIComponent(1663, x, 18860);
				return true;
			...
			case "h":
				...
				return true;
			case "valuevar":
				System.out.println(player.getVarsManager().getValue(Integer.parseInt(cmd[1])));
				return true;
			case "override":
				player.getEquipment().getCosmeticItems().set(Integer.parseInt(cmd[1]), new Item(34610, 1));
				player.getAppearence().generateAppearenceData();
				return true;
			case "slider":
			...
			case "loopcsvars":
				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++) {
					player.getPackets().sendCSVarInteger(x, Integer.parseInt(cmd[3]));
				}
				return true;
			case "unlockbuttons":
				System.out.println(Utils.getInterfaceDefinitionsComponentsSize(1674));
				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++)
					player.getPackets().sendIComponentSettings(1674, x, -1, 500, Integer.parseInt(cmd[1]));
				return true;
			case "hidebuttons":

				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++)
					player.getPackets().sendHideIComponent(1623, Integer.parseInt(cmd[1]), true);
				return true;
			case "lootbeamvalue":
				System.out.println(player.getLootBeamValue());
				// player.getPackets().sendIComponentText(1623, 121, "" +
				// player.getLootBeamValue());
				return true;
			case "tilehash":
				System.out.println(player.getTileHash());
				return true;
			case "boxinter":
				player.getInterfaceManager().setWindowInterface(17, 1177);
				System.out.println("box inter");
				return true;
			case "lootvalueloop":
				// for (int x = Integer.parseInt(cmd[1]); x <
				// Integer.parseInt(cmd[2]); x++)
				// player.getPackets().sendIComponentInputInteger(1623, x, 10);
				return true;
			case "lootvalue":
				// player.getPackets().sendIComponentInputInteger(1623,
				// Integer.parseInt(cmd[1]), 10);
				return true;
			case "setallstats":
				for (int x = 0; x < 26; x++)
					player.getSkills().addXp(x, Integer.parseInt(cmd[1]));
				return true;
			case "setcrafting":
				player.getSkills().addXp(Skills.CRAFTING, Integer.parseInt(cmd[1]));
				return true;
			case "freeslots":
				System.out.println(player.getInventory().getFreeSlots());
				return true;
			case "getlootinter":
				System.out.println(player.getLootInterface());
				return true;
			case "getarealoot":
				System.out.println(player.getAreaLoot());
				return true;
			case "testcommand":
				for (int x = 0; x < Integer.parseInt(cmd[1]); x++)
					player.getPackets().sendIComponentSettings(1670, x, 0, 50, 1);
				System.out.println("hiti");
				return true;
			case "messagee":
				player.getPackets().sendGameMessage("<col=008000><shad>Latest Update: Fully converted 831 to 876.");
				return true;
			case "hidecompt":
				player.getPackets().sendHideIComponent(1458, Integer.parseInt(cmd[1]), true);
				return true;
			case "restorecomps":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(1458); x++)
					player.getPackets().sendHideIComponent(1458, x, false);
				return true;
			case "skillcounter":
				player.getPackets().sendHideIComponent(1443, 10, true);
				player.getPackets().sendHideIComponent(1443, 19, true);
				player.getPackets().sendHideIComponent(1443, 28, true);
				player.getPackets().sendHideIComponent(1443, 37, true);
				player.getPackets().sendHideIComponent(1443, 46, true);
				player.getPackets().sendHideIComponent(1443, 58, true);
				player.getPackets().sendHideIComponent(1443, 66, true);
				player.getPackets().sendHideIComponent(1443, 75, true);
				player.getPackets().sendHideIComponent(1443, 83, true);
				player.getPackets().sendHideIComponent(1443, 10, false);
				return true;
			case "settings":
				for (int i = 1400; i < 1500; i++) {
					for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(i); x++)
						player.getPackets().sendIComponentSettings(i, x, -5, 10, Integer.parseInt(cmd[1]));
				}
				System.out.println("xd");
				return true;
			case "columns":
				player.getVarsManager().sendVar(3704, 65792);
				player.getVarsManager().sendVar(4736, -1);
				player.getVarsManager().sendVar(4734, -1);
				player.getVarsManager().sendVar(4735, -1);
				return true;
			case "clock":
				player.getPackets().sendHideIComponent(635, Integer.parseInt(cmd[1]), true);
				return true;
			case "restore":
				for (int x = 0; x < Utils.getInterfaceDefinitionsComponentsSize(635); x++)
					player.getPackets().sendHideIComponent(635, x, false);
				return true;
			case "menuinter":
				player.getInterfaceManager().setWindowInterface(Integer.parseInt(cmd[1]), 1448);
			...
			case "history":
				...
				return true;
			case "hidecomp":
				player.getPackets().sendHideIComponent(1638, Integer.parseInt(cmd[1]), true);
				return true;
			case "cursesprayers":
			...
			case "loopvars":
				if (Integer.valueOf(cmd[3]) == 1) {
					for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++) {
						player.getVarsManager().sendVar(x, 67108864);
					}
				} else {
					for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++) {
						player.getVarsManager().sendVar(x, 1140850688);
					}
				}
			...
			case "loopvars1":
				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++)
					player.getVarsManager().sendVar(x, Integer.parseInt(cmd[3]));

				return true;
			case "loopvarbits":
				for (int x = Integer.parseInt(cmd[1]); x < Integer.parseInt(cmd[2]); x++) {
					player.getVarsManager().sendVarBit(x, Integer.parseInt(cmd[3]));
				}
				return true;
			case "legacy":
				System.out.println(player.isInLegacyCombatMode());
				return true;
			case "curses":
			...
			case "sprite":
				for (int i = 0; i < 100; i++)
					player.getPackets().sendSpriteOnIComponent(335, i, 1);
				return true;
			case "prjdebugmisc":
			...
			case "givespinsall":
				...
				if (type == 0) {
					World.sendWorldMessage("<img=7><col=ff0000>News: " + player.getDisplayName()
							+ " has given everyone that's online " + cmd[2] + " earned keys!", false);
					for (Player p : World.getPlayers()) {
						if (p == null || !p.isRunning())
							continue;
						p.getTreasureHunter().handleEarnedKeys(Integer.parseInt(cmd[2]));
					}
				} else if (type == 1) {
					World.sendWorldMessage("<img=7><col=ff0000>News: " + player.getDisplayName()
							+ " has given everyone that's online " + cmd[2] + " bought keys!", false);
					for (Player p : World.getPlayers()) {
						if (p == null || !p.isRunning())
							continue;
						p.getTreasureHunter().handleBoughtKeys(Integer.parseInt(cmd[2]));
					}
				} else if (type == 2) {
					World.sendWorldMessage(
							"<img=7><col=ff0000>News: " + player.getDisplayName() + " has reset that's online keys!",
							false);
					for (Player p : World.getPlayers()) {
						if (p == null || !p.isRunning())
							continue;
						p.getTreasureHunter().reset();
					}
				}
				return true;
			case "loyalty":
				player.getLoyaltyPoints().giveDailyPoints();
				return true;
			case "getspins":
				type = Integer.parseInt(cmd[1]);
				if (type == 0)
					player.getTreasureHunter().giveDailyKeys();
				else if (type == 1)
					player.getTreasureHunter().handleEarnedKeys(Integer.parseInt(cmd[2]));
				else if (type == 2)
					player.getTreasureHunter().handleBoughtKeys(Integer.parseInt(cmd[2]));
				else if (type == 3)
					player.getTreasureHunter().reset();
				else if (type == 4)
					player.getTreasureHunter().setDailyKeys(Integer.parseInt(cmd[2]));
				return true;
			case "ugd":
			...
			case "ecotestcutscene":
				...
				return true;
			case "intro":
				player.getCutscenesManager().play("IntroScene");
				return true;
			case "istest":
			...
			case "isprite":
				player.getPackets().sendSpriteOnIComponent(Integer.valueOf(cmd[1]), Integer.valueOf(cmd[2]),
			...
			case "compdump":
				if (cmd.length < 2 || cmd.length > 3) {
					player.getPackets().sendPanelBoxMessage("Use: ::compo interfaceId [compId]");
					return true;
				}

				int s = Utils.getInterfaceDefinitionsComponentsSize(Integer.parseInt(cmd[1]));
				player.getPackets().sendPanelBoxMessage("Components number: " + Integer.toString(s));
				if (Integer.parseInt(cmd[2]) >= s) {
					player.getPackets().sendPanelBoxMessage("CompId too high!");
				}
				FileOutputStream fos = null;
				try {
					File f = null;
					if (cmd.length == 2)
						f = new File("dump/compo/" + Integer.parseInt(cmd[1]) + ".dat");
					else
						f = new File(
								"dump/compo/" + Integer.parseInt(cmd[1]) + "_" + Integer.parseInt(cmd[2]) + ".dat");
					f.getParentFile().mkdirs();
					f.createNewFile();
					Logger.log("File", "Create file at : " + f.getAbsolutePath());
				...
				try {
					if (cmd.length == 2)
						fos.write(Cache.STORE.getIndexes()[3].getArchive(Integer.parseInt(cmd[1])).getData());
					else
						fos.write(Cache.STORE.getIndexes()[3].getFile(Integer.parseInt(cmd[1]),
			...
			case "shutdown":
				...
				GameLauncher.shutdown(delay, true, true);
				return true;
			...
			case "sz":
			case "staffzone":
				Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(3744, 6626, 0));//1258 7439 0
				return true;
			case "devzone":
				Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(1258, 7439, 0));
				return true;
			case "newzone":
				Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(3882, 7646, 0));
				return true;
			case "fightkiln":
 ...
 static boolean processModCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "sendhome":
				...
				target = World.getPlayerByDisplayName(name);
				if (target == null
						|| (target.getControlerManager() != null && target
	...
	public static boolean processSupportCommands(Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "sz":
			case "staffzone":
				Magic.sendNormalTeleportSpell(player, 1, 0, new WorldTile(3744, 6626, 0));
				return true;
			case "dz":
			case "donatorzone":
	...
	public static boolean processNormalCommand(final Player player, String[] cmd, boolean console,
		...
		if (clientCommand) {
			...
			case "myrights":
				...
				return true;
			case "hideall":
				player.getInventory().addItem(new Item(1518, 1001));
				return true;
			case "mode":
