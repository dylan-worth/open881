...
public class CompletionistCapeManager implements Serializable {
	...
	public void sendComplete() {
		...
		World.sendNews(player, player.getDisplayName() + " has just successfully achieved the trimmed completionist cape!", 0);
	}
	...
	public void increaseRequirement(Requirement req, int amount) {
		...
		if (progress.get(req) >= req.amount) {
			player.getPackets().sendGameMessage("Congratulations, you have successfully finished the " + req.toString().toLowerCase().replace("_", " ") + " completionist cape requirement!");
			World.sendNews(player, player.getDisplayName()
					+ " has just successfully finished the "
					+ req.toString().toLowerCase().replace("_", " ")
					+ " completionist cape requirement!", 0);
		}
	}
