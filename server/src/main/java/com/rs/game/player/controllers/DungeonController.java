			...
			public void run() {
				if (loop == 0) {
					...
					if (previousOvlEffect != null)
						player.getEffectsManager().startEffect(new Effect(EffectType.OVERLOAD, previousOvlEffect.getCycle()));
					if (previousRenewalEffect != null)
						player.getEffectsManager().startEffect(new Effect(EffectType.OVERLOAD, previousRenewalEffect.getCycle()));
		...
		case "anvil":
			for (int index = 0; index < Smithing.BARS[1].length; index++) {
				if (Smithing.BARS[1][index] == item.getId()) {
					player.getDialogueManager().startDialogue("SmithingD", object, index,true);
					break;
				}
			}
			return true;
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 1465) {
				...
				return false;
			} else if (interfaceId == Inventory.INVENTORY_INTERFACE || interfaceId == Inventory.INVENTORY_INTERFACE_2) {
				Item item = player.getInventory().getItem(slotId);
				if (item == null || item.getId() != slotId2)
					return false;
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					/*int leatherIndex = LeatherCraftingD.getIndex(item.getId());//TODO
					if (leatherIndex != -1) {
						player.getDialogueManager().startDialogue("LeatherCraftingD", leatherIndex, true);
						return false;
					}*/
				...
				Summoning.handleInfusionOptions(player, packetId, componentId, slotId, slotId2, true);
				return false;
