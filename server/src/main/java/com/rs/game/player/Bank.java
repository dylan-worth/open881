...
public class Bank implements Serializable {
	...
	public void switchItem(int fromSlot, int toSlot, int fromComponentId,
			int toComponentId) {
		if (toSlot == -1) {
			int toTab = toComponentId >= 148 ? 8 - (156 - toComponentId)
					: 9 - ((toComponentId - 46) / 2);
			if (toTab < 0 || toTab > 9)
				return;
			System.out.println(8 - (156 - toComponentId));
			System.out.println("Tab: " + toTab);
	...
	public void openBank() {
		if (!hasVerified(2))
			return;
		if (Settings.DEBUG)
			Logger.log(this, "Player " + player.getDisplayName() + " opened bank.");
		lastContainerCopy = null;
		player.getInterfaceManager().sendBankInterface(762);
		player.getInterfaceManager().setInterface(true, 762, 120, 1463);
		...
		refreshTotalSize();
		if (player.getFamiliar() != null && player.getFamiliar().getBob() != null && player.getFamiliar().isBoB(player))
			player.getPackets().sendCSVarInteger(4619, player.getFamiliar().getBOBSize());
		else {
	...
	public void withdrawBoBItem(int[] slots, int quantity) {
		...
		if (!ItemConstants.isTradeable(item)) {
			player.getPackets().sendGameMessage("You cannot store untradeable items to your familiar.");
			return;
		}
		...
		if (noted || defs.isStackable()) {
			if (player.getFamiliar().getBob().getBeastItems().containsOne(item)) {
				int slot = player.getFamiliar().getBob().getBeastItems().getThisItemSlot(item);
				Item bobItem = player.getFamiliar().getBob().getBeastItems().get(slot);
				assert bobItem != null;
				if (bobItem.getAmount() + item.getAmount() <= 0) {
					item.setAmount(Integer.MAX_VALUE - bobItem.getAmount());
					player.getPackets().sendGameMessage("Not enough space in your BoB-inventory.");
				}
	...
	public void depositBoBItem(int bobSlot, int quantity, boolean refresh) {
		...
		if (item == null)
			return;
		int amt = player.getFamiliar().getBob().getBeastItems().getNumberOf(item);
		if (amt < quantity)
		...
		if (bankedItem != null) {
			if (bankedItem.getAmount() + item.getAmount() <= 0) {
				...
				player.getPackets().sendGameMessage("Unable to bank '" + item.getName() + "'.");
				return;
			}
		...
		player.getFamiliar().getBob().deleteItem(new Item(originalId, item.getAmount(), 0));
	...
	public void depositItem(int invSlot, int quantity, boolean refresh) {
		...
		if (item == null)
			return;
		if (player.getTemporaryAttributtes().contains("ClanBank") && !ItemConstants.isTradeable(item)) {
			player.getPackets().sendGameMessage("Untradeable items can't be deposited into a clan bank.");
			return;
		}
		...
		player.getInventory()
				.deleteItem(new Item(originalId,
						item.getAmount(), /* item.getDegrade() */
						0));
		addItem(item, refresh);
		player.getVarsManager().sendVar(3922, item.getAmount());
		setLastDepositX(item.getAmount());
	...
	public int[] getRealSlot(int slot) {
		if (Settings.DEBUG)
			Logger.log("Bank slot", "getRealSlot: " + slot);
		if (Settings.DEBUG)
			Logger.log("Tabs", "Number of tabs: " + bankTabs.length);
	...
	public void unlockButtons() {
		...
		for (int componentId = 281; componentId < 291; componentId++) {
			player.getPackets().sendIComponentSettings(762, componentId, 0, 1, 10485760);
		}
	}
	...
	public int getLastX() {
		return lastWithdrawX;
	}

	public void setLastX(int lastX) {
		this.lastWithdrawX = lastX;
	}
