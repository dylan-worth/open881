...
public final class MainFile {
	...
	public byte[] getArchiveData(int archiveId) {
		...
		Path src = Paths.get("data", "cache", Integer.toString(id), Integer.toString(archiveId) + ".dat");
		if (!Files.exists(src)) {
			return null;
		}
		...
		// read file from main_file_cache
		synchronized (data) {
			try {
	...
	public int getArchivesCount() throws IOException {
		...
		Path src = Paths.get("data", "cache", Integer.toString(id));
		if (Files.exists(src)) {
			try (Stream<Path> stream = Files.list(src)) {
				// return last file id {id}.dat
				Optional<Path> last = stream.sorted((a, b) ->
						Integer.parseInt(a.getFileName().toString().replaceAll(".dat", "")) -
						Integer.parseInt(b.getFileName().toString().replaceAll(".dat", "")))
					.reduce((first, second) -> second);
				if (last.isPresent()) {
					return Integer.parseInt(last.get().getFileName().toString().replaceAll(".dat", "")) + 1;
				}
			}
		}

		// read main_file_cache len
		synchronized (index) {
